PRO GPLOT, X=x, Y=y, COL=col, STYLE=style, LABELS=labels, SYM=sym, $
	   ...
	   THICK=thick, NOLINES=nolines, LEGCOL=legcol, $
...
IF (N_ELEMENTS(TEXT) GT 0) THEN BEGIN
 IF (N_ELEMENTS(TEXT) GT 1) THEN BEGIN
  PRINT, ''
  PRINT, 'GPLOT ERROR - TEXT=text must have just one value.'
  HELP, TEXT
  PRINT, ''
  STOP
 ENDIF
 IF ((N_ELEMENTS(X) GT 1) OR (N_ELEMENTS(Y) GT 1)) THEN BEGIN
  PRINT, ''
  PRINT, 'GPLOT ERROR - X=x and Y=y must have just one value each when plotting text.'
  HELP, X, Y
  PRINT, ''
  STOP
 ENDIF
...
IF (N_ELEMENTS(TEXT) EQ 1) THEN BEGIN
 ...
 ENDIF ELSE BEGIN
  ...
  ypt=pt(1)
 ENDELSE  
 com='XYOUTS, xpt, ypt-y_extra, text'
...
FOR i=0, nlines-1 DO BEGIN
 ...
 com=com+'THICK=thick, COLOR=linecol'
 IF (NOT KEYWORD_SET(DEVICE)) THEN com=com+', NOCLIP=0'
 IF KEYWORD_SET(STYLE) THEN com=com+', LINESTYLE=STYLE(i)'
 IF (N_ELEMENTS(SYM) GE 1) THEN com=com+', PSYM=symmult*GSYM(sym(i))'
 com=com+', SYMSIZE=symsize'
 IF KEYWORD_SET(DEVICE) THEN com=com+', /DEVICE'
...
IF KEYWORD_SET(LEGEND) THEN BEGIN
 ...
 com='LEGEND, LABELS=labels, COL=col, LEGPOS=legpos'
