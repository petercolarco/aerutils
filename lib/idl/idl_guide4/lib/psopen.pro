PRO psopen, FILE=file, PORTRAIT=portrait, LETTER=letter, $
       A5=a5, EPS=eps, $
       ...
       CB_WIDTH=cb_width, CB_HEIGHT=cb_height, BORDER_THICK=border_thick, FONT=font, CHARSIZE=charsize, $
       BOLD=bold, ITALIC=italic, TFONT=tfont, TCHARSIZE=tcharsize, $
       TBOLD=tbold, TITALIC=titalic, CYLMAX=cylmax, DEF=def 
;Procedure to setup a postscript file
;(C) NCAS 2008
...
validnames=['portrait', 'letter', 'xplots', 'yplots', 'xoffset', 'yoffset', $
	    ...
	    'tfont', 'tcharsize', 'tbold', 'titalic']

;Read in system and user setup files
file1=GETENV('GUIDE_LIB')+'/idl_guide4.def'
file2=GETENV('HOME')+'/.idl_guide4.def'
files=[file1, file2]
...
IF (N_ELEMENTS(FONT) EQ 1) THEN BEGIN
 ...
 ENDELSE
ENDIF
IF (N_ELEMENTS(CHARSIZE) EQ 1) THEN BEGIN
 guide_charsize=FLOAT(charsize)
 guide_tcharsize=FLOAT(charsize)
ENDIF 
...
IF (N_ELEMENTS(TCHARSIZE) EQ 1) THEN guide_tcharsize=FLOAT(tcharsize)
...
;Modify the character and line thckness parameters to suit IDL.
...
ENDIF
guide_charsize=1.0*guide_charsize/sf
guide_tcharsize=1.3*guide_tcharsize/sf
