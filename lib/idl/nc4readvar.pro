...
;     On first successful read, save information about returned
;     variable to fill in any missing dates
...
;     If first file, this is a fail
      if(ifile eq 0) then return
;     Otherwise, return an array filled with missing values
      if(varlist[0] ne '') then varval = [varval,make_array(size=sz,val=!values.f_nan)]
      time    = [time,time[0]]
     endelse
    endfor
