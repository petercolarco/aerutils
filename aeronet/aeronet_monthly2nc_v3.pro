...
; saved here.
  ...
  dataDir = '/misc/prc10/AERONET/V3/AOT/AOD/AOD20/MONTHLY/'
...
; Parse the filenames for start and end dates
  ...
  len = strlen(dataDir)
  dateBeg = strmid(filelist,len,8)
  dateEnd = strmid(filelist,len+9,8)
...
; to the locations listed in the latest of the end dates
  ...
  loc0 = strmid(filelist[b0],len+18,100)
  loc1 = strmid(filelist[b1],len+18,100)
...
; Structure of desired output file
  ...
  nchannels = 22
  channels = [1640, 1020, 870, 865, 779, 675, 667, 620, 560, 555, 551, 532, 531, 510, 500, $
              490, 443, 440, 412, 400, 380, 340]
  nangpairs = 6
  angstrompairs = ['440-870', '380-500', '440-675', '500-870', '340-440', $
                   '440-675 polar']

; Create netcdf file for output
  cdfid = ncdf_create(datadir+'../../../../LEV30/aot_monthly.nc', /clobber)
   ...
   idANGm      = NCDF_VARDEF(cdfid,'angstrom_parameter_of_daily_average', [idLoc,idAngPair,idTime], /float)
   idNdangs    = NCDF_VARDEF(cdfid,'angstrom_number_of_days', [idLoc,idAngPair,idTime], /long)
   idNobsang   = NCDF_VARDEF(cdfid,'angstrom_number_of_observations', [idLoc,idAngPair,idTime], /long)
   ...
   ncdf_attput, cdfid, idANGm, 'long_name', 'Angstrom Parameter mean of daily average'
   ncdf_attput, cdfid, idNdangs, 'long_name', 'Angstrom Parameter Number of days of measurement in month'
   ncdf_attput, cdfid, idNobsang, 'long_name', 'Angstrom Parameter Number of observations in month'
...
;   create data table to write
    ...
    angm  = make_array(nangpairs,n_elements(date),val=-9999.)
    ndang = make_array(nangpairs,n_elements(date),val=-9999)
    nobsang = make_array(nangpairs,n_elements(date),val=-9999)

    filename = dataDir + dateBeg[0] +'_'+uniqfileend[nFileEnd-1] +'_'+loc1[iloc]
    read_aeronet_monthly_v3, filename, location, lon, lat, elev, nmeas, pi, pi_email, $
     dateIn, aotmIn, ndaysIn, nobsIn, angmIn, ndangIn, nobsangIn
...
;    Check dates for overwrite
     ...
     if(a[0] ne -1) then begin
      ...
      nobs[*,a] = nobsIn[*,idate]
      angm[*,a] = angmIn[*,idate]
      ndang[*,a] = ndangIn[*,idate]
      nobsang[*,a] = nobsangIn[*,idate]
     endif
    endfor
...
;   and write...
    ...
    ncdf_varput, cdfid, idangm, aotm, count=[1,nangpairs,n_elements(date)], offset=[iloc,0,0]
    ncdf_varput, cdfid, idndangs, ndang, count=[1,nangpairs,n_elements(date)], offset=[iloc,0,0]
    ncdf_varput, cdfid, idnobsang, nobsang, count=[1,nangpairs,n_elements(date)], offset=[iloc,0,0]
