...
# extractions at present.
  ...
  set expid  = dR_SMOKE
  set inpdir = /misc/prc15/colarco/$expid/inst2d_carma_x/
  set outdir = $inpdir

  set varsext = DUEXTTAU,SSEXTTAU,BCEXTTAU
  set varssca = DUSCATAU,SSSCATAU,SUSCATAU,BCSCATAU,OCSCATAU
  set varscol = DUCMASS,SSCMASS,BCCMASS,OCCMASS,SO4CMASS,SO2CMASS
  set varssfc = DUSMASS,SSSMASS,BCSMASS
  set varstot = TOTEXTTAU,TOTSCATAU,TOTANGSTR  #,PLS,PCU,SLP

  set vars    = $varsext,$varssca,$varscol,$varssfc
  set vars    = duexttau,ssexttau,bcexttau,dusmass,sssmass,bcsmass,totexttau,totscatau,totangstr

  foreach years ( 2007 )

  ./extract_stations.x -rc aeronet_locs.dat \
    -i $inpdir/Y%y4/M%m2/$expid.inst2d_carma_x.%y4%m2%d2_%h200z.nc4 \
    -o $outdir/$expid.inst2d_hwl.aeronet.%s.%y4.nc4 \
    -e $expid \
    -nymd ${years}0801 -nhms 000000 -timestep 010000 -lastnymd ${years}0930 \
    -vars $vars
