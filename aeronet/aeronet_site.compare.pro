...
;   readmodel_aeronet, modeltemplate1, locwant, nymd, nhms, lambdabase, $
;                      varwant, modeldate, modelAOT1, tinc=tinc
;   modelaot1 = total(modelaot1,2)
...
;   oplot, indgen(nt), modelaot1, thick=4, color=84
...
;   plots, alog(aeronetaot[a]+.01), alog(modelAOT1[a]+.01), psym=8
...
; Make a map
  loadct, ct
  map_set, 0, -90, /cont
  usersym, 2.*[-1,0,1,0,-1],2.*[0,-1,0,1,0], /fill, color=0
  for iloc = 0, nlocs-1 do begin
    plots, lon[iloc], lat[iloc], psym=8, noclip=0
    label = location[iloc]
    xyouts, lon[iloc]+2.5, lat[iloc]-1, label, $
