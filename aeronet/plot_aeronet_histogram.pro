  ...
  plots, [.05,.1], .80, thick=8
  xyouts, .11, .73, 'MERRAero'

  if(nexp gt 1) then begin
   loadct, 39
;   oplot, bins, hist[*,6], thick=8, color=176, lin=1
   oplot, bins, hist[*,5], thick=8, color=176, lin=2
   ...
   xyouts, -.35, .73, '0.25!Eo!N x 0.3125!Eo!N'
   xyouts, -.35, .55, '0.5!Eo!N x 0.625!Eo!N'
   xyouts, -.35, .40, '1!Eo!N x 1.25!Eo!N'
   xyouts, -.35, .30, '2!Eo!N x 2.5!Eo!N'
  endif
...
; Make a map showing the distributions
  set_plot, 'ps'
  device, file='plot_aeronet_histogram.map.eR_F25b18.ps', /color, font_size=12, $
   xoff=.5, yoff=.5, xsize=14, ysize=12
  !p.font=0
  loadct, 39
...
;  Find the bias and color code
   ...
   statistics, aeraot[nstart:nstart+numloc[a[i]]-1], modaot[nstart:nstart+numloc[a[i]]-1,1], $
