...
; Dimension of calculation
  ...
  levels = [1,1.1,1.2,1.3,1.4,1.5,1.7,2,2.2,2.5,3]
  colors = indgen(11)*25
  x = findgen(8)
  y = su
  col_ = intarr(4)
;  for i = 0, 3 do begin
;   a = where(levels lt frh_[0,i])
;   col_[i] = colors(max(a))
;  endfor

  loadct, 38
  contour, frh[*,*,0], x, y, levels=levels, c_color=colors, /fill, $
   xrange=[0,7], xtitle='age [days]', xstyle=1, $
   yrange=[0,0.5], ytitle = 'SU fraction', ystyle=1, $
   position=[.1,.65,.45,.95], title='RH = 20%'

  contour, frh[*,*,1], x, y, levels=levels, c_color=colors, /fill, $
   xrange=[0,7], xtitle='age [days]', xstyle=1, $
   yrange=[0,0.5], ytitle = 'SU fraction', ystyle=1, $
   position=[.6,.65,.95,.95], title='RH = 40%', /noerase

  contour, frh[*,*,2], x, y, levels=levels, c_color=colors, /fill, $
   xrange=[0,7], xtitle='age [days]', xstyle=1, $
   yrange=[0,0.5], ytitle = 'SU fraction', ystyle=1, $
   position=[.1,.25,.45,.55], title='RH = 80%', /noerase

  contour, frh[*,*,3], x, y, levels=levels, c_color=colors, /fill, $
   xrange=[0,7], xtitle='age [days]', xstyle=1, $
   yrange=[0,0.5], ytitle = 'SU fraction', ystyle=1, $
   position=[.6,.25,.95,.55], title='RH = 95%', /noerase

  makekey, .15, .1, .7, .05, 0., -0.035, $
   label = string(levels,format='(f4.1)'), colors=colors, align=0
  xyouts, .5, .16, 'f(RH) [550 nm]',$
   /normal, align=.5
