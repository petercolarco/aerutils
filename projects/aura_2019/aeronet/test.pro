...
;  varWant = 'AOD_'+lamWant+'nm'
  ymax = 2.5
  expid = 'c180R_J10p17p1dev_aura_2'
...
;  event = [180,260]
  event = [496,735] ; september

  xtickv = [0,112,248,360,496,606,736]
  xtickn = ['Jul 1','Jul15','Aug 1','Aug 15','Sep 1','Sep 15',' ']
  ...
  for i = 0, n_elements(lamWant)-1 do begin
   ...
   if(i eq 0) then begin
    plot, x, aodm, thick=6, color=colors[i], /nodata, $
      ...
      xticks=6, xtickv=xtickv, xtickn=xtickn, $
      position=[0.1,0.55,0.95,0.9], ytitle='AOD', title='c180R_J10p17p1dev_aura'
  ...
  for i = 0, n_elements(lamWant)-1 do begin
   xyouts, .18+i*.04,.875,string(lamwant[i],format='(i4)'), charsize=.7, /normal, color=colors[i]
  endfor
  xyouts, .16, .85, 'bias', /normal, align=1, charsize=.7  
  xyouts, .16, .83, 'rms', /normal, align=1, charsize=.7  
  xyouts, .16, .81, 'r', /normal, align=1, charsize=.7  
  xyouts, .16, .79, 'skill', /normal, align=1, charsize=.7  
  plots, [.17,.17,.47], [.78,.8725,.8725], /normal
...
;check, aod
   ...
   xyouts, .18+i*.04,.85,string(bias,format='(f5.2)'), charsize=.7, /normal
   xyouts, .18+i*.04,.83,string(rms,format='(f5.2)'), charsize=.7, /normal
   xyouts, .18+i*.04,.81,string(r,format='(f5.2)'), charsize=.7, /normal
   xyouts, .18+i*.04,.79,string(skill,format='(f5.2)'), charsize=.7, /normal
...
;   plots, x, aod, psym=0, symsize=.7, color=colors[i], lin=2
  ...
  plot, x, aod, /noerase, /nodata, position=[.15,.325,.5,.425], $
   ...
   xticks=2, xtickv=[496,606,736], xtickn=['Sep 1','Sep 15',' '], charsize=.7, $
  ...
  loadct, 0
  plot, fix(lamwant),aodmevent,/noerase, /nodata, position=[.55,.73,.75,.88], $
  ...
  plots, 500, 1.25, psym=sym(3), symsize=.75
  plots, 500, 1.1, psym=sym(1), symsize=.75
  plots, 500, 0.95, psym=sym(1), symsize=.5, color=120
  xyouts, 525,1.2, 'AERONET', charsize=.7
  xyouts, 525,1.05, 'Model', charsize=.7
  xyouts, 525,0.90, 'Model (BrC)', charsize=.7
  xyouts, 800, 1.35, 'AE!D440-870!N', charsize=.7
  xyouts, 900, 1.2, string(-alog(aodoevent[2]/aodoevent[5])/alog(440/870.),format='(f4.2)'), charsize=.7
  xyouts, 900, 1.05, string(-alog(aodmevent[2]/aodmevent[5])/alog(440/870.),format='(f4.2)'), charsize=.7
  xyouts, 900, 0.90, string(-alog(aodmbevent[2]/aodmbevent[5])/alog(440/870.),format='(f4.2)'), charsize=.7

; Plot the fractional AOD
  loadct, 39
  plot, indgen(2), /nodata, /noerase, position=[.75,.73,.82,.88], $
