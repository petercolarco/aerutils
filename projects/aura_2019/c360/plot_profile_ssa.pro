...
; Variables
  ...
  expid = ['c360R_v10p22p2_aura', 'c360R_v10p22p2_aura_loss_bb4', $
           'c360R_era5_v10p22p2_aura_loss_bb4', 'c360R_era5_v10p22p2_aura_loss_bb3']
  psd   = ['', '.oracles']
  ...
  for iexp = 0, 3 do begin
   ...
   for ipsd = 0, 1 do begin
    for icas = 0, 3 do begin
     ...
     read_ext_profile, fileinp, date, ext_, ssa_, rc
     if(rc) then begin
...
; ----------------------------
  ...
  for icas = 3,3 do begin
   for ipsd = 1,1 do begin
    for iexp = 0,3 do begin
     icas_ = icas
     if(iexp eq 0) then icas_ = 0
     oplot, ssa[*,icas_,ipsd,iexp], h[*,iexp]/1000., $
  ...
  for icas = 3,3 do begin
   for ipsd = 1,1 do begin
    for iexp = 0,3 do begin
     icas_ = icas
     if(iexp eq 0) then icas_ = 0
     oplot, ext[*,icas_,ipsd,iexp]*1000., h[*,iexp]/1000., $
       thick=thick[icas], color=color[ipsd,iexp], lin=lin[icas]
    endfor
   endfor
  endfor
