...
; Modified to screen by surface pressure as written into MERRAero files
  ...
  filed = './'
  file  = file_search(filed,'*2007m06*.vl_rad.he5')

; Disregard files in the "hold" sub-directory
  file = file[where(strpos(file,'hold') eq -1) and $
              where(strpos(file,'no_ash') eq -1) and $
              where(strpos(file,'c360') eq -1) ]
...
;  Get the information write a grads template
   ...
   fileout = strmid(file[ifile],0,x+4)+'.vl_rad.he5'
