...
; Get the ozone VMR
  ...
  nc4readvar, filename0, 'dtdtrad', t_0, lon=lon, lat=lat, lev=lev
  filename1 = filedir+expid1+'/geosgcm_tend/'+expid1+'.geosgcm_tend.monthly.'+datewant+'.nc4'
  nc4readvar, filename1, 'dtdtrad', t_1, lon=lon, lat=lat, lev=lev
...
; Make a plot of the difference
  set_plot, 'ps'
  device, file='plot_dtdtrad_curtain.'+datewant+'.ps', $
  ...
  loadct, 72
  levels = [-100,-1.5,-1,-.5,-.1,.1,.5,1,1.5,2]
  colors = [255-findgen(5)*30,108-findgen(5)*25]
  ...
  plot, indgen(100), /nodata, /noerase, $
        ...
        xrange=[-90,-30], yrange=[10,40], xstyle=9, ystyle=9, $
        xticks=6, xtickv=findgen(7)*10-90
  xyouts, .15, .12, "!9D!3T' (Radiation) [K day!E-1!N]", /normal
