...
; Plot the zonal mixing ratio of sulfate over time
  ...
  nt = n_elements(nymd)
  nc4readvar, filename, 'suextcoefstrat', su, lon=lon, lat=lat
...
; Loop and plot
  ...
  set_plot, 'ps'
  device, file='plot_zonal_vertical_extinction_gocart.strat.clim.'+expid+'.'+strmid(nymd[i],0,6)+'.ps', /color, $
  ...
  xyouts, .15, .12, 'sulfate extinction [Mm-1]', /normal
