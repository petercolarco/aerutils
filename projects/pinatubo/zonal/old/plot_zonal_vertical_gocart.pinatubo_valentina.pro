...
; Plot the zonal mixing ratio of sulfate over time
  filename = '/misc/prc14/colarco/'+expid+'/tavg3d_aer_p/'+expid+'.tavg3d_aer_p.monthly.199112.nc4'
  nc4readvar, filename, 'so4', sut, lon=lon, lat=lat, lev=lev, nymd=nymd
  expid = 'c48F_G41-spin'
; Plot the zonal mixing ratio of sulfate over time
  filename = '/misc/prc14/colarco/'+expid+'/tavg3d_aer_p/'+expid+'.tavg3d_aer_p.monthly.199112.nc4'
  nc4readvar, filename, 'so4', su, lon=lon, lat=lat, lev=lev, nymd=nymd
...
; Form into a zonal mean
  ...
  sut = reform(total(sut,1)/n_elements(lon))*1e9  ; ug m-3
  su = su-sut


; Loop and plot
  set_plot, 'ps'
  device, file='plot_zonal_vertical_gocart.pinatubo_valentina.'+expid+'.'+strmid(nymd,0,6)+'.ps', /color, $
