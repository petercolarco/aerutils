; Make plots of zonal mean effective radius from CARMA for zonal mean
; extinction coefficient greater than some threshold value [Mm-1]
  wantnymd = ['199106','199107','199108','199109','199110','199111']
  for id = 0, 5 do begin

  dtitle= wantnymd[id]
  expid = 'c48Fc_H43_stratv2'
  filedir  = '/misc/prc18/colarco/'+expid+'/tavg3d_carma_v/'
  filename = expid+'.tavg3d_carma_v.monthly.'+dtitle+'.nc4'
;  expid = 'c48F_G41-pintc'
;  filedir  = '/home/colarco/'
;  filename = expid+'.tavg3d_carma_v.monthly.'+dtitle+'.nc4'
...
; Get the file
  ...
  nc4readvar, filedir+filename, 'suextcoef', suext
...
;    if(suext[iy,iz] gt 1) then begin
; for background
    if(suext[iy,iz] gt 0) then begin
     for ix = 0, nx-1 do begin
;     Compute growth factor for grid point RH
      gf  = sulfate_growthfactor(rh[ix,iy,iz])
      dr_ = dr*gf
      r_  = r*gf
      dndr_ = su[ix,iy,iz,*]/dr_ / (4./3.*!pi*r^3.*rhop)*rhoa[ix,iy,iz]
