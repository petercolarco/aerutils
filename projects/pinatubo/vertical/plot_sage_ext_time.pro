...
; Pick a date to look at
  a = where(nymd ge 19910101 and nymd lt 20010101)
  data  = reform(ae525[a,*,*])
  ...
  set_plot, 'ps'
  device, file='sage_ext_525_time.ps', $
  ...
  plot, indgen(100), /nodata, $
        position=[.15,.25,.95,.9], $
        ytitle='Altitude [km]', $
        xrange=1991+[0,nt/12], yrange=[10,40], xstyle=9, ystyle=9, $
        xticks=nt/12, xminor=2
  loadct, 56
  levels = [.05,.1,.15,.2,.25,.3,.4,.5,.7]*1e-6
  levels = [.1,.5,1,2,3,4,5,10,20]
  plotgrid, data*1e3, levels, colors, 1991+x, height, 1./12., 0.5
  loadct, 0
