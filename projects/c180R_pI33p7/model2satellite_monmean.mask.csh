...
# Satellite data
  ...
  set datadir   = /misc/prc18/colarco/mask/
  
#  set datatype = ( mask.calipso mask.calipso.offset+1hr mask.calipso.offset-1hr mask.calipso.sza+1hr mask.calipso.sza-1hr mask.calipso.sza  mask.calipso.sza+4hr mask.calipso.sza-4hr mask.calipso.offset+4hr mask.calipso.offset-4hr)
  set datatype = ( mask.calipso.offset+10deg mask.calipso.offset-10deg)

   foreach datatyp ( `echo $datatype` )
    foreach sample ( caliop )

cat > qafl.${rand}.ddf << EOF
dset $datadir/Y$YYYY/M$MM/$datatyp.%y4%m2%d2_%h200z.nc4
options template
$tdefstrsat
EOF
   ...
   # Pass subset to extraction
     lats4d.sh -v -i tmp_aer.${rand}.nc -j qafl.${rand}.ddf -o tmp_qawt.${rand} \
                     -func "ave(maskout(@.1,mask.2),t=1,t=$ntimesat)" \
...
#     lats4d.sh -i tmp_qawt.${rand}.nc -o tmp_qawt.10.${rand} -shave -geos10x10a
   ...
   # clean-up
     ...
     mv -f tmp_qawt.${rand}.nc4 \
           $expdir/Y$YYYY/M$MM/$expstr.$datatyp.$YYYY$MM.nc4
