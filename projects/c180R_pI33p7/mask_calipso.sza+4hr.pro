...
; Take CALIPSO date and convert to Julian day
  ...
  frac = date-nymd
  nhms = nhms(frac)
  jday0 = julday_nymd(nymd,nhms)

; Adjust jday to 2016
  jdayf = julday_nymd(20151231L,0L)-4.d/24.
  djday = jdayf-jday0[0]
  jday  = jday0+djday

; Go back to day+fraction
  caldat, jday, mm, dd, yyyy, hh, nn, ss
  nymd_ = double(yyyy*10000L+mm*100L+dd)
  date_ = nymd_+hh/24.d + nn/1440.d + ss/86400.d
  date  = date_
...
; See if this helps with sza calculation
     date__[k*npts:k*npts+npts] = date[a[k]]
    endfor
    lon_ = lon__
    lat_ = lat__
    date_ = date__
...
;   Find points where SZA < 90
    ...
    write_mask, '/misc/prc18/colarco/mask/', 'mask.calipso.sza+4hr', nx, ny, dx, dy, datef, lon, lat, mask, res='d'
