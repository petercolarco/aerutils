...
;    nhms = ['133000']
    ...
    ihmax[iexpid] = imax[iexpid]+min(where(so2v[imax[iexpid]:nt-1,iexpid] lt so2v[imax[iexpid],iexpid]/2.))
    if(ihmax[iexpid] lt imax[iexpid]) then ihmax[iexpid] = nt-1
...
; Make a plot of the distribution of SO2 at the half point
  set_plot, 'ps'
  device, file='plot_so2volc.'+mon+'01.half.ps', $
...
; Make a plot of the distribution of SO4 at the half point
  set_plot, 'ps'
  device, file='plot_so4volc.'+mon+'01.half.ps', $
   /color, /helvetica, font_size=10, $
   xsize=40, ysize=18, xoff=.5, yoff=.5
  !p.font=0
; Set up a six-element array of the positions for individual panels
...
; Make a plot of the distribution of SO4 at the half point
  set_plot, 'ps'
  device, file='plot_suexttau.'+mon+'01.half.ps', $
   /color, /helvetica, font_size=10, $
   xsize=40, ysize=18, xoff=.5, yoff=.5
  !p.font=0
; Set up a six-element array of the positions for individual panels
  ...
  loadct, 0
  levels=[1,2,5,10,20,50,100,200]/1000.
