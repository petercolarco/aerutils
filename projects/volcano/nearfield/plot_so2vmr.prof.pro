...
; Experiments
  ...
  expid = expids[iexpid]
  case expid of
   'VMNHL': locstr = 'Northern Hemisphere High Latitude, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 60!Eo!NN'
   'VMNML': locstr = 'Northern Hemisphere Mid Latitude, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 45!Eo!NN'
   'VMTRO': locstr = 'Northern Hemisphere Tropics, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 15!Eo!NN'
   'VMSTR': locstr = 'Southern Hemisphere Tropics, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 15!Eo!NS'
   'VMSML': locstr = 'Southern Hemisphere Mid Latitude, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 60!Eo!NS'
   'VMSHL': locstr = 'Southern Hemisphere High Latitude, Zonal Mean SO2 mixing ratio [10!Ex!N ppbv] @ 75!Eo!NS'
  endcase

; Set up plot
  set_plot, 'ps'
  device, file='plot_so2vmr.prof.'+expid+'01.ps', $
...
; Get ensemble of simulations
  ...
  for icc = 0, 4 do begin
    filetemplate = expid+mon+cc[icc]+'.tavg3d_aer_p.ddf'
    ga_times, filetemplate, nymd, nhms, template=template
;   Pick off times in the window 
    nymd = nymd[0:44]
    nhms = nhms[0:44]
    filename=strtemplate(template,nymd,nhms)
    nc4readvar, filename, 'so2v', su_, lon=lon, lat=lat, lev=lev, wantlat=latwant+[-5,5], rc=rc
print, filetemplate
if(rc ne 0) then goto, jump
;   Throw out undef
    a = where(su_ gt 1e14)
    su_[a] = !values.f_nan
;   Convert so2 mmr to vmr in ppbv
    su_ = su_*28./64.*1e9
;   Zonally average
