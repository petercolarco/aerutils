...
; Get the g5 aot and save area average
  ...
  nc4readvar, g5expid[0], ['susd','sudp'], su, time=time, /sum
  ...
  for i = 1, 4 do begin
   nc4readvar, g5expid[i], ['susd','sudp'], su, time=time, /sum
   g5[*,*,i] = total(su,1)/nx
  endfor

; Get the GEOS-5 climatology and remove
  g5clim = 'VMcontrol'+['01','02','03','04','05']+'.tavg2d_carma_x.ddf'
  nc4readvar, g5clim[0], ['susd','sudp'], su, time=time, /sum
  g5c = fltarr(ny,nt,5)
  g5c[*,*,0] = total(su,1)/nx
  for j = 1, 4 do begin
   nc4readvar, g5clim[j], ['susd','sudp'], su, time=time, /sum
   g5c[*,*,j] = total(su,1)/nx
  endfor

;  g5 = transpose(total(g5-g5c,3)/5.) ; time x lat
  g5 = transpose(total(g5,3)/5.) ; time x lat
...
;  if(a[0] ne -1) then g5[a] = 0.
  ...
  set_plot, 'ps'
  device, file='VM'+g5v+g5s+'.horz_drydep.ps', /color, /helvetica, font_size=14, $
   xsize=18, ysize=8, xoff=.5, yoff=.5
  !p.font=0
  loadct, 0
  ...
  device, file='drydep_key.ps', /color, /helvetica, font_size=14, $
   xsize=18, ysize=2, xoff=.5, yoff=.5
  !p.font=0
  loadct, 70
  makekey, .05,.28,.9,.4,0,-.2,colors=colors, $
   label=['-4','-2','0','2','4'], /noborder
  loadct, 0
  xyouts, .05, .75, 'Dry Deposition [10!E-13!N kg m!E-2!N s!E-1!N]', /normal
  device, /close
