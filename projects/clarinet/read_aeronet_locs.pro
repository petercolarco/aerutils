...
;   Check negative lon/lat
    flon = 1.
    flat = 1.
    if(float(strparse[1]) lt 0) then flat = -1.
    if(float(strparse[4]) lt 0) then flon = -1.
    lat_ = float(strparse[1]) + flat*float(strparse[2])/60. + flat*float(strparse[3])/3600.
    lon_ = float(strparse[4]) + flon*float(strparse[5])/60. + flon*float(strparse[6])/3600.
