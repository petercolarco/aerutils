...
; Find median altitude
  ...
  for i = 0, icnt-1 do begin
   tot = 0
   dutot = total(du[*,i]*delp)
   for iz = nz-1,0,-1 do begin
    tot = tot + du[iz,i]*delp[iz]
    if(tot ge 0.5*dutot) then begin
     vert[il,iz] = vert[il,iz]+1
     break
    endif
   endfor
  endfor
...
; Make an altitude in km
  ...
  set_plot, 'ps'
  device, file='duconc_med.ps', /color, /helvetica, font_size=20, $
  ...
  plot, vert[2,*], alt, /nodata, xrange=[0,.3], xstyle=9, $
   ...
   title='West', xticks=3
  oplot, vert[0,*]/total(vert[2,*]), alt, thick=8
  oplot, vert2[0,*]/total(vert2[2,*]), alt, thick=8, lin=2

  plot, vert[2,*], alt, /nodata, xrange=[0,.3], xstyle=9, $
   yrange=[0,4], ystyle=9, ytitle='Altitude [km]', $
   xtitle='frequency', xminor=2, yticks=4, yminor=2, $
   title='East', xticks=3
  oplot, vert[1,*]/total(vert[2,*]), alt, thick=8
  oplot, vert2[1,*]/total(vert2[2,*]), alt, thick=8, lin=2
