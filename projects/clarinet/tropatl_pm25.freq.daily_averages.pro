...
; Now grab noon of each
  ...
  for i = 0, nf-1 do begin
   print, i
   nc4readvar, filename[i], 'dusmass25', duext_, time=time, lon=lon, lat=lat
   nc4readvar, filename[i], ['sssmass25','ocsmass','bcsmass'], pm25, time=time, lon=lon, lat=lat, /sum
   nc4readvar, filename[i], ['so4smass'], so4smass, time=time, lon=lon, lat=lat
   duext_ = mean(duext_,dim=3)
   pm25   = mean(pm25,dim=3)
   so4smass = mean(so4smass,dim=3)
   duext_ = duext_ + pm25 + 132./96.*so4smass  ; add ammonium sulfate
;   duext_ = pm25 + 132./96.*so4smass  ; add ammonium sulfate
...
;  map_grid, /box, glinethick=6, color=iblack
  ...
  makekey, .05, .1, .9, .05, 0, -.025, $
   color=colors, align=0, label=string(levels*101,format='(i2)')+'%'

  xyouts, .5, .98, align=.5, /normal, $
   'June 2010 - 2015: Percent of time PM2.5 exceeds 24-hr standard'
...
; Overwrite masks in order
  ...
  bimg[1,*,*] = bimgg
  bimg[2,*,*] = bimgb
  write_png, 'tropatl_pm25.freq.daily_averages.png', bimg
