...
; Now grab noon of each
  ...
  for i = 0, nf-1 do begin
   ...
   nc4readvar, filename[i], ['sssmass25','ocsmass','bcsmass'], pm25_, time=time, lon=lon, lat=lat, wantnymd=nymd[i], /sum
   nc4readvar, filename[i], ['so4smass'], so4smass, time=time, lon=lon, lat=lat, wantnymd=nymd[i]
   pm25_ = duext_ + pm25_ + 132./96.*so4smass  ; add ammonium sulfate
  ...
  pm25 = reform(pm25,n_elements(lon),nf,n_elements(lat))
  pm25 = transpose(pm25,[0,2,1])
...
; Form the mean
  ...
  pm25_mean = mean(pm25,dimension=3)
...
; Find the grid boxes where all mean AOT > 0.05
  ...
  for ix = 0, nx-1 do begin
   for iy = 0, ny-1 do begin
    a = where(pm25[ix,iy,*] gt thresh and pm25[ix,iy,*]-duext[ix,iy,*] lt thresh)
    b = where(pm25[ix,iy,*] gt thresh)
    if(a[0] ne -1 and b[0] ne -1 and 1.*n_elements(b)/nf gt .02) then dufreq[ix,iy] = 1.*n_elements(a)/n_elements(b)
   endfor
  endfor
...
;  map_grid, /box, glinethick=6, color=iblack
  ...
  xyouts, .5, .98, align=.5, /normal, $
   'June 2010 - 2015: Percent of time PM2.5 exceeds 24-hr standard is caused by dust'
...
; Overwrite masks in order
  ...
  bimg[1,*,*] = bimgg
  bimg[2,*,*] = bimgb
  write_png, 'tropatl_pm25-dust.freq.png', bimg
