...
print, i
   ...
   nc4readvar, filename, 'duexttau550', duext_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   nc4readvar, filename2, 'aod', ocean_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   a = where(ocean_ lt 0 or ext_ lt 0)
   ext_[a] = !values.f_nan
  ...
  save, file='plot_ocean.misr.sav', /variables

jump:
  restore, file='plot_ocean.misr.sav'

  for i = 0, 0 do begin

   set_plot, 'ps'
   device, file='plot_ocean.misr.'+expid+'.'+string(i,format='(i1)')+'.ps', $
   ...
   plot, indgen(nt), /nodata, $
    ...
    xrange=[0,nt+1], xticks=xyrs, xminor=2, xstyle=1, $
...
;  Inset an anomaly plot
   ...
   plot, indgen(nt), /nodata, /noerase, $
    ...
    xrange=[0,nt+1], xticks=xyrs, xminor=2, xstyle=8, $
