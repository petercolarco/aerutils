...
; Define the source regions
  lon0 = [-20, 7.5, -20, 35, 70, -130, 110, -80,   0]
  lon1 = [7.5, 35,   35, 75, 120, -80, 160, -20,  40]
  lat0 = [ 18, 18,    0,  0,  35,  20, -40, -60, -40]
...
; Get the dust emissions
  ddf = expid+'.inst2d_hwl_x.deep.ddf'
...
print, i
   ...
   nc4readvar, filename, 'duexttau002', duext_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   nc4readvar, filename2, 'tau_', deep_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   a = where(deep_ gt 100 or ext_ gt 100)
   ext_[a] = !values.f_nan
   duext_[a] = !values.f_nan
   deep_[a] = !values.f_nan
   if(i eq 3) then begin
    ...
    for j = a[0], a[n_elements(a)-1] do begin
     for k = b[0], b[n_elements(b)-1] do begin
      ...
      deep_[j,k,*] = !values.f_nan
     endfor
    endfor
   endif
   ext[*,i] = aave(ext_,area_,/nan)
   duext[*,i] = aave(duext_,area_,/nan)
   deep[*,i] = aave(deep_,area_,/nan)
  endfor

  save, file='plot_deep.sav', /variables

jump:
  restore, file='plot_deep.sav'

  for i = 0, 8 do begin

   set_plot, 'ps'
   device, file='plot_deep.'+expid+'.'+string(i,format='(i1)')+'.ps', $
   ...
   oplot, x+1, deep[*,i], thick=8, color=0

;  Plot the climatology
   ext_ = reform(ext[*,i],12,xyrs)
   duext_ = mean(reform(duext[*,i],12,xyrs),dimension=2)
   deep_ = mean(reform(deep[*,i],12,xyrs), dimension=2)

   ymax = max([max(ext_),max(deep_)])
   ...
   oplot, indgen(12)+1, deep_, thick=8
;  Print the total mean emissions
   ...
   mxmean = string(total(deep[*,i])/xyrs/12.,format='(f4.2)')
   xmean = string(total(ext[*,i])/xyrs/12.,format='(f4.2)')
...
;  Compute the correlation
   statistics, deep[*,i], ext[*,i], xmean, ymean, xstd, ystd, $
               r, bias, rms, skill, linslope, linoffset
   statistics, deep[*,i], duext[*,i], xmean, ymean, xstd, ystd, $
