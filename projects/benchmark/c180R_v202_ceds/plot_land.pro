...
; Get the dust emissions
  ddf = expid+'.inst2d_hwl_x.land.ddf'
...
print, i
   ...
   nc4readvar, filename, 'duexttau002', duext_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   nc4readvar, filename2, 'tau_', land_, wantlon=[x0,x1], wantlat=[y0,y1], $
    lon=lon_, lat=lat_, /template, /sum
   a = where(land_ gt 100 or ext_ gt 100)
   ext_[a] = !values.f_nan
   duext_[a] = !values.f_nan
   land_[a] = !values.f_nan
   if(i eq 3) then begin
    ...
    for j = a[0], a[n_elements(a)-1] do begin
     for k = b[0], b[n_elements(b)-1] do begin
      ...
      land_[j,k,*] = !values.f_nan
     endfor
    endfor
   endif
   ext[*,i] = aave(ext_,area_,/nan)
   duext[*,i] = aave(duext_,area_,/nan)
   land[*,i] = aave(land_,area_,/nan)
  endfor

  save, file='plot_land.sav', /variables

jump:
  restore, file='plot_land.sav'

  for i = 0, 8 do begin

   set_plot, 'ps'
   device, file='plot_land.'+expid+'.'+string(i,format='(i1)')+'.ps', $
   ...
   oplot, x+1, land[*,i], thick=8, color=0

;  Plot the climatology
   ext_ = reform(ext[*,i],12,xyrs)
   duext_ = mean(reform(duext[*,i],12,xyrs),dimension=2)
   land_ = mean(reform(land[*,i],12,xyrs), dimension=2)

   ymax = max([max(ext_),max(land_)])
   ...
   oplot, indgen(12)+1, land_, thick=8
;  Print the total mean emissions
   ...
   mxmean = string(total(land[*,i])/xyrs/12.,format='(f4.2)')
   xmean = string(total(ext[*,i])/xyrs/12.,format='(f4.2)')
...
;  Compute the correlation
   statistics, land[*,i], ext[*,i], xmean, ymean, xstd, ystd, $
               r, bias, rms, skill, linslope, linoffset
   statistics, land[*,i], duext[*,i], xmean, ymean, xstd, ystd, $
