...
; Get the full
  get_sample, 'full', filetag, full, fullhr, fullhrn, fullhrhst, fullhrs, fullhrmn, fullhrmx
                  

; Get "TOMCAT1"
  get_sample, 'tomcat1.nadir', filetag, tom1, tom1hr, tom1hrn, tom1hrhst, tom1hrs, tom1hrmn, tom1hrmx, $
                  /tcld, /taod
; Get "TOMCAT2:
  get_sample, 'tomcatall.nadir', filetag, tom2, tom2hr, tom2hrn, tom2hrhst, tom2hrs, tom2hrmn, tom2hrmx, $
                  /tcld, /taod

; Get "TOMCAT3"
  get_sample, 'tomcat1.450km', filetag, tom3, tom3hr, tom3hrn, tom3hrhst, tom3hrs, tom3hrmn, tom3hrmx, $
                  /tcld, /taod

; Get "TOMCAT4"
  get_sample, 'tomcatall.450km', filetag, tom4, tom4hr, tom4hrn, tom4hrhst, tom4hrs, tom4hrmn, tom4hrmx, $
                  /tcld, /taod

  set_plot, 'ps'
  device, file='tomcatall_cloud_aod01.site.pdf_pm25.range.'+filetag+'.ps', /color, /helvetica, font_size=12, $
  ...
  xyouts, 31, .91, 'n='+string(total(tom1),format='(i-9)'), /data
  xyouts, 31, .87, 'n='+string(total(tom2),format='(i-9)'), /data
  xyouts, 31, .83, 'n='+string(total(tom3),format='(i-9)'), /data
  xyouts, 31, .79, 'n='+string(total(tom4),format='(i-9)'), /data
...
; TOMCAT4
  ...
  for i = 0, 23 do begin
   ...
   polyfill, [x0,x1,x1,x0,x0], [y0,y0,y1,y1,y0], color=84
   plots, [x0,x1,x1,x0,x0], [y0,y0,y1,y1,y0]
   xyouts, 11.9, y0-0.025*ymax, 'Full', orient=-60, charsize=.75
   xyouts, 12.4, y0-0.025*ymax, 'TOMCAT1 (n)', orient=-60, charsize=.75, color=176
   xyouts, 12.9, y0-0.025*ymax, 'TOMCATA (n)', orient=-60, charsize=.75, color=254
   xyouts, 13.4, y0-0.025*ymax, 'TOMCAT1 (w)', orient=-60, charsize=.75, color=208
   xyouts, 13.9, y0-0.025*ymax, 'TOMCATA (w)', orient=-60, charsize=.75, color=84


  device, /close

  openw, lun, 'tomcatall_cloud_aod01.pdf_pm25.range.'+filetag+'.stats.txt', /get
  printf, lun, 'full'
