...
;     var = aod[ix,iy,*]-min(aod[ix,iy,*])
    ...
    noerase = 1
    xyouts, xoff, .55, string(total(num[ix,iy,*])/365.,format='(i8)'), $
...
; Overplot another case
  filen = varn+'_annual.'+inc+'.450km.2014.aod.cloud.nc4'
  aggregate_diurnal, filen, varn, nx, ny, aod, num
  plotit, nx, ny, num, aod, 1, 39, 3, 254, 8

; Overplot another case
  filen = varn+'_annual.'+inc+'.nadir.2014.aod.cloud.nc4'
  aggregate_diurnal, filen, varn, nx, ny, aod, num
