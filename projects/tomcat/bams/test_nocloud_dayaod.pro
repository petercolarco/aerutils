; Beijing
  ...
  ymax = 40
;nts = 240
  wantlon = lon0+[-1.5,1.5] & wantlat = lat0+[-1.5,1.5] & filetag = 'beijing.01_5deg.jun1-aug31'
  wantlon = lon0+[-2.5,2.5] & wantlat = lat0+[-2.5,2.5] & filetag = 'beijing.02_5deg.jun1-aug31' ;.nodaod'
;  nts = 1440
;  wantlon = lon0+[-2.5,2.5] & wantlat = lat0+[-2.5,2.5] & filetag = 'beijing.02_5deg.jun1-jul30'
...
; Get "TOMCAT2:
  get_sample, 'tomcat2.450km', fnoff, nts, tom2, tom2hr, tom2hrn, tom2hrhst, tom2hrs, tom2hrmn, tom2hrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon;, /trnd;, /taod

; Get "TOMCAT3"
  get_sample, 'tomcat3.450km', fnoff, nts, tom3, tom3hr, tom3hrn, tom3hrhst, tom3hrs, tom3hrmn, tom3hrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon

; Get "TOMCAT4"
  get_sample, 'tomcat4.450km', fnoff, nts, tom4, tom4hr, tom4hrn, tom4hrhst, tom4hrs, tom4hrmn, tom4hrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon

; Get "TOMCAT1"
  get_sample, 'tomcat1.450km', fnoff, nts, tom1, tom1hr, tom1hrn, tom1hrhst, tom1hrs, tom1hrmn, tom1hrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon;, /taod

  set_plot, 'ps'
  device, file='test_nocloud_dayaod.site.pdf_pm25.range.'+filetag+'.ps', /color, /helvetica, font_size=12, $
   xoff=.5, yoff=.5, xsize=42, ysize=12
  !p.font=0
  bins = findgen(21)*2
  loadct, 39
  plot, bins, float(full)/max(full), thick=6, $
   xtitle='PM2.5 [!4m!3g m!E-3!N]', ytitle='Relative Frequency', $
   position=[.04,.1,.31,.95]
  ...
  xyouts, 20, .91, 'TOMCAT1', /data, color=176
  xyouts, 20, .87, 'TOMCAT2', /data, color=254
  xyouts, 20, .83, 'TOMCAT3', /data, color=208
  xyouts, 20, .79, 'TOMCAT4', /data, color=84
  xyouts, 31, .95, 'n='+string(total(full),format='(i8)'), /data
  xyouts, 31, .91, 'n='+string(total(tom1),format='(i6)'), /data
  xyouts, 31, .87, 'n='+string(total(tom2),format='(i6)'), /data
  xyouts, 31, .83, 'n='+string(total(tom3),format='(i6)'), /data
  xyouts, 31, .79, 'n='+string(total(tom4),format='(i6)'), /data

  plot, fullhr, yrange=[0,ymax], thick=6, /nodata, /noerase, $
   xrange=[-1,24], xstyle=1, $
   xtitle='Local Hour', ytitle='PM2.5 [!4m!3g m!E-3!N]', $
   position=[.38,.1,.98,.95]
  loadct, 0
  for i = 0, 23 do begin
   x0 = i-.4
...
; TOMCAT4
  ...
  for i = 0, 23 do begin
   ...
   xyouts, 2.4, y0-0.025*ymax, 'TOMCAT1', orient=-60, charsize=.75, color=176
   xyouts, 2.9, y0-0.025*ymax, 'TOMCAT2', orient=-60, charsize=.75, color=254
   xyouts, 3.4, y0-0.025*ymax, 'TOMCAT3', orient=-60, charsize=.75, color=208
   xyouts, 3.9, y0-0.025*ymax, 'TOMCAT4', orient=-60, charsize=.75, color=84
  ...
  openw, lun, 'test_nocloud_dayaod.site.pdf_pm25.range.'+filetag+'.stats.txt', /get
  printf, lun, 'full'
