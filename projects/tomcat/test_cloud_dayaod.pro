...
;  wantlon = lon0+[-2.5,2.5] & wantlat = lat0+[-2.5,2.5] & filetag = 'beijing.02_5deg.jun1-aug31'
  nts = 1440
  wantlon = lon0+[-2.5,2.5] & wantlat = lat0+[-2.5,2.5] & filetag = 'beijing.02_5deg.jun1-jul30'
  wantlon = lon0+[-3.,3.] & wantlat = lat0+[-3.,3.] & filetag = 'beijing.03_0deg.jun1-jul30'
...
; Get "CATS" - equivalent to ISS1 -- but with 66% random sampling
  get_sample, 'iss1', fnoff, nts, cats, catshr, catshrn, catshrhst, catshrs, catshrmn, catshrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon, /trnd, /tcld, /daod
...
; Get "TOMCAT1" - equivalent to ISS1 (450km) -- but only AOD > 0.2 
  get_sample, 'iss1day.450km', fnoff, nts, tom1, tom1hr, tom1hrn, tom1hrhst, tom1hrs, tom1hrmn, tom1hrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon, /tcld, /daod, unionorbit='iss1'

; Get "CATS" - equivalent to ISS1 -- but with 100% duty cycle and no
;              AOT constraint
  get_sample, 'iss1', fnoff, nts, pcats, pcatshr, pcatshrn, pcatshrhst, pcatshrs, pcatshrmn, pcatshrmx, $
                  wantlat=wantlat, wantlon=wantlon, lat=lat, lon=lon, /tcld, /daod
...
; Get CALIPSO
  ...
  set_plot, 'ps'
  device, file='test_cloud_dayaod.site.pdf_pblh.range.'+filetag+'.ps', /color, /helvetica, font_size=12, $
  ...
  oplot, bins, float(pcats)/max(pcats), thick=6, color=84
  ...
  xyouts, 500, .79, 'CATS (100%)', /data, color=84
  ...
  xyouts, 1600, .79, 'n='+string(total(pcats),format='(i6)'), /data
...
; TOMCAT (2)
  ...
  for i = 0, 23 do begin
   ...
   if(ym gt 0) then begin
    ...
    plots, [x0,x1,x1,x0,x0], [y0,y0,y1,y1,y0]
    plots, [x0,x1],[ym,ym], thick=4
    plots, i+.32, [pcatshrhst[i,0],y0]
    plots, i+.32, [pcatshrhst[i,4],y1]
    plots, [i+0.27,i+.37], pcatshrhst[i,0]
    plots, [i+0.27,i+.37], pcatshrhst[i,4]
   endif
  endfor
   ...
   xyouts, 3.9, y0-0.025*ymax, 'CATS (100%)', orient=-60, charsize=.75, color=84
  ...
  openw, lun, 'test_cloud_dayaod.site.pdf_pblh.range.'+filetag+'.stats.txt', /get
  printf, lun, 'full'
  ...
  printf, lun, 'hr', 'n', '10%','25%','median','75%','90%','mean','stddev','min','max', $
   format='(a2,2x,a6,9(2x,a8))'
  for i = 0, 23 do begin
   printf, lun, i, pcatshrn[i], pcatshrhst[i,*], pcatshrmn[i], pcatshrmx[i],$
    format='(i2,2x,i6,9(2x,f8.3))'
  endfor
  printf, lun, ''
