...
; Get CALIPSO
  ...
  a = where(aot gt 1e14)
  aot[a] = !values.f_nan
  cali = histogram(alog10(aot),binsize=.2,min=-3,max=1,/nan)
...
; Get DUALISS
  ...
  set_plot, 'ps'
  device, file='site.pdf_totexttau.range.'+filetag+'.ps', /color, /helvetica, font_size=12, $
   xoff=.5, yoff=.5, xsize=42, ysize=12
  !p.font=0
  bins = 10^(-3.+findgen(21)/5.)
  loadct, 39
  plot, bins, float(full)/max(full), thick=6, $
   xtitle='AOD', ytitle='Relative Frequency', $
   position=[.04,.1,.31,.95], /xlog
  ...
  xyouts, .002, .91, 'CALIPSO', /data, color=176
  xyouts, .002, .87, 'CATS', /data, color=254
  xyouts, .002, .83, 'TOMCAT', /data, color=84
