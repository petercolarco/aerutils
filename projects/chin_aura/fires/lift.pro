...
; Lift
  ...
  for iz = 71, 1, -1 do begin
   for iy = 0, ny-1 do begin
    zlow = he[iy,iz+1]+liftz
    zup  = he[iy,iz]+liftz
    a = where(zlow gt he[iy,*]) ; min index = lower
    b = where(zup gt he[iy,*])  ; min index = upper
    for izz = min(a), min(b), -1 do begin
     if(he[iy,izz] lt zlow) then $
      f = (he[iy,izz-1]-zlow)/(zup-zlow)
     if(he[iy,izz-1] gt zup) then $
      f = (zup-he[iy,izz])/(zup-zlow)
     f = min([f,1.])
     ocout[iy,izz] = ocout[iy,izz]+oc[iy,iz]*delp[iy,iz]*f/delp[iz,izz]
;     print, izz, zlow, zup, he[iy,izz], he[iy,izz-1], f
    endfor
   endfor
  endfor
...
; Make a plot
  set_plot, 'ps'
  device, file='lift.dR_F25b18.'+nymd+'.ps', /color, /helvetica, $
  ...
  plotgrid, ocout*rhoa*1e9, levels, colors, x, y, dx, dy
...
; Check column mass
  set_plot, 'x'
  plot, total(oc*delp,2), thick=3, lin=2
  oplot, total(ocout*delp,2)
