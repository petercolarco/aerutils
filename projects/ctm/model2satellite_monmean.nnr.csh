...
# Model parameters (to be modified)
  set expid      = c180ctm_I32p9_M2mrdamon
  set resolution = d
  set coll       = inst2d_hwl_x
  set expdir     = /misc/prc18/colarco/${expid}/
  set expstr     = ${expid}.${coll}
  set ntimes     = 8     # number of times per day on input files

# What fields/levels to extract?
  set varstr = "-vars totexttau duexttau ssexttau suexttau ocexttau bcexttau"
#  set varstr = ""
  set levstr = ""
  set regridstr = ""
...
# Code needs some work on selecting on dimensioning
  ...
  set samples = `echo aero_tc8_006.qast`
...
# Satellite data
  ...
  set datadir   = /science/modis/data/Level3/$satid/NNR/$resolution/
  
  set datatype = ( ${satid}_L3a.ocean ${satid}_L3a.land ${satid}_L3a.deep)

   foreach datatyp ( `echo $datatype` )


cat > qafl.${rand}.ddf << EOF
dset $datadir/Y$YYYY/M$MM/nnr_003.$datatyp.%y4%m2%d2_%h200z.nc4
options template
$tdefstrsat
EOF
echo $datadir/Y$YYYY/M$MM/nnr_003.$datatyp.%y4%m2%d2_%h200z.nc4
   ...
   # clean-up
     ...
     mv -f tmp_qawt.${rand}.nc4 \
           $expdir/$expstr.nnr_003.$datatyp.$YYYY$MM.nc4
