; Plot the MEE ~550 nm for each case using GEOS size bins
  ...
  set_plot, 'ps'
  device, file='plot_ssa.size.grasp.ps', /color, /helvetica, font_size=18, $
...
; OPAC
  ...
  filename = 'integ-du_opac_grasp-raw.nc'
...
; GEOS
  filename = 'integ-du_v15_6_grasp-raw.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
  ilam = 5
  ssa = reform(bsca[ilam,0,*])/reform(bext[ilam,0,*])
...
; Woodward
  filename = 'integ-du_woodward_grasp-raw.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
...
; Balkanski
  filename = 'integ-du_balkanski_grasp-raw.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
  ilam = 28
  ssa = reform(bsca[ilam,0,*])/reform(bext[ilam,0,*])
...
; Balkanski
  filename = 'integ-du_balkanski27_grasp-raw.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
  ilam = 28
  ssa = reform(bsca[ilam,0,*])/reform(bext[ilam,0,*])
