...
; truncate all here in 0.2 - 10 um diameter range
  ...
  models = ['GEOS/GEFS','MASINGAR','AM4','MONARCH','ModelE','IFS','SILAM','UM','NAAPS']
...
; MASINGAR
  ...
  compute_model, 'MASINGAR', diam, dm, dv, dv_, xmin, xmax, mee, ssa, $
   tau_, ssa_, mfr_, afr_
  tau_table[imod,*] = tau_
  ssa_table[imod,*] = ssa_
  mfr_table[imod,*] = mfr_
...
; sort by AOD and put NAAPS last
  ...
  plot, tau_table[a,0], /nodata, $
   xrange=[0,8], yrange=[0.5,1.75], $
   xstyle=1, ystyle=1, xticks=8, yticks=5, $
   ytitle='"Normalized" AOD', $
   position=[.05,.25,.3,.95], xtickname=make_array(9,val=' ')
  xyouts, indgen(9)-0.1, .45, models[a], $
   orientation=-90, align=0
  oplot, tau_table[a,0], thick=12, color=84
  plots, indgen(9), tau_table[a,0], psym=sym(1), color=84, symsize=1.5
  oplot, tau_table[a,1], thick=12, color=148
  plots, indgen(9), tau_table[a,1], psym=sym(1), color=148, symsize=1.5
  xyouts, 5, 1.65, 'Kok', color=84, /data
  xyouts, 5, 1.58, 'OPAC', color=148, /data
  loadct, 0
  oplot, tau_table[a,2], thick=12, color=160
  plots, indgen(9), tau_table[a,2], psym=sym(1), color=160, symsize=1.5
  xyouts, 5, 1.51, 'MODIS', color=160, /data

; Co-albedo
  loadct, 39
  plot, tau_table[a,0], /nodata, /noerase, $
   xrange=[0,8], yrange=[0,0.16], $
   xstyle=1, ystyle=1, xticks=8, yticks=4, $
   ytitle='Co-Albedo', $
   position=[.375,.25,.625,.95], xtickname=make_array(9,val=' ')
  xyouts, indgen(9)-0.1, -0.0064, models[a], $
   orientation=-90, align=0
  oplot, 1.-ssa_table[a,0], thick=12, color=84
  plots, indgen(9), 1.-ssa_table[a,0], psym=sym(1), color=84, symsize=1.5
  oplot, 1.-ssa_table[a,1], thick=12, color=148
  plots, indgen(9), 1.-ssa_table[a,1], psym=sym(1), color=148, symsize=1.5
  loadct, 0
  oplot, 1.-ssa_table[a,2], thick=12, color=160
  plots, indgen(9), 1.-ssa_table[a,2], psym=sym(1), color=160, symsize=1.5
...
; Calculate forcing after Chylek and Wong (1995)
  ...
  loadct, 39
  plot, tau_table[a,0], /nodata, /noerase, $
   xrange=[0,8], yrange=[-.9,-.3], $
   xstyle=1, ystyle=1, xticks=8, yticks=4, $
   ytitle='"Normalized" Forcing', $
   position=[.7,.25,.95,.95], xtickname=make_array(9,val=' ')
  xyouts, indgen(9)-0.1, -0.924, models[a], $
   orientation=-90, align=0
  oplot, force[a,0], thick=12, color=84
  plots, indgen(9), force[a,0], psym=sym(1), color=84, symsize=1.5
  oplot, force[a,1], thick=12, color=148
  plots, indgen(9), force[a,1], psym=sym(1), color=148, symsize=1.5
  loadct, 0
  oplot, force[a,2], thick=12, color=160
  plots, indgen(9), force[a,2], psym=sym(1), color=160, symsize=1.5
  device, /close
