; get the du column mass
  wantlat = [9,35]
  wantlon = [-18,35]
  nc4readvar, 'ducmass.nc', 'ducmass', ducmass, lon=lon, lat=lat, wantlat=wantlat, wantlon=wantlon

; Get the emissions
  nc4readvar, 'duem.nc', 'duem', duem, wantlat=wantlat, wantlon=wantlon, /sum, /tem
...
; Generate the total dust load
  nt = n_elements(ducmass[0,0,*])
  ducmt = fltarr(nt)
  duemt = fltarr(nt)
  for it = 0, nt-1 do begin
   ducmt[it] = total(ducmass[*,*,it]*area)/1e9
   duemt[it] = total(duem[*,*,it]*area*3*3600.)/1e9
  endfor
