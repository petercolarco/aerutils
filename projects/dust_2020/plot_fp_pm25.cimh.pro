; Make plots of the dust AOD from the BSC group
  ...
  while(long(yyyymmdd) le 20200618L) do begin
   print, yyyymmdd
   read_fp, yyyymmdd, 'dusmass25', index, lon, lat, pm25
   pm25 = pm25*1e9

;  Make a plot
   set_plot, 'ps'
   device, file='fp_pm25.cimh.'+yyyymmdd+'.+'+string(index*1,format='(i03)')+'.ps', $
...
;  hack to fit the forecast date as the plot title
   ...
   contour, /overplot, pm25, lon, lat, /cell, c_colors=colors, levels=levels
...
;   xyouts, .5, 0.95, yyyymmdd, align=.5, /normal, chars=1.5
   ...
   xyouts, .5, 0.95, 'Surface Dust Concentration [PM2.5, !Mm!3g m!E-3!N] Valid '+str, align=.5, /normal, chars=1.5, color=0
