...
; get header line
  ...
  fltalt = data[*,3]
  fltso2 = data[*,4]
  fltsec = data[*,0]-data[0,0]

; Check the generated file
  cdfid = ncdf_open('GEOS5_virgas2_'+datestr+'.nc')
...
; Make a map of the traj
  set_plot, 'ps'
  device, file='virgas2.'+datestr+'.ps', /color, /helvetica, font_size=12, $
...
; Curtain
  ...
  loadct, 0
  oplot, fltsec, fltalt/1000., thick=6
  contour, transpose(so2_vmr), transpose(time), transpose(h)/1000., $
...
; Line plot at altitude
  ...
  plot, x[0,*], fltso2, /nodata, $
   xtitle='seconds of flight', ytitle='SO2 [pptv]', $
   title='VIRGAS Flight: '+datestr, $
   yrange=[.1,10000], /ylog, $
   position=[.15,.25,.95,.9]
...
; Pull the model values at altitude
  ...
  for ix = 0, nx-1 do begin
   fltalt_ = interpolate(fltalt,interpol(indgen(i-1),fltsec,x[0,ix]))
   iz = interpol(indgen(nz),h[*,ix],fltalt_)
