; Plot the component AOT over the Arctic for 4 years of MERRA-2
  ...
  ytitle='Particulate Organic Matter AOT'
  filen = 'plot_ocaot.ps'
  var = 'ocexttau'
  yrange=[0,0.14]
...
; Series 1
  ...
  nc4readvar, filename, var, tau, lon=lon, lat=lat, $
              wantnymd=['19810115','19811215']
  area, lon, lat, nx, ny, dx, dy, area, grid='d'
  tau1981 = fltarr(12)
  a = where(lat gt 60)
  for it = 0, 11 do begin
   tau1981[it] = total(tau[*,a,it]*area[*,a])/total(area[*,a])
  endfor
...
; Series 2
  ...
  a = where(lat gt 60)
  for it = 0, 11 do begin
   tau1994[it] = total(tau[*,a,it]*area[*,a])/total(area[*,a])
  endfor
...
; Series 3
  ...
  a = where(lat gt 60)
  for it = 0, 11 do begin
   tau2002[it] = total(tau[*,a,it]*area[*,a])/total(area[*,a])
  endfor
...
; Series 4
  ...
  a = where(lat gt 60)
  for it = 0, 11 do begin
   tau2011[it] = total(tau[*,a,it]*area[*,a])/total(area[*,a])
  endfor
