...
; expids for comparisons
  expid = ['c180R_J10p12p3.tavg2d_aer_x', $
           'c180R_J10p12p3dev_asd.tavg2d_aer_x']
  nexpid = n_elements(expid)
  cexpid = [84,254,0,176]
  tau_ = fltarr(12,nexpid)
  tau_oc_ = fltarr(12,nexpid)

  scale = [1.5,1.125,1.2,1.85,1.25]
...
; Get the compare experiment(s)
  for iexpid = 0, nexpid-1 do begin
   ...
   if( (expid[iexpid] eq 'c180R_J10p12p3dev_asd.tavg2d_aer_x' or $
     ...
     nc4readvar, filename_, comp[icomp], ext, lon=lon, lat=lat
   endelse
ext = ext*scale[icomp]
   area, lon, lat, nx, ny, dx, dy, area
   tau_[*,iexpid] = aave(ext,area,/nan)
   a = where(tau_ eq 0)
...
; Make a nice plot!
  set_plot, 'ps'
  device, file='component_mean.scale.'+comp[icomp]+'.ps', /color, /helvetica, font_size=12, $
...
; Make a nice plot of total!
  set_plot, 'ps'
  device, file='component_mean.scale.totexttau.ps', /color, /helvetica, font_size=12, $
          xsize=16, ysize=10, xoff=.5, yoff=.5
  !p.font=0
  loadct, 39
