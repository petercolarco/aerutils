...
; Fake up an altitude
  ...
  rthresh = [.008, .008] ; microns
  
; put in units of per mg air
  nI33p2 = fltarr(nz,10)
...
;    rh[iz] = .1
   ...
   for ib = 0, n_elements(rthresh)-1 do begin
    ...
    nI33p2[iz,ib] = total(dndr[iz,a]*dr[a])/rhoa[iz]/1000.
    a = where(r_*1e6 gt rthresh[ib] )
    nI33p2cor[iz,ib] = total(dndr[iz,a]*dr_[a])/rhoa[iz]/1000.
   endfor
  endfor

; plot
  set_plot, 'ps'
  device, file='n_gt_8nm.ps', /helvetica, font_size=14, /color
  !p.font=0

  loadct, 39
  plot, nI33p2[*,0], z, /nodata, position=[.2,.1,.9,.9], $
   xrange=[10,10000], yrange=[5,30], /xlog, $
   xtitle = '# mg!E-1!N', ytitle='Alt [km]'
