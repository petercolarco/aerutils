...
; zonal mean and transpose
  su = transpose(total(suexttau,1)/n_elements(lon))


; Now make a plot
  set_plot, 'ps'
  device, file='plot_aod_sulfate.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0

  loadct, 0
  if(strpos(expid,'ocs') gt -1) then begin
   levels = findgen(11)*.0003
  endif else begin
  ...
  endelse
  xyouts, .525, .005, 'AOD', align=.5, /normal
...
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
   ...
   loadct, 39
   axis, yaxis=1, yrange=[0.1,20], ylog=1, ytitle='Sulfate Mass Loading [Tg] / AOD (x 100)', /save, ystyle=1
   oplot, x, sucm, thick=6
   oplot, x, 100*suext, thick=6, color=254
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
   xyouts, 158, .2, '!4Sulfate Mass [Tg]'
   xyouts, 158, .12, '!4Sulfate AOD (x100)', color=254
  endelse
