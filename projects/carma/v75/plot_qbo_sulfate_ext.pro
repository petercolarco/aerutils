...
; Get the sulfate mass loading and AOD
  ...
  nc4readvar, filename, 'suextcoef', suext_, lon=lon, lat=lat, lev=lev, wantlat=[5,10], wantlev=[20.]
  nc4readvar, filename, 'suconc', suconc_, lon=lon, lat=lat, lev=lev, wantlat=[5,10], wantlev=[20.]
; zonal mean and transpose and units to km-1
  ...
  suconc_ = transpose(total(total(suconc_,1)/n_elements(lon),1)/n_elements(lat))
...
;  suext_ = transpose(total(total(suext_,1)/n_elements(lon),1)/n_elements(lat))*1000.
  ...
  if(strpos(expid,'ocs') gt -1) then begin
   loadct, 39
   axis, yaxis=1, yrange=[0,2], ylog=0, $
    ytitle='Sulfate Extinction Coefficient [10!E4!N km!E-1!N]', /save
...
;   oplot, x, suconc_*1e11, thick=6, color=0, lin=2
;   oplot, x, suext_*1e4, thick=6, color=0
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
;   xyouts, 200, .1475, '!4Sulfate Mass [Tg]'
   xyouts, 20, 0.85*2, '!4Sulfate Extinction (20 hPa, ~26.5 km, CARMA)'
   xyouts, 20, 0.79*2, '!4Sulfate Extinction (10 hPa, ~31.5 km, CARMA)', color=254
  endif else begin
   loadct, 39
   axis, yaxis=1, yrange=[0,2], ylog=0, $
    ytitle='Sulfate Extinction Coefficient [10!E4!N km!E-1!N]', /save
