...
; Get the c90F38 bin case
  ...
  if(nx eq 288) then grid='c'
  area, lon, lat, nx, ny, dx, dy, area, grid=grid
  suc90b = aave(sucmass,area)*total(area)/1e9
  suextc90b = aave(suexttau,area)


  set_plot, 'ps'
  device, file='plot_suexttau.compare.ps', /helvetica, font_size=12, /color, $
  ...
  plot, indgen(nt), suc48, /nodata, $
   ...
   xminor = 2, $
   xtickname = xtickn, $
   ytitle = 'AOD [550 nm]', yrange=[.0012,.0018]
  oplot, indgen(nt), suextc48, thick=6
  oplot, indgen(nt), suextc90, thick=6, color=254
  oplot, indgen(nt), suextc90b, thick=6, color=254, lin=2
