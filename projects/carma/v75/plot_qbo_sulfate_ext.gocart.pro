...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_qbo_sulfate_ext.gocart.'+expid+'.ps', /color, /helvetica, font_size=12, $
...
; Get the sulfate mass loading and AOD
  ...
  nc4readvar, filename, 'so4', su, lon=lon, lat=lat, lev=lev, wantlat=[5,10], wantlev=[10.]
  nc4readvar, filename, 'airdens', rhoa, lon=lon, lat=lat, lev=lev, wantlat=[5,10], wantlev=[10.]
; zonal mean and transpose and units to km-1
  suext = transpose(total(total(suext,1)/n_elements(lon),1)/n_elements(lat))*1000.
  suconc = transpose(total(total(su*rhoa,1)/n_elements(lon),1)/n_elements(lat))
...
;  suext_ = transpose(total(total(suext_,1)/n_elements(lon),1)/n_elements(lat))*1000.
  ...
  if(strpos(expid,'ocs') gt -1) then begin
   loadct, 39
   axis, yaxis=1, yrange=[0,3], ylog=0, $
...
;   xyouts, 200, .1475, '!4Sulfate Mass [Tg]'
   xyouts, 150, 2.7, '!4Sulfate Concentration (GOCART)'
   xyouts, 150, 2.52, '!4Sulfate Extinction (GOCART)', color=254
  endif else begin
   loadct, 39
...
; make a second plot of the mass extinction efficiency
  set_plot, 'ps'
  device, file='plot_qbo_sulfate_ext.gocart.mee.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0
  loadct, 39
  plot, x, suext/suconc/1000., /nodata, $
   position=[.1,.2,.9,.9], $
   title = 'Sulfate Mass Extinction Efficiency (GOCART) [m!E2!N kg!E-1!N]', $
   xstyle=1, xminor=2, xticks=xyrs, xrange=[0,xmax+1], $
   xtickname=[xtickname,' '], $
  ...
  xyouts, 150, 2.7, '!4Sulfate Concentration (GOCART)', color=84
  xyouts, 150, 2.52, '!4Sulfate Extinction (GOCART)', color=254
