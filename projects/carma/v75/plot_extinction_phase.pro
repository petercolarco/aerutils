...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_extinction_phase.westerly.'+expid+'.ps', /color, /helvetica, font_size=12, $
...
;  levels = [1,5,10,20,30,40,60]/100.
  ...
  contour, suwest*1e6, lat, zu, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, suwest, lat, zu, /nodata, /noerase, $
   title = 'Sulfate Extinction Coefficient [Mm!E-1!N], westerly QBO phase', $
   position=[.1,.2,.9,.9], $
...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_extinction_phase.easterly.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0
...
;  levels = [1,5,10,20,30,40,60]/100.
  ...
  contour, sueast*1e6, lat, zu, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, sueast, lat, zu, /nodata, /noerase, $
   title = 'Sulfate Extinction Coefficient [Mm!E-1!N], easterly QBO phase', $
   position=[.1,.2,.9,.9], $
   xstyle=1, xrange=[-90,90], xtitle='latitude (degrees)', xticks=6, $
   ystyle=1, yrange=[15,35], ytitle = 'Altitude [km]'
