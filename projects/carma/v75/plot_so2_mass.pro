...
;  nc4readvar, filename, 'so4cmass', sucmass, lon=lon
  nc4readvar, filename, 'so2cmass', so2cmass, lon=lon
  nx = n_elements(lon)
  case nx of
   144: grid='b'
   288: grid='c'
...
;  sucm    = aave(sucmass,area)*total(area)/1.e9  ; Tg
  so2cm   = aave(so2cmass,area)*total(area)/1.e9  ; Tg


; zonal mean and transpose
;  su  = transpose(total(sucmass,1)/n_elements(lon))*1.e6
  so2 = transpose(total(so2cmass,1)/n_elements(lon))*1.e6


; Now make a plot
  set_plot, 'ps'
  device, file='plot_so2_mass.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0

  loadct, 0
  if(strpos(expid,'ocs') gt -1) then begin
   levels = findgen(11)*.003
  endif else begin
  ...
  contour, so2, x, lat, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, so2,x,lat, /nodata, noerase=1, $
   position=[.1,.2,.9,.9], $
   xstyle=1, xminor=2, xticks=xyrs, xrange=[0,xmax+1], $
   xtickname=[xtickname,' '], $
   ystyle=9, yrange=[-60,60],  yticks=6, $
  ...
  endelse
  xyouts, .525, .005, 'Zonal Mean SO2 Mass Loading [mg m!E-2!N]', align=.5, /normal
  ...
  if(strpos(expid,'ocs') gt -1) then begin
   loadct, 39
   axis, yaxis=1, yrange=[0.005,0.011], ylog=0, ytitle='SO2 Mass Loading [Tg]', /save, ystyle=1
   oplot, x, so2cm, thick=6
;   oplot, x, 100*suext, thick=6, color=254
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
   xyouts, 158, .00535, '!4SO2 Mass [Tg]'
