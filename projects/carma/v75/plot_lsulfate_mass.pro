...
; Get the sulfate mass loading and AOD
  ...
  nc4readvar, filename, ['sudp','susd','suwt','susv'], sucmass, lon=lon, /sum
...
; zonal mean and transpose
  su = transpose(total(sucmass,1)/n_elements(lon))*1.e6*30*86400 ; mg m-2 month


; Now make a plot
  set_plot, 'ps'
  device, file='plot_lsulfate_mass.'+expid+'.ps', /color, /helvetica, font_size=12, $
  ...
  endelse
  xyouts, .525, .005, 'Zonal Mean Sulfate Mass Loss [mg m!E-2!N mon!E-1!N]', align=.5, /normal
  ...
  if(strpos(expid,'ocs') gt -1) then begin
   loadct, 39
   axis, yaxis=1, yrange=[0.007,0.013], ylog=0, ytitle='Sulfate Mass Loss [Tg mon!E-1!N]', /save, ystyle=1
   oplot, x, sucm, thick=6
;   oplot, x, 100*suext, thick=6, color=254
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
   xyouts, 158, .00735, '!4Sulfate Mass Loss [Tg mon!E-1!N]'
