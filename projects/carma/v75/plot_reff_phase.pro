...
; Get the westerly fields
  ...
  nc4readvar, filenames, 'sureff', surwest, lon=lon, lat=lat, lev=lev
  filenames = filename[where(westerly eq -1)]
  nc4readvar, filenames, 'sureff', sureast, lon=lon, lat=lat, lev=lev
...
; reset undef values and screen min extinction
  surwest[where(surwest gt 1e14 or suwest lt 1e-8)] = !values.f_nan
  sureast[where(sureast gt 1e14 or sueast lt 1e-8)] = !values.f_nan
; zonal mean
  surwest = mean(surwest,dim=1,/nan)
  sureast = mean(sureast,dim=1,/nan)
; Temporal mean
  surwest = mean(surwest,dim=3,/nan)
  sureast = mean(sureast,dim=3,/nan)
...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_reff_phase.westerly.'+expid+'.ps', /color, /helvetica, font_size=12, $
...
;  levels = [1,5,10,20,30,40,60]/100.
  ...
  loadct, 52
  levels = findgen(11)*.01 + .12
  dcolors = indgen(11)*25

  contour, surwest, lat, zu, /nodata, $
   position=[.1,.2,.9,.9], $
   xstyle=1, xrange=[-90,90], xtitle=' ', xticks=6, $
   ystyle=1, yrange=[15,35], ytitle = ' '

  contour, surwest*1e6, lat, zu, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, surwest, lat, zu, /nodata, /noerase, $
   title = 'Sulfate Effective Radius [!9m!3m], westerly QBO phase', $
   position=[.1,.2,.9,.9], $
...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_reff_phase.easterly.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0
...
;  levels = [1,5,10,20,30,40,60]/100.
  ...
  contour, sureast, lat, zu, /nodata, $
   position=[.1,.2,.9,.9], $
   xstyle=1, xrange=[-90,90], xtitle=' ', xticks=6, $
   ystyle=1, yrange=[15,35], ytitle = ' '

  contour, sureast*1e6, lat, zu, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, sureast, lat, zu, /nodata, /noerase, $
   title = 'Sulfate Effective Radius [!9m!3m], easterly QBO phase', $
   position=[.1,.2,.9,.9], $
   xstyle=1, xrange=[-90,90], xtitle='latitude (degrees)', xticks=6, $
   ystyle=1, yrange=[15,35], ytitle = 'Altitude [km]'
