; Make a plot of the global mean sulfate column mass
  ...
  expid = 'c48F_pI33p5_carma_ex'
  ctl   = expid+'.tavg2d_carma_x.ctl'
  ga_times, ctl, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
stop
  nc4readvar, filename, 'sucmass', sucmass, lon=lon, lat=lat
  nc4readvar, filename, 'ducmass', ducmass, lon=lon, lat=lat
  nc4readvar, filename, 'sscmass', sscmass, lon=lon, lat=lat
  nc4readvar, filename, 'smcmass', smcmass, lon=lon, lat=lat
  nx = n_elements(lon)
  if(nx eq 144) then grid='b'
  if(nx eq 288) then grid='c'
  area, lon, lat, nx, ny, dx, dy, area, grid=grid
  sucmass = aave(sucmass,area)*total(area)/1e9
  ducmass = aave(ducmass,area)*total(area)/1e9
  sscmass = aave(sscmass,area)*total(area)/1e9
  smcmass = aave(smcmass,area)*total(area)/1e9
  ...
  set_plot, 'ps'
  device, file='plot_ducmass.mixed.ps', /helvetica, font_size=12, /color, $
  ...
  plot, indgen(nt), sucmass, /nodata, $
   ...
   xthick=6, ythick=6, $
   xtickname = xtickn, $
   ytitle = 'Dust column loading [Tg]', yrange=[0,75]
  oplot, indgen(nt), mxducmass, thick=12, color=208, lin=2
  oplot, indgen(nt), ducmass, thick=12, color=208
  oplot, indgen(nt), gducmass, thick=12
