...
; Get the sulfate mass loading and AOD
  ...
  nc4readvar, filename, 'h2so4cmass', sucmass, lon=lon
  nx = n_elements(lon)
  case nx of
   144: grid='b'
   288: grid='c'
...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_h2so4v_mass.'+expid+'.ps', /color, /helvetica, font_size=12, $
   xsize=24, ysize=12, xoff=.5, yoff=.5
  !p.font=0

  loadct, 0
  if(strpos(expid,'ocs') gt -1) then begin
   levels = findgen(11)*.002
  endif else begin
  ...
  endelse
  xyouts, .525, .005, 'Zonal Mean H2SO4vapor Mass Loading [mg m!E-2!N]', align=.5, /normal
  ...
  if(strpos(expid,'ocs') gt -1) then begin
   loadct, 39
   axis, yaxis=1, yrange=[0.002,0.008], ylog=0, ytitle='H2SO4v Mass Loading [Tg]', /save, ystyle=1
   oplot, x, sucm, thick=6
;   oplot, x, 100*suext, thick=6, color=254
;  polyfill, [156,198,198,156,156], [.1425,.1425,.15,.15,.1425], color=255
   xyouts, 158, .00235, '!4H2SO4v Mass [Tg]'
