...
a= where(nymd gt 19841231L and nymd lt 19910101L)
filename = filename[a]
nymd = nymd[a]
...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_qbo_sulfate_parker.'+expid+'.ps', /color, /helvetica, font_size=12, $
...
;  xtickname[1:xyrs-1:2] = ' '
  ...
  contour, u, x, lev, /nodata, noerase=1, $
   ...
   xstyle=1, xminor=2, xticks=xyrs, xrange=[0,xmax+1], $
   xtickname=[xtickname,' '], $
   ystyle=1, yrange=[100,10],  yticks=7, ytickv=[100,70,50,40,30,20,15,10], $
   ytitle = 'pressure [hPa]', /ylog
