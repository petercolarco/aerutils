...
; do a size distribution
  ...
  set_plot, 'ps'
  device, file='global_dust_vfall.maring.ps', /color, /helvetica, font_size=14
  !p.font=0
...
; Apply Maring correction
   vfall = dus[0,45,72,*]*100.
   oplot, 2.*r, vfall, thick=6
   for i = 0, n_elements(r)-1 do begin
    vfall[i] = max([vfall[i]-.33,0.0001])
   endfor
   oplot, 2.*r, vfall, thick=6, lin=1
;  Find the fall speeds that correspond to falling 1 km in some time
   daystofall = [1./24,1.,30.,365.,3650.]
   ytickv = 1.e5/daystofall/86400.
   axis, yaxis=1, yrange=[1e-4,10], ystyle=1, /ylog, /save, $
    ytitle='Time to fall 1 km', yticks=4, ytickv=ytickv, $
    ytickn=['1 hour','1 day','1 month', '1 year','1 decade']
