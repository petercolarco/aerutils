...
;gf = 0
  ...
  set_plot, 'ps'
  device, file='plot_nh3_budget.'+expid+'.ps', $
  ...
  loadct, 39
   ...
   nc4readvar, filename, 'nh3cmass', ducmass, lon=lon, lat=lat, time=time
   if(gf) then begin
    nc4readvar, filename, 'nh3sv', dusv, rc=rc
    ...
    nc4readvar, filename, 'nh3sv', dusv, /sum, /tem
   endelse
   nc4readvar, filename, 'nh3dp', dudp, /sum, /tem
   nc4readvar, filename, 'nh3wt', duwt, /sum, /tem
   nc4readvar, filename, ['nh3em'], duem, /sum
   ...
   oplot, x, dudp, thick=6, color=208
   oplot, x, duem, thick=6, color=160
   oplot, x, duwt+dusv+dudp, thick=6, color=254, lin=2

; print the average values
  mloss = mean(duwt+dusv+dudp,/nan)
  ...
  plot, xt, /nodata, xticks=20, /noerase, $
   ...
   yrange=[-0.5,1], xrange=[2000,2020], $
   position=[.05,.05,.95,.4], xminor=1, yticks=7, $
   ytickn=[string((indgen(6)-2)*0.25,format='(f5.2)'),' '], yminor=2
   oplot, x, dum, thick=6
   tot = duem - (dudp+duwt+dusv)
