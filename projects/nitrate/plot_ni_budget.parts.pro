...
;gf = 0
  ...
  set_plot, 'ps'
  device, file='plot_ni_budget.parts.'+expid+'.ps', $
  ...
  plot, xt, indgen(241), /nodata, xticks=20, $
   yrange=[-5,15], xrange=[2000,2020],  $
;   position=[.05,.4,.95,.95], xminor=1, xtickn=make_array(21,val=' ')
   position=[.05,.1,.95,.95], xminor=1, xtickn=make_array(21,val=' ')
...
;   nc4readvar, filename, 'nh4cmass', nh4cmass, /sum, /tem
   nc4readvar, filename, 'niwt0', duwt, /sum, /tem
   nc4readvar, filename, ['niht001','niht002','niht003'], duht, /sum
   nc4readvar, filename, ['nipno3aq'], duem, lon=lon, lat=lat, time=time
...
;   nh4cmass = aave(nh4cmass,area)*total(area)/1.e9
   ...
   duht = aave(duht,area)*total(area)/1.e9*ndaysmon*86400
   x = 2000+expip+findgen(n_elements(time))/12.
   oplot, x, duwt, thick=6, color=84
   oplot, x, dusv, thick=6, color=84, lin=2
   oplot, x, dusd, thick=6, color=208
   oplot, x, dudp, thick=6, color=208, lin=2
   oplot, x, duht, thick=6, color=160
   oplot, x, duem, thick=6, color=160, lin=2
...
; print the average values
  ...
  if(expip ge 5) then begin
   ...
   xoff2 = max(x)+3
  endelse
  xyouts, xoff1, mhet, string(mhet,format='(f6.1)')+' Tg', color=176
  xyouts, xoff2, memi, string(memi,format='(f6.1)')+' Tg', color=176
  xyouts, xoff1, mwet, string(mwet,format='(f6.1)')+' Tg', color=84
  xyouts, xoff2, mscv, string(mscv,format='(f6.1)')+' Tg', color=84
  xyouts, xoff1, msed, string(msed,format='(f6.1)')+' Tg', color=208
  xyouts, xoff2, mdry, string(mdry,format='(f6.1)')+' Tg', color=208
