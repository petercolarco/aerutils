...
; Screen reff where SU < 0.01 ug kg-1
  ...
  set_plot, 'ps'
  device, file='reff.volc.'+expid+'.'+dtitle+'.ps', /color, /helvetica, font_size=11, $
...
;  levels = [0.025,0.05,0.075,0.1,0.125,0.15,0.175,0.2,0.25]
  ...
  makekey, .15, .06, .8, .05, 0., -.05, align=0, $
           color=colors, labels=string(levels,format='(f4.2)')
  loadct, 56
  makekey, .15, .06, .8, .05, 0., -.05, align=0, $
...
; Overplot the zonal mean SU mixing ratio
  loadct, 0
  levels = [.1,.2,.5,1,2,5,10,20,50]
  contour, su, lat, z, levels=levels,/over, color=80, c_label=indgen(10)
