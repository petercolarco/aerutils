...
; Plot the zonal mixing ratio of sulfate over time
  ...
  nt = n_elements(nymd)
  nc4readvar, filename, 'so4s', su, lon=lon, lat=lat, lev=lev
...
; Form into a zonal mean
  su = reform(total(su*rhoa,1)/n_elements(lon))*1e12  ; ng m-3
...
; Loop and plot
  ...
  set_plot, 'ps'
  device, file='plot_zonal_vertical_gocart.strat.clim.'+expid+'.'+strmid(nymd[i],0,6)+'.ps', /color, $
