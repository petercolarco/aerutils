...
; Now make a plot
  set_plot, 'ps'
  device, file='plot_carma_conc.'+expid+'.ps', $
...
;  xtickname[1:xyrs-1:2] = ' '
  ...
  tvlct, red, green, blue
  dcolors=indgen(n_elements(red))
  contour, su*1e12, indgen(n_elements(nymd)), lev, /over, $
   levels=levels, c_color=dcolors, /cell

  loadct, 0
  contour, su*1e12, x, lev, /nodata, noerase=1, $
   position=[.1,.2,.95,.9], $
   xstyle=1, xminor=2, xticks=xyrs, xrange=[0,xmax+1], $
   xtickname=[xtickname,' '], $
   ystyle=9, yrange=[100,5], yticks=4, ytickv=[100,50,20,10,5], $
  ...
  makekey, .1, .08, .85, .04, 0., -0.04, color=findgen(7), $
           labels=string([levels],format='(i4)'), align=.5
  xyouts, .525, .01, 'CARMA OCS Produced Sulfate Concentration [ng m!E-3!N]', align=.5, /normal
  tvlct, red, green, blue
  dcolors=indgen(n_elements(red))
