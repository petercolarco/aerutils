...
; For every monthly mean available for that year a plot is made.
 ...
 !quiet = 1l
  ...
  if(aertype eq 'OCS') then plot_title = 'GEOS ('+expid+') SU from OCS '
...
;           vars = ['emis','dep','wet','scav','load','tau']
   ...
   'POM':  begin
           ...
           formcode = '(f5.2)'
           vars = ['emis','dep','wet','scav','load','tau','ssa','tauabs','taufrac','embb','embf','eman','embg']
;           vars = ['emis','dep','wet','scav','load','tau','embbbo','embbnb']
  ...
  'OCS':   begin
           vars = ['load','tau','sed','dep','wet','scav','pso4g']
           end
  'SUV':   begin
           vars = ['emis','emisso2','emisdms','dep','depso2','wet','scav',$
                   'pso4g','pso4liq','pso2','load','loadso2','loaddms', $
...
;   Check to flip the levelarray
    ...
    plotgrid, varvalue*convfac, levelarray_, dcolors, lon, lat, dx, dy, map=map
    ...
    map_set, limit=geolimit, position=position[*,imon], noerase=noerase, $
             /hammer, /iso, /noborder, /horizon, glinestyle=0, color=160, glinethick=.5
    if(strmid(vars[ipage],0,4) eq 'load') then convfac = convfac * (ndays*86400.)
...
;  If variable not available then exit loop
   ...
   for ii = 9, 11 do begin
    ...
    if(aertype eq 'OCS') then begin
     scalestr = ''
     if(strmid(vars[ipage],0,3) eq 'load') then scalestr = 'x10!E-3!N'
     if(strmid(vars[ipage],0,3) eq 'tau') then scalestr = 'x10!E-3!N'
     if(strmid(vars[ipage],0,3) eq 'dep') then scalestr = 'x10!E-6!N'
     if(strmid(vars[ipage],0,3) eq 'sed') then scalestr = 'x10!E-6!N'
     if(strmid(vars[ipage],0,3) eq 'wet') then scalestr = 'x10!E-5!N'
     if(strmid(vars[ipage],0,4) eq 'scav') then scalestr = 'x10!E-5!N'
     if(strmid(vars[ipage],0,4) eq 'pso4') then scalestr = 'x10!E-5!N'
