  filetemplate = 'omps.ctl'
  ga_times, filetemplate, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
  a = where(nymd lt 20130101L)
  nymd = nymd[a]
  filename = filename[a]
  nc4readvar, filename, 'troph', tropp, lon=lon, lat=lat
  nc4readvar, filename, 'scatang', scat, lon=lon, lat=lat
  nc4readvar, filename, 'ext869', ext
  ...
  a = where(tropp gt 1.e14)
  tropp[a] = !values.f_nan
  a = where(scat gt 1.e14)
  scat[a] = !values.f_nan
  a = where(ext gt 1.e14)
  ext[a] = !values.f_nan


  aod = make_array(nx,ny,nt,val=!values.f_nan)
  for it = 0, nt-1 do begin
   for iy = 0, ny-1 do begin
    for ix = 0, nx-1 do begin
     if(finite(tropp[ix,iy,it]) eq 1 and finite(scat[ix,iy,it]) eq 1) then begin
      if(scat[ix,iy,it] le 148) then begin
       a = where(alt ge tropp[ix,iy,it])
       aod[ix,iy,it] = total(ext[ix,iy,a,it])*1e6
      endif
     endif
    endfor
   endfor
  ...
  area, lon, lat, nx, ny, lon[1]-lon[0], lat[1]-lat[0], area
