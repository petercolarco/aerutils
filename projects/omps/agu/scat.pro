...
; --------------------------
  get_profile_properties, rh, delp, tables, const, ssabak, taubak, gbak, pmombak
...
; --------
  expid = 'MERRA2_GMI'
  filename = '/misc/prc13/MERRA2_GMI/c180/MERRA2_GMI.inst3_3d_aer_Nv.monthly.199208.nc4'
  wantlon = 10.
  wantlat = 10.
  nc4readvar, filename, 'rh', rh, wantlon=wantlon, wantlat=wantlat, lev=lev, lon=lon, lat=lat
  nc4readvar, filename, 'delp', delp, wantlon=wantlon, wantlat=wantlat, lev=lev, lon=lon, lat=lat
...
; Compute the vertical optical properties
  ...
  ssapin  = fltarr(nz)
  gpin    = fltarr(nz)
  pmompin = fltarr(nz,nmom)

; Get the aerosol properties
; --------------------------
  get_profile_properties, rh, delp, tables, const, ssapin, taupin, gpin, pmompin
...
; --------
  expid = 'MERRA2_GMI'
  filename = '/misc/prc13/MERRA2_GMI/c180/MERRA2_GMI.inst3_3d_aer_Nv.monthly.200508.nc4'
  wantlon = 10.
  wantlat = 10.
  nc4readvar, filename, 'rh', rh, wantlon=wantlon, wantlat=wantlat, lev=lev, lon=lon, lat=lat
  nc4readvar, filename, 'delp', delp, wantlon=wantlon, wantlat=wantlat, lev=lev, lon=lon, lat=lat
...
; And construct the phase function
  ...
  for iz = 0, nz-1 do begin
     for iang = 0, nangles-1 do begin
        ...
        for imom = 0, nmom do begin
         ...
         p11pin[iang,iz] = p11pin[iang,iz] + pmompin[iz,imom]*leg[imom]
  ...
  set_plot, 'ps'
  device, file='scat.'+expid+'.divide4pi.ps', /color, /helvetica, font_size=14
  !p.font=0
  loadct, 39
...
; total(p11*fac*dmu)*2*pi = 1
  ...
  oplot, angles, p11[*,33]*fac, thick=6, color=254 ; 20 km
  oplot, angles, p11pin[*,33]*fac, thick=6, color=74 ; 20 km
  oplot, angles, p11mod[*,33]*fac, thick=6, color=208 ; 20 km
