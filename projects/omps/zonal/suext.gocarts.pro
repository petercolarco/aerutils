...
; Get a variable
  nc4readvar, filename, ['suextcoef','suextcoefvolc'], su, lon=lon, lat=lat, lev=lev, /sum
  a = where(su gt 1e14)
  su[a] = !values.f_nan
...
;   Now make a plot
    set_plot, 'ps'
    device, file='suext.gocarts.latmin='+strcompress(string(latmin),/rem)+'.ps', $
    ...
    contour, sup*1e6, x, zu, /cell, $
     ...
     title='Sulfate Extinction (all sulfate) [Mm!E-1!N], ' $
