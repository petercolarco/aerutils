...
; Get a variable
  ...
  nc4readvar, filename, 'so4s', su2, lon=lon, lat=lat, lev=lev
  nc4readvar, filename, 'so4v', su3, lon=lon, lat=lat, lev=lev
...
; Colors
  ...
  levels = [50,100,200,400,800,1200,1600]
...
;   Now make a plot
    set_plot, 'ps'
    device, file='so4trop+volc_mr.latmin='+strcompress(string(latmin),/rem)+'.ps', $
    ...
    contour, sup*1e12, x, zu, /cell, $
     ...
     title='Sulfate Mixing Ratio (trop.+volc. sources) [ng kg!E-1!N], ' $
