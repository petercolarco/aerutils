...
; Get a variable
  ...
  nc4readvar, filename, 'suextcoefstrat', su2, lon=lon, lat=lat, lev=lev
  su = su-su2
  a = where(su gt 1e14)
  su[a] = !values.f_nan
; zonal mean
...
;   Now make a plot
    set_plot, 'ps'
    device, file='suext.latmin='+strcompress(string(latmin),/rem)+'.ps', $
    ...
    contour, sup*1e6, x, zu, /cell, $
     ...
     title='Sulfate Extinction (tropospheric sources) [Mm!E-1!N], ' $
