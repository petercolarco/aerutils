...
; This if from ../scatang
  ...
  readf, lun, phaseo
  free_lun, lun
; Blow up to three years
  phaseo = transpose([transpose(phaseo),transpose(phaseo),transpose(phaseo)])
...
;   Now make a plot
    set_plot, 'ps'
    device, file='so4tv_mr.scaled.latmin='+strcompress(string(latmin),/rem)+'.ps', $
