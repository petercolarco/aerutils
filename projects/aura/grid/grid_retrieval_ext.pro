...
; Grid the retrievals to a regular lat/lon grid
  ...
  pro grid_retrieval_ext, filen, tdef, lon, lat, $
                      ler388, aod, ssa, $
...
; create the output filename
  ...
  duext_  = make_array(nx,ny,val=1.e15)
  ssext_  = make_array(nx,ny,val=1.e15)
  suext_  = make_array(nx,ny,val=1.e15)
  ocext_  = make_array(nx,ny,val=1.e15)
  bcext_  = make_array(nx,ny,val=1.e15)
  ...
  prso_    = make_array(nx,ny,val=1.e15)
  ...
  duext__  = make_array(nx,ny,val=0.)
  ssext__  = make_array(nx,ny,val=0.)
  suext__  = make_array(nx,ny,val=0.)
  ocext__  = make_array(nx,ny,val=0.)
  bcext__  = make_array(nx,ny,val=0.)
  ssa__    = make_array(nx,ny,val=0.)
  residue__= make_array(nx,ny,val=0.)
  ai__     = make_array(nx,ny,val=0.)
  prs__    = make_array(nx,ny,val=0.)
  prso__    = make_array(nx,ny,val=0.)
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(duext[ipts] gt -1e14) then begin
    duext__[ix[ipts],iy[ipts]] = duext__[ix[ipts],iy[ipts]] + duext[ipts]
    naodext__[ix[ipts],iy[ipts]] = naodext__[ix[ipts],iy[ipts]] + 1
   endif
  endfor
  a = where(naodext__ gt 0)
  duext_[a] = duext__[a]/naodext__[a]


  pts  = where(ssext gt -1e14)
  ssext = ssext[pts]
  npts = n_elements(pts)
  ix = interpol(indgen(nx), lon_, lon[pts])
  iy = interpol(indgen(ny), lat_, lat[pts])
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(ssext[ipts] gt -1e14) then begin
    ssext__[ix[ipts],iy[ipts]] = ssext__[ix[ipts],iy[ipts]] + ssext[ipts]
   endif
  endfor
  a = where(naod__ gt 0)
  ssext_[a] = ssext__[a]/naodext__[a]


  pts  = where(suext gt -1e14)
  suext = suext[pts]
  npts = n_elements(pts)
  ix = interpol(indgen(nx), lon_, lon[pts])
  iy = interpol(indgen(ny), lat_, lat[pts])
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(suext[ipts] gt -1e14) then begin
    suext__[ix[ipts],iy[ipts]] = suext__[ix[ipts],iy[ipts]] + suext[ipts]
   endif
  endfor
  a = where(naod__ gt 0)
  suext_[a] = suext__[a]/naodext__[a]


  pts  = where(ocext gt -1e14)
  ocext = ocext[pts]
  npts = n_elements(pts)
  ix = interpol(indgen(nx), lon_, lon[pts])
  iy = interpol(indgen(ny), lat_, lat[pts])
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(ocext[ipts] gt -1e14) then begin
    ocext__[ix[ipts],iy[ipts]] = ocext__[ix[ipts],iy[ipts]] + ocext[ipts]
   endif
  endfor
  a = where(naod__ gt 0)
  ocext_[a] = ocext__[a]/naodext__[a]


  pts  = where(bcext gt -1e14)
  bcext = bcext[pts]
  npts = n_elements(pts)
  ix = interpol(indgen(nx), lon_, lon[pts])
  iy = interpol(indgen(ny), lat_, lat[pts])
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(bcext[ipts] gt -1e14) then begin
    bcext__[ix[ipts],iy[ipts]] = bcext__[ix[ipts],iy[ipts]] + bcext[ipts]
   endif
  endfor
  a = where(naod__ gt 0)
  bcext_[a] = bcext__[a]/naodext__[a]
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(prso[ipts] gt -1e14) then begin
    prso__[ix[ipts],iy[ipts]] = prso__[ix[ipts],iy[ipts]] + prso[ipts]
    nprso__[ix[ipts],iy[ipts]] = nprso__[ix[ipts],iy[ipts]] + 1
   endif
  endfor
  a = where(nprso__ gt 0)
  prso_[a] = prso__[a]/nprso__[a]
...
; Write an intermediary file
  ...
  fileout = 'aot.'+rnum+'.nc'
   cdfid = ncdf_create(fileout, /clobber)
    ...
    idDU   = NCDF_VARDEF(cdfid,'duext',[idLon,idLat], /float)
    idSS   = NCDF_VARDEF(cdfid,'ssext',[idLon,idLat], /float)
    idSU   = NCDF_VARDEF(cdfid,'suext',[idLon,idLat], /float)
    idOC   = NCDF_VARDEF(cdfid,'ocext',[idLon,idLat], /float)
    idBC   = NCDF_VARDEF(cdfid,'bcext',[idLon,idLat], /float)
    ...
    idPRSO = NCDF_VARDEF(cdfid,'prso',[idLon,idLat], /float)
    idnAot  = NCDF_VARDEF(cdfid,'naot',[idLon,idLat], /float)
    idnAotext  = NCDF_VARDEF(cdfid,'naotext',[idLon,idLat], /float)
    ...
    idnPrso = NCDF_VARDEF(cdfid,'nprso',[idLon,idLat], /float)
    ...
    ncdf_varput, cdfid, idDU, duext_
    ncdf_varput, cdfid, idSS, ssext_
    ncdf_varput, cdfid, idSU, suext_
    ncdf_varput, cdfid, idOC, ocext_
    ncdf_varput, cdfid, idBC, bcext_
    ...
    ncdf_varput, cdfid, idPRSO, prso_
    ncdf_varput, cdfid, idnAot, naod__
    ncdf_varput, cdfid, idnAotext, naodext__
    ...
    ncdf_varput, cdfid, idnPRSO, nprso__
   ncdf_close, cdfid
