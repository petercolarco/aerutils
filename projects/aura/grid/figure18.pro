...
; AOD is OMI - MERRAero
  ...
  loadct, 0
  map_set, /noerase, limit=[-45,-90,5,30], e_horizon={fill:1,color:200}, $
   /noborder, position=[.025,.575,.475,.925]
  xyouts, .025, .955, /normal, charsize=.75, $
   'a) OMAERUV - MERRAero AOD Difference (September 2007)'
  dx = lon[1]-lon[0]
  dy = lat[1]-lat[0]
  ...
  loadct, 0
  plots, [-65,-55,-55,-65,-65], [-22,-22,-18,-18,-22], thick=6, lin=0
...
; AOD is OMI - MERRAero
  ...
  xyouts, .525, .955, /normal, charsize=.75, $
   'b) OMAERUV - MERRAero SSA Difference (September 2007)'
  dx = lon[1]-lon[0]
  dy = lat[1]-lat[0]
...
; a box, and compare model SSA to OMAERUV SSA for dust retrievals
  files  = file_search('/misc/prc14/colarco/aura/dR_MERRA-AA-r2-v1621f_Full/', $
                       'OMI-Aura_L2-OMAERUV_2007m09*vl_rad.geos5_pressure.he5')
...
; Sahel box
; Sort dust by AI
  a = where(lon gt -65 and lon le -55 and lat gt -22 and lat le -18 and $
            aert eq 1 and aodomi gt 0)
  loadct, 39
