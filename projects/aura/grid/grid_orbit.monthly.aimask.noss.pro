; Grid only places where OMI returned an AOD
  ...
  file  = file_search(filed,'*2007m*.geos5_pressure.ext_noss.he5')

; Get the mask
  aimaskdir = '/misc/prc14/colarco/aura/dR_MERRA-AA-r2-v1621f_Full_returned/PGEO/'
...
;  Get the information write a grads template
   ...
   fileout = strmid(file[ifile],0,x+2)+'.vl_rad.geos5_pressure.noss.monthly.aimask.he5'
   ...
   if(ifile eq sd) then begin
    read_retrieval, file(ifile), lon, lat, $
                    ler388, aod, ssa, $
                    residue, ai, prs, prso, $
                    rad354, rad388, aimaskdir=aimaskdir
    nxy = n_elements(lon)
    lon = reform(lon,nxy)
    lat = reform(lat,nxy)
    ...
    read_retrieval, file(ifile), lon_, lat_, $
                    ...
                    rad354_, rad388_, aimaskdir=aimaskdir
