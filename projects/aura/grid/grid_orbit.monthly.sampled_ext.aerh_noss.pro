; Grid only places where OMI returned an AOD

  filed = '/misc/prc14/colarco/aura/dR_MERRA-AA-r2-v1621f_Full/'
  file  = file_search(filed,'*2007m07*.geos5_pressure.ext_noss.he5')

; Get the mask
  aodmaskdir = '/misc/prc14/colarco/aura/dR_MERRA-AA-r2-v1621f_Full_returned/PGEO_noss/'
...
;  Get the information write a grads template
   ...
   fileout = strmid(file[ifile],0,x+2)+'.vl_rad.geos5_pressure.monthly.sampled_ext.aerh_noss.he5'
   ...
   if(ifile eq sd) then begin
    read_retrieval_ext, file(ifile), lon, lat, $
                    ...
                    residue, ai, prs, prso, $
                    rad354, rad388, aerhmaskdir=aodmaskdir
    nxy = n_elements(lon)
    lon = reform(lon,nxy)
    lat = reform(lat,nxy)
    ...
    read_retrieval_ext, file(ifile), lon_, lat_, $
                    ler388_, aod_, ssa_, $
                    duext_, ssext_, suext_, ocext_, bcext_, $
                    residue_, ai_, prs_, prso_, $
                    rad354_, rad388_, aerhmaskdir=aodmaskdir
   ...
   grid_retrieval_ext, fileout, tdef, lon, lat, $
                   ler388, aod, ssa, $
                   duext, ssext, suext, ocext, bcext, $
                   residue, ai, prs, prso, $
                   rad354, rad388, $
