; Plot from the OMI granule
  ...
  set_plot, 'ps'
  device, file='ler.ps', xsize=16, ysize=8, xoff=.5, yoff=.5, /color
  loadct, 39
  !p.font=0
  !p.multi=[0,2,1]
...
;  refl = refl-(albedo[1,*,*]-albedo[0,*,*])
  ...
  map_set, /cont, /noerase, title='Pete - Uncorrected LER 388nm > 0'
  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
    if(refl[ix,iy] gt 0) then begin
     val = refl[ix,iy]
     color = fix(val*2300+.5)    ; Max LER is ~ 0.11 so this scales to color max 254.
  ...
  for i = 0, 10 do begin
   labels[i*23] = string(i/100.,format='(f4.2)')
  endfor
...
; Put onto same grid as below will have...Santiago gives 60x1201
  ...
  map_set, /cont, /noerase, title='Santiago - Uncorrected LER 388nm > 0'
  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
