; Colarco
; Gather retrieval information for a specified month
...
print, ifile, '/', n_elements(files)-1
...
; Reduce data set only to points where OMI retrieved
  a = where(aodomi gt 0 and prs lt 1013.25 and prs gt 600.)
  aodomi = aodomi[a]
  aiomi  = aiomi[a]
  ssaomi = ssaomi[a]
  aert   = aert[a]
  ...
  save, file='getregions.2007'+mm+'.sav', /all
