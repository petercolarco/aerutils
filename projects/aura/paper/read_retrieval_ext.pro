...
; Read my gridded retrievals files
  ...
  pro read_retrieval_ext, filen, lon, lat, $
    ...
    if(doaae) then begin
     ...
     ssa    = h5d_read(var_id)
     aae    = -alog( (aod[1,*,*]-ssa[1,*,*]) / (aod[0,*,*]-ssa[0,*,*])) / alog(388./354.)
     aae    = reform(aae,nxy)
    endif
...
; returns to that
  if(keyword_set(aodmaskdir)) then begin
   ...
   aod[a] = min(aodmask)
   if(doaae) then aae[a] = min(aodmask)
   duext[a] = min(aodmask)
   ssext[a] = min(aodmask)
   suext[a] = min(aodmask)
   ocext[a] = min(aodmask)
...
; returns to that
  if(keyword_set(aimaskdir)) then begin
   ...
   ler388[a] = min(aimask)
   aod[a] = min(aimask)
   if(doaae) then aae[a] = min(aimask)
   duext[a] = min(aimask)
   ssext[a] = min(aimask)
   suext[a] = min(aimask)
   ocext[a] = min(aimask)
