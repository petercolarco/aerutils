...
; Get a time series of AI and scatter against OMI
  ...
  plotfile = 'aaot_scatter.samerica.v1_5.ps'

  ga_times, 'v1_5.ddf', nymd, nhms, template=filetemplate
  filename = strtemplate(filetemplate,nymd,nhms)
  nc4readvar, filename, 'aaot', aot, lon=lon, lat=lat
  ga_times, 'omi.ddf', nymd, nhms, template=filetemplate
  filename = strtemplate(filetemplate,nymd,nhms)
  nc4readvar, filename, 'aaot', aot_omi

  a = where(lon ge -80 and lon le -30)
  aot = aot[a,*,*]
  aot_omi = aot_omi[a,*,*]
  b = where(lat ge -40 and lat le 10)
  aot = aot[*,b,*]
  aot_omi = aot_omi[*,b,*]
  ...
  x = alog10(aot_omi[where(finite(aot) eq 1)]+.001)
  y =alog10( aot[where(finite(aot) eq 1)]+.001)
  ...
  xyouts, -1.75, .9, 'log!D10!N(AAOT+0.001) based statistics', charsize=.65
