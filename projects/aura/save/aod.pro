...
; Make a plot
  set_plot, 'ps'
  device, file='aod.ps', xsize=12, ysize=16, xoff=.5, yoff=.5, /color
  loadct, 39
  !p.font=0
  !p.multi=[0,1,1]
...
; reduce aod
  ...
  a = where(finite(aod) eq 1)
  maxx = max(aod[a])

  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
    if(finite(aod[ix,iy]) eq 1) then begin
     val = aod[ix,iy]
  ...
  xyouts, .5, .95, 'AOD [388 nm]', /normal, align=.5
  device, /close
