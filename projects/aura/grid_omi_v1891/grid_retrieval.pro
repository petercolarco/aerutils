...
; create the output filename
  ...
  maod_   = make_array(nx,ny,val=1.e15)
  mssa_   = make_array(nx,ny,val=1.e15)
  ...
  maod__   = make_array(nx,ny,val=0.)
  mssa__   = make_array(nx,ny,val=0.)
  ...
  nmaod__   = make_array(nx,ny,val=0)
  nmssa__   = make_array(nx,ny,val=0)
...
stop
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(maod[ipts] gt -1e14) then begin
    maod__[ix[ipts],iy[ipts]] = maod__[ix[ipts],iy[ipts]] + maod[ipts]
    nmaod__[ix[ipts],iy[ipts]] = nmaod__[ix[ipts],iy[ipts]] + 1
   endif
  endfor
  a = where(nmaod__ gt 0)
  maod_[a] = maod__[a]/nmaod__[a]
  ...
  if(a[0] ne -1) then ix[a] = 0
  for ipts = 0L, npts-1 do begin
   if(mssa[ipts] gt -1e14) then begin
    mssa__[ix[ipts],iy[ipts]] = mssa__[ix[ipts],iy[ipts]] + mssa[ipts]
    nmssa__[ix[ipts],iy[ipts]] = nmssa__[ix[ipts],iy[ipts]] + 1
   endif
  endfor
  a = where(nmssa__ gt 0)
  mssa_[a] = mssa__[a]/nmssa__[a]
...
; Write an intermediary file
  ...
  fileout = 'aot.'+rnum+'.nc'
   cdfid = ncdf_create(fileout, /clobber)
    ...
    idmAot = NCDF_VARDEF(cdfid,'maot',[idLon,idLat], /float)
    idmSSA = NCDF_VARDEF(cdfid,'mssa',[idLon,idLat], /float)
    ...
    ncdf_varput, cdfid, idmAot, maod_
    ncdf_varput, cdfid, idmSSA, mssa_
