...
;   Get the OMI returned values
    ao2_    = make_array(nx,ny,val=-1.e30)
    cdfid = ncdf_open(ofiles[ifile])
    id    = ncdf_varid(cdfid,'SSA388')
    ncdf_varget, cdfid, id, ao2_inp
    ncdf_close, cdfid
    ao2_[*,199:1399] = ao2_inp
    ao2_ = reform(ao2_,nxy)
    a = where(ao2_ lt -100)
    aod_[a] = -1.e30
...
; Now set up a plot
  set_plot, 'ps'
  device, file='ssa388_mask.ps', xsize=16, ysize=12, xoff=.5, yoff=.5, /color, /helvetica
  !p.font=0
  loadct, 39
  p0 = 0
  p1 = 0
  ...
  for ixy = 0, nxy-1 do begin
   plots, lon[ixy], lat[ixy], color=aod[ixy], psym=8
  endfor
