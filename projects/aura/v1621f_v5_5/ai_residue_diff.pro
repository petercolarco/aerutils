...
; Make a plot
  ...
  set_plot, 'ps'
  device, file='ai_residue_diff.ps', xsize=12, ysize=16, xoff=.5, yoff=.5, /color
  loadct, 39
  !p.font=0
  !p.multi=[0,1,1]
...
minn = -.4
  ...
  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
    if(finite(diff[ix,iy]) eq 1) then begin
     val = diff[ix,iy]-minn
  ...
  xyouts, .5, .95, 'OMAERUV - MERRAero AI', /normal, align=.5
  device, /close
