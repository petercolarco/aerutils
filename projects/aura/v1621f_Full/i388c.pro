; Plot from the OMI granule
  ...
  set_plot, 'ps'
  device, file='i388c.ps', xsize=16, ysize=8, xoff=.5, yoff=.5, /color
  loadct, 39
  !p.font=0
  !p.multi=[0,2,1]
...
; This is the LER I stuffed into OMI granule (only at 388 nm)
  ...
  map_set, /cont, /noerase, title='Pete - I388calc(Ray) > 0'
  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
    if(i388c[ix,iy] gt 0) then begin
     val = i388c[ix,iy]
     color = fix(val*3000+.5)    ; Max i388c is ~ 0.084 so this scales to color max 254.
...
; Put onto same grid as below will have...Santiago gives 60x1201
  ...
  map_set, /cont, /noerase, title='Santiago - I388calc(Ray) > 0'
  for ix = 0, 59 do begin
   for iy = 0, 1643 do begin
    if(ler_return[ix,iy] gt 0) then begin
     val = ler_return[ix,iy]
     color = fix(val*3000+.5)    ; 
