...
; Read in the synthetic OMI data and plot the radiances at 388 nm for
; a day
...
; Now set up a plot
  set_plot, 'ps'
  device, file='rad388.ps', xsize=16, ysize=12, xoff=.5, yoff=.5, /color, /helvetica
  !p.font=0
  loadct, 39
  p0 = 0
  p1 = 0
  ...
  for ixy = 0, nxy-1 do begin
   plots, lon[ixy], lat[ixy], color=rad[ixy], psym=8
  endfor
