...
; Pick off the "Octobers"
  a = where(long(nymd)/10000L ge 2001 and long(nymd)/10000L le 2020 and $
            long(nymd-long(nymd)/10000L*10000L)/100L eq 10)
  nymd = nymd[a]
  nhms = nhms[a]
...
; Reduce to ROI
  ...
  b = where(lat ge 0 and lat le 10)
  nx = n_elements(a)
  ny = n_elements(b)
...
; Now break by day of month
  ...
  set_plot, 'ps'
  device, file='tropatl_ocexttau.clim.october.ps', /color, $
   /helvetica, font_size=14, xsize=28, ysize=18
  !p.font=0
  loadct, 0

  plot, indgen(30), /nodata, $
   xrange=[0,32], yrange=[0,.8], xticks=32, xminor=1, $
