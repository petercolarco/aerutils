...
; Track
  ...
  trj = '/misc/prc19/colarco/CPEX/GEOS.fp.ext_532nm.'+date+'.du.nc'
  cdfid = ncdf_open(trj)
...
; Interpolate to a regular height grid
  ...
  for i = 0, nt-1 do begin
   iy = interpol(findgen(72),h[*,i],z)
   var[i,*] = interpolate(ext[*,i],iy)
  endfor

; Make a plot
  set_plot, 'ps'
  device, file='plot_profile.'+date+'.du.ps', /helvetica, font_size=14, $
...
;  colors=255-indgen(36)*7
  ...
  contour, /over, alog10(var), indgen(nt), z/1000., $
   levels=levels, /cell, c_colors=colors
  for i = 0, n_elements(un)-1 do begin
   plots, un[i], [0,10], thick=6, color=255
  endfor

  plots, indgen(nt), pblh, thick=12, color=255

  axis, yaxis=1, yrange=[0,1000], ytitle='Precipitation [mm day!E-1!N]', $
   color=254, /save
  oplot, indgen(nt), prectot*86400., thick=6, color=254

  loadct, 0
  makekey, /orientation, .9, .1, .03, .8, .05, 0, $
   labels=['0.01',make_array(19,val=' '),'0.1',make_array(15,val=' ')], $
   colors=make_array(36,val=0)
;  loadct, 74
  loadct, 39
  makekey, /orientation, .9, .1, .03, .8, .05, 0, $
   labels=make_array(36,val=' '), colors=colors, /noborder
  xyouts, .98, .5, 'Extinction [Mm!E-1!N]', orient=90, /normal, align=.5
