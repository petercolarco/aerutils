...
; Make a plot
  set_plot, 'ps'
  device, file='plot_met_profile.'+date+'.du.ps', /helvetica, font_size=14, $
...
;  colors=255-indgen(36)*7
  ...
  contour, /over, w, indgen(nt), z/1000., $
   level=[-100.,-.1,-.05,-.01,.01,.05,.1], /cell, c_colors=[48,84,174,255,196,208,254]
  ...
  makekey, /orientation, .9, .1, .03, .8, .05, 0, $
   labels=[' ','-0.1','-0.05','-0.01','0.01','0.05','0.1'], $
   colors=make_array(7,val=0), align=.5
;  loadct, 74
  loadct, 39
  makekey, /orientation, .9, .1, .03, .8, .05, 0, $
   labels=make_array(7,val=' '), colors=[48,84,174,255,196,208,254], /noborder
  xyouts, .98, .5, 'Vertical Velocity [m s!E-1!N]', orient=90, /normal, align=.5
