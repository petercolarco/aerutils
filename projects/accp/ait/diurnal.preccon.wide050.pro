...
;  filename = 'wide050.totexttau.day.hourly.nc'
;  cdfid = ncdf_open(filename)
;  id = ncdf_varid(cdfid,'totexttau')
;  ncdf_varget, cdfid, id, tau_wide050
;  ncdf_close, cdfid

  filename = 'wide050.cloud.nc'
  cdfid = ncdf_open(filename)
...
; reform to get at diurnal cycle
  tau_wide050   = reform(tau_wide050,24,365,10)
...
; Annual mean diurnal cycle and counts
  d_wide050   = make_array(24,10,7,val=!values.f_nan)
  n_wide050   = make_array(24,10,7,val=0)
...
; select not undef
   ...
   if(a[0] ne -1) then begin
    n_wide050(i,j,k) = n_elements(a)
    d_wide050(i,j,k) = mean(tau[i,a,j,k])
   endif
  endfor
  endfor
  endfor
...
; correct to local time
  ...
  for k = 0,6 do begin
  for j = 0,9 do begin
   n_wide050[*,j,k] = shift(n_wide050[*,j,k],xs[j])
   d_wide050[*,j,k] = shift(d_wide050[*,j,k],xs[j])
  endfor
  endfor
...
;  plot the number
   ...
   loadct, 39
   plots, indgen(24), n_wide050[*,j,0], psym=sym(3), symsize=2, color=254
   plots, indgen(24), n_wide050[*,j,0], psym=sym(8), symsize=2, color=0
   plots, indgen(24), n_wide050[*,j,6], psym=sym(4), symsize=2, color=254
   plots, indgen(24), n_wide050[*,j,6], psym=sym(9), symsize=2, color=0
   plots, indgen(24), n_wide050[*,j,1], psym=sym(1), symsize=2, color=254
   plots, indgen(24), n_wide050[*,j,1], psym=sym(6), symsize=2, color=0
   plots, indgen(24), n_wide050[*,j,2], psym=sym(1), symsize=1, color=208
   plots, indgen(24), n_wide050[*,j,2], psym=sym(6), symsize=1, color=0
   plots, indgen(24), n_wide050[*,j,3], psym=sym(1), symsize=1, color=192
   plots, indgen(24), n_wide050[*,j,3], psym=sym(6), symsize=1, color=0
   plots, indgen(24), n_wide050[*,j,4], psym=sym(1), symsize=1, color=176
   plots, indgen(24), n_wide050[*,j,4], psym=sym(6), symsize=1, color=0
   plots, indgen(24), n_wide050[*,j,5], psym=sym(1), symsize=1, color=84
   plots, indgen(24), n_wide050[*,j,5], psym=sym(6), symsize=1, color=0
   
;  Make a key
   plots, 1, 3, psym=sym(3), color=254
   plots, 1, 3, psym=sym(8), color=0
   me = string(total(n_wide050[*,j,0],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,0]),format='(i4)')
   mn = string(min(n_wide050[*,j,0]),format='(i4)')
   xyouts, 1.25, 2.8, 'Full:            total/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, 2.4, psym=sym(4), color=254
   plots, 1, 2.4, psym=sym(9), color=0
   me = string(total(n_wide050[*,j,6],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,6]),format='(i4)')
   mn = string(min(n_wide050[*,j,6]),format='(i4)')
   xyouts, 1.25, 2.3, 'Sun-synch: total/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, 2, psym=sym(1), color=254
   plots, 1, 2, psym=sym(6), color=0
   me = string(total(n_wide050[*,j,1],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,1]),format='(i4)')
   mn = string(min(n_wide050[*,j,1]),format='(i4)')
   xyouts, 1.25, 1.9, '65!Eo!N:             total/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, 1.6, psym=sym(1), color=208
   plots, 1, 1.6, psym=sym(6), color=0
   me = string(total(n_wide050[*,j,2],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,2]),format='(i4)')
   mn = string(min(n_wide050[*,j,2]),format='(i4)')
   ...
   me = string(total(n_wide050[*,j,3],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,3]),format='(i4)')
   mn = string(min(n_wide050[*,j,3]),format='(i4)')
   xyouts, 10.25, 2.3, '55!Eo!N:             total/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 10, 2, psym=sym(1), color=176
   plots, 10, 2, psym=sym(6), color=0
   me = string(total(n_wide050[*,j,4],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,4]),format='(i4)')
   mn = string(min(n_wide050[*,j,4]),format='(i4)')
   ...
   me = string(total(n_wide050[*,j,5],/nan),format='(i4)')
   mx = string(max(n_wide050[*,j,5]),format='(i4)')
   mn = string(min(n_wide050[*,j,5]),format='(i4)')
...
;  plot the number
   ...
   loadct, 39
   plots, indgen(24), d_wide050[*,j,0], psym=sym(3), symsize=2, color=254
   plots, indgen(24), d_wide050[*,j,0], psym=sym(8), symsize=2, color=0
   plots, indgen(24), d_wide050[*,j,6], psym=sym(4), symsize=2, color=254
   plots, indgen(24), d_wide050[*,j,6], psym=sym(9), symsize=2, color=0
   plots, indgen(24), d_wide050[*,j,1], psym=sym(1), symsize=2, color=254
   plots, indgen(24), d_wide050[*,j,1], psym=sym(6), symsize=2, color=0
   plots, indgen(24), d_wide050[*,j,2], psym=sym(1), symsize=1, color=208
   plots, indgen(24), d_wide050[*,j,2], psym=sym(6), symsize=1, color=0
   plots, indgen(24), d_wide050[*,j,3], psym=sym(1), symsize=1, color=192
   plots, indgen(24), d_wide050[*,j,3], psym=sym(6), symsize=1, color=0
   plots, indgen(24), d_wide050[*,j,4], psym=sym(1), symsize=1, color=176
   plots, indgen(24), d_wide050[*,j,4], psym=sym(6), symsize=1, color=0
   plots, indgen(24), d_wide050[*,j,5], psym=sym(1), symsize=1, color=84
   plots, indgen(24), d_wide050[*,j,5], psym=sym(6), symsize=1, color=0

;  Make a key
   plots, 1, .2, psym=sym(3), color=254
   plots, 1, .2, psym=sym(8), color=0
   me = string(mean(d_wide050[*,j,0],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,0]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,0]),format='(f4.2)')
   xyouts, 1.25, .19, 'Full:            mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, .16, psym=sym(4), color=254
   plots, 1, .16, psym=sym(9), color=0
   me = string(mean(d_wide050[*,j,6],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,6]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,6]),format='(f4.2)')
   xyouts, 1.25, .15, 'Sun-synch: mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, .12, psym=sym(1), color=254
   plots, 1, .12, psym=sym(6), color=0
   me = string(mean(d_wide050[*,j,1],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,1]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,1]),format='(f4.2)')
   xyouts, 1.25, .11, '65!Eo!N:             mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 1, .08, psym=sym(1), color=208
   plots, 1, .08, psym=sym(6), color=0
   me = string(mean(d_wide050[*,j,2],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,2]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,2]),format='(f4.2)')
   xyouts, 1.25, .07, '60!Eo!N:             mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 10, .16, psym=sym(1), color=192
   plots, 10, .16, psym=sym(6), color=0
   me = string(mean(d_wide050[*,j,3],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,3]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,3]),format='(f4.2)')
   xyouts, 10.25, .15, '55!Eo!N:             mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 10, .12, psym=sym(1), color=176
   plots, 10, .12, psym=sym(6), color=0
   me = string(mean(d_wide050[*,j,4],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,4]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,4]),format='(f4.2)')
   xyouts, 10.25, .11, '50!Eo!N:             mean/min/max = '+me+'/'+mn+'/'+mx, charsize=.75

   plots, 10, .08, psym=sym(1), color=84
   plots, 10, .08, psym=sym(6), color=0
   me = string(mean(d_wide050[*,j,5],/nan),format='(f4.2)')
   mx = string(max(d_wide050[*,j,5]),format='(f4.2)')
   mn = string(min(d_wide050[*,j,5]),format='(f4.2)')
