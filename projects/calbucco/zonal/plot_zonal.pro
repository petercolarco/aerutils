  ...
  for i = 0, nf-1 do begin
   ...
   su = reform(total(su,1)/n_elements(lon))
   a = where(su gt 1e12)
   su[a] = !values.f_nan

   nc4readvar, filename[i], 'duconc', du, lon=lon, lat=lat, lev=lev
   du = reform(total(du,1)/n_elements(lon))
   a = where(du gt 1e12)
   du[a] = !values.f_nan

   set_plot, 'ps'
   device, file='zonal.'+expid+'.'+strmid(filename[i],17,14,/rev)+'.ps', $
   ...
   contour, su, lat, lev, /nodata, $
    xrange=[-60,-15], xstyle=9, xtitle='Latitude', $
    yrange=[1000,10], ystyle=9, /ylog, ytitle='Pressure [hPa]', $
    thick=3, position=[.15,.25,.95,.9], xticks=9
...
;               xrange=[-60,-15], yrange=[1000,10], /ylog)
   ...
   contour, /overplot, du*1e9, lat, lev, levels=levelsd, c_colors=7, c_thick=3
