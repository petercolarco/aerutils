...
; variables, coordinates, etc.
  ...
  pro plot_map, varval, lon, lat, dx, dy, levels, colors, position, mapcolor, $
                ...
                varsum=varsum, varscale=varscale, contour=contour
  ...
  if(keyword_set(contour)) then begin
    contour, /overplot, varval, lon, lat, levels=levels, c_colors=colors, /fill
  endif else begin
...
; If a title is provided write it and value at top of map
  if(keyword_set(title)) then begin
   ...
   xyouts, position[0]+diff/10, position[3]-.06, title, color=textcolor, $
           /normal, align=0, charsize=1
   xyouts, position[2]-diff/10, position[3]-.06, mval, color=textcolor, $
           /normal, charsize=1, align=1
  endif
