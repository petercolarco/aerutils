...
; various model run optics versions
  filen = 'c180R_NoT_I32_pyroCB_pt3smoke.v1_5.aaod_Nc.20170815_1800z.nc4'
  vars  = ['brcphobic','brcphilic']
  nc4readvar, filen, vars, aaod, lon=lon, lat=lat, lev=lev, /sum
  aaod = aaod[*,*,1]

  filen = 'c180R_NoT_I32_pyroCB_pt3smoke.v1_5.taod_Nc.20170815_1800z.nc4'
  vars  = ['brcphobic','brcphilic']
  nc4readvar, filen, vars, aod, lon=lon, lat=lat, lev=lev, /sum
  aod = aod[*,*,1]
  a = where(aod lt 0.05)
  aod = (aod-aaod)/aod ; ssa
  aod[a] = !values.f_nan

  set_plot, 'ps'
  device, file='ssa.nobc.388nm.ps', xsize=16, ysize=12, xoff=.5, yoff=.5, /color, /helvetica
  !p.font=0
  loadct, 0

  levels = findgen(11)*.02+.8
