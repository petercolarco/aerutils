  ...
  nc4readvar, filename, 'so2', so2v3, lon=lon, lat=lat, lev=lev, time=time, wantlev=[level]
  nc4readvar, filename, 'airdens', rhoa, lon=lon, lat=lat, lev=lev, time=time, wantlev=[level]

  filetemplate = 'v4.tavg3d_aer_p.ddf'
  ga_times, filetemplate, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
  nc4readvar, filename, 'so2', so2v4, lon=lon, lat=lat, lev=lev, time=time, wantlev=[level]
  ...
  set_plot, 'ps'
  device, file='so2conc_omi.'+nymd[i]+'.'+levels+'hpa.ps', /color, /helvetica, $
