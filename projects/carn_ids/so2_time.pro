  ...
  nc4readvar, filename, 'so2cmass', so2v3_, lon=lon, lat=lat, lev=lev, time=time
  
  filetemplate = 'v4.ddf'
  ga_times, filetemplate, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
  nc4readvar, filename, 'so2cmass', so2v4_, lon=lon, lat=lat, lev=lev, time=time

  
  filetemplate = 'v5.ddf'
  ga_times, filetemplate, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
  nc4readvar, filename, 'so2cmass', so2v5_, lon=lon, lat=lat, lev=lev, time=time

  
  filetemplate = 'v7.ddf'
  ga_times, filetemplate, nymd, nhms, template=template
  filename=strtemplate(template,nymd,nhms)
  nc4readvar, filename, 'so2cmass', so2v7_, lon=lon, lat=lat, lev=lev, time=time
...
; Find the global mean value
  ...
  oplot, indgen(nt), (so2v5-min(so2v3)), thick=6, color=176 ; Tg
  oplot, indgen(nt), (so2v7-min(so2v3)), thick=6, color=254 ; Tg
