...
# Copy OMI L2 file locally and stuff it with model produced radiances
  ...
  foreach file (`\ls -1 /misc/prc19/colarco/OMAERUV_V187_DATA/2011/OMI-Aura_L2-OMAERUV_2011m0701t1[2,3,4,5,6,7,8,9]*he5`)
   set file_ = $file:t:r.vl_rad.geos5_pressure.he5
   /bin/cp -f $file $file_
...
data = np.load('radiance.2011_a2_apport_frland_7_aer_L2-${fileend}Full.npz')
x = data['rad_VL'].shape
x = x[0]/60
v[:,:,:] = np.reshape(data['rad_VL'],(x,60,3))

# Now get and stuff the "uncorrected" AI (called Residue)
data = np.load('ai.2011_a2_apport_frland_7_aer_L2-${fileend}Full.npz')
g = f.get('HDFEOS/SWATHS/Aerosol NearUV Swath/Data Fields')
v = g.get('Residue')
...
data3 = np.load('../adriana/2011_a2_apport_frland_7_aer_L2-${fileend}Full.npz')
