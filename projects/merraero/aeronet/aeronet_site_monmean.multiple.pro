...
; Get a desired list of aeronet sites
  read_aeronet_locs, locSessions, lat, lon, elevation, database='aeronet_locs.sessions.dat', /rem
...
; Create a plot file
  ...
  device, file=filename, color=color, /helvetica, font_size=8, $
          xoff=.5, yoff=.5, xsize=24, ysize=17
  !p.font=0


; position the plots
  position = fltarr(4,9)
  position[*,0] = [.05,.72,.45,.95]
  position[*,1] = [.5,.72,.7,.95]
  position[*,2] = [.75,.72,.95,.95]

  position[*,3] = [.05,.4,.45,.63]
  position[*,4] = [.5,.4,.7,.63]
  position[*,5] = [.75,.4,.95,.63]

  position[*,6] = [.05,.08,.45,.31]
  position[*,7] = [.5,.08,.7,.31]
  position[*,8] = [.75,.08,.95,.31]
...
;  Check locations is in desired list of sites
   a = where(location[iloc] eq locSessions)
...
; Make a map
  ...
  map_set, p0, p1, position=[.1,.3,.9,.9], /noborder, limit=geolimits, $
   /hammer, /iso, /horizon, /grid, glinestyle=0, color=160, glinethick=.5
  xyouts, 0.5, .93, 'AOT Bias', /normal, align=0.5
  map_continents, thick=1
  map_continents, /countries
  incnum = 1
  loadct, 39, /silent
...
; Plot the key
  ...
  plots, .2, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .2, .3, psym=8, /normal
  xyouts, .2, .26, 'b< -0.1', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], /fill, color=80    ;color=0
  plots, .35, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .35, .3, psym=8, /normal
  xyouts, .35, .26, '-0.1 < b < -0.025', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], /fill, color=255        ;color=120
  plots, .5, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], color=0
  plots, .5, .3, psym=8, /normal
  xyouts, .5, .26, '-0.025 < b < 0.025', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=208   ;color=200
  plots,  .65, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .65, .3, psym=8, /normal
  xyouts, .65, .26, '0.025 < b < 0.1', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=254   ;color=255
  plots,  .8, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .8, .3, psym=8, /normal
  xyouts, .8, .26, 'b > 0.1', align=.5, charsize=.8, /normal
...
; Make a map
  ...
  map_set, p0, p1, position=[.1,.3,.9,.9], /noborder, limit=geolimits, $
   /hammer, /iso, /horizon, /grid, glinestyle=0, color=160, glinethick=.5
  xyouts, 0.5, .93, 'Angstrom Parameter Bias', /normal, align=0.5
  map_continents, thick=1
  map_continents, /countries
  incnum = 1
  loadct, 39, /silent
...
; Plot the key
  ...
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .2, .3, psym=8, /normal
  xyouts, .2, .26, 'b< -0.3', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], /fill, color=80    ;color=0
  plots, .35, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .35, .3, psym=8, /normal
  xyouts, .35, .26, '-0.3 < b < -0.1', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], /fill, color=255        ;color=120
  plots, .5, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], color=0
  plots, .5, .3, psym=8, /normal
  xyouts, .5, .26, '-0.1 < b < 0.1', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=208   ;color=200
  plots,  .65, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .65, .3, psym=8, /normal
  xyouts, .65, .26, '0.1 < b < 0.3', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=254   ;color=255
  plots,  .8, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .8, .3, psym=8, /normal
  xyouts, .8, .26, 'b > 0.3', align=.5, charsize=.8, /normal
...
; Make a map
  ...
  map_set, p0, p1, position=[.1,.3,.9,.9], /noborder, limit=geolimits, $
   /hammer, /iso, /horizon, /grid, glinestyle=0, color=160, glinethick=.5
  xyouts, 0.5, .93, 'Absorption AOT Bias', /normal, align=0.5
  map_continents, thick=1
  map_continents, /countries
  incnum = 1
  loadct, 39, /silent
...
; Plot the key
  ...
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .2, .3, psym=8, /normal
  xyouts, .2, .26, 'b< -0.025', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], /fill, color=80    ;color=0
  plots, .35, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[1,1,-1,-2,-1,1], color=0
  plots, .35, .3, psym=8, /normal
  xyouts, .35, .26, '-0.025 < b < -0.005', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], /fill, color=255        ;color=120
  plots, .5, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,-1,-1],2.*[1,1,-1,-1,1], color=0
  plots, .5, .3, psym=8, /normal
  xyouts, .5, .26, '-0.005 < b < 0.005', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=208   ;color=200
  plots,  .65, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .65, .3, psym=8, /normal
  xyouts, .65, .26, '0.005 < b < 0.025', align=.5, charsize=.8, /normal

  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=254   ;color=255
  plots,  .8, .3, psym=8, /normal
  usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], color=0
  plots,  .8, .3, psym=8, /normal
  xyouts, .8, .26, 'b > 0.025', align=.5, charsize=.8, /normal
