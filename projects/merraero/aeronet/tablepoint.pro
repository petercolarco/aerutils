pro tablepoint, incnum, location, latitude, longitude, elevation, piname, stattable, statelem=statelem
   ...
   print, '('+strcompress(string(incnum),/rem)+')' $
          ...
          stattable[4,2], format='(a-90,4x,i3,6(a1,2x,f6.2),a2,4x,i6,3(a3,2x,f10.4))'
...
;  AOT
   ...
   d = convert_coord(longitude,latitude, /data, /to_normal)
   x = d[0]
   y = d[1]

   if(ic gt 2) then begin
    usersym, 2.*[-1,1,1,0,-1,-1],2.*[-1,-1,1,2,1,-1], /fill, color=colorarray[ic]
    plots, x, y, psym=8, noclip=0, /normal
   ...
   xyouts, x, y-.005, strcompress(string(incnum),/rem), $
           align=.5, clip=[0,0,1,1], charsize=.8, color=colorlabel[ic], /normal
