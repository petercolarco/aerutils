...
# Time loop
  foreach YYYY ( $years )
   ...
   # Create the DDF files for the data set
     ...
     set starttime  = 0z$DD$month[$MM]$YYYY
     set middletime = 12z$DD$month[$MM]$YYYY 
     set tdefstr = "tdef time "$ntime" linear "$starttime" 1hr"
     set tdefstrsat = "tdef time "$ntimesat" linear "$starttime" 1hr"
     echo $tdefstr

   # Create a time extraction string for the subsetting
     set timestr = "00z"$DD$month[$MM]$YYYY" 23z"$DD$month[$MM]$YYYY" 1"
...
# Satellite data
  ...
  set datadir   = /science/modis/data/Level3/$satid/hourly/$resolution/GRITAS/
...
EOF
   ...
   # Pass subset to extraction
     lats4d.sh -v -i tmp_aer.${rand}.nc -j qafl.${rand}.ddf -o tmp_qawt.${rand} \
                     -func "ave(maskout(@.1,aot.2(z=1)),t=1,t=$ntimesat)" \
   ...
   # clean-up
     ...
     mkdir -p $expdir/Y$YYYY/M$MM/
     mv -f tmp_qawt.${rand}.nc4 \
           $expdir/Y$YYYY/M$MM/$expstr.$datatyp.$sample.$YYYY$MM$DD.nc4
