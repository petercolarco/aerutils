...
; Get SS450 all month PDF
  filen = file_search('../'+satid+'/','*polar*txt')
...
; Get the random distribution pdf
  file = '../'+satid+'/'+satid+'.random.2006'+mm[im]+'.txt'
...
; Make some plots
  set_plot, 'ps'
  device, file='compare_distribution.'+satid+'.2006'+mm[im]+'.ps', /helvetica, font_size=14, /color, $
   xsize=24, ysize=36
  !p.font=0
  !p.multi=[0,2,3]
  loadct, 39
