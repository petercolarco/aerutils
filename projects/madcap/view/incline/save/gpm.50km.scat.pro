...
b = where(scat ge 170.)
nthresh = nthresh + n_elements(b)
ntotal  = ntotal + n_elements(scat)
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm.50km.scat.max.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm.50km.scat.min.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm.50km.scat.diff.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Print the fraction of time scat > 170
  ...
  openw, lun, 'gpm.50km.scat.max_freq.txt', /get
