...
; Get the filenames to probe...
  ...
  for imm = 1, 12 do begin
   for idd = 1, 31 do begin
    mm = strpad(imm,10)
    dd = strpad(idd,10)
    filen = file_search('/misc/prc19/colarco/OBS/A-CCP/GPM050/LevelB/Y2019/M'+mm+'/',$
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm050.100km.scat.max.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm050.100km.scat.ge165.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
  ...
  plot, indgen(10), /nodata, $
   ...
   xtickn = [string(xtickv[0:7],format='(i3)'),' '], $
   yticks=8, ytickn=[' ','-60','-40','-20','0','20','40','60',' '], $
   title='Precessing, Occurrence of Maximum Scattering Angle > 165!Eo!N', $
   xtitle='Day of Year', ytitle='Latitude'
  levels = 80+findgen(10)*10
  dx = 1.
  dy = 1.
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm050.100km.scat.min.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Make a plot
  set_plot, 'ps'
  device, file='gpm050.100km.scat.diff.ps', /color, /helvetica, font_size=12, $
   xsize=30, ysize=18
  !p.font=0
...
; Print the fraction of time scat > 165
  ...
  openw, lun, 'gpm050.100km.scat.max_freq.txt', /get
