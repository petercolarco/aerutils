...
jump:
  ...
  set_plot, 'ps'
  device, file='gpm050.nadir.lat_x_time.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
...
; histogram
  set_plot, 'ps'
  device, file='gpm050.nadir.lat_x_time_hist.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
...
; count
  for ij = 0, nj-1 do begin
   a = where(finite(eqt[ij,*]) ne 1)
   if(a[0] ne -1) then hist[0,ij] = n_elements(a)
   for ilev = 1, n_elements(hlev)-2 do begin
  ...
  plot, indgen(10), /nodata, $
   ...
   xticks=n_elements(hlev), xtickn=[' ','N',string(levels,format='(i2)'),' '], $
   position=[.1,.15,.8,.9]
  loadct, 39
