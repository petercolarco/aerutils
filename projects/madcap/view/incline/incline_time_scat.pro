; Merge the created scattering angle field with the local time of day
; of observations above thresholds

  pro incline_time_scat, inc, swath
  sinc = '0'+inc
  outstr = 'gpm'+sinc+'.'+swath+'km'
...
; Outputs
  ...
  eqt = make_array(nj,365,val=!values.f_nan)
  szo = make_array(nj,365,val=!values.f_nan)
  sct = make_array(nj,365,val=!values.f_nan)
  loo = make_array(nj,365,val=!values.f_nan)
  lao = make_array(nj,365,val=!values.f_nan)
  fdy = fltarr(365)
...
; reduce to nearest points in a 5 degree latitude grid
  ...
  if(min(abs(lat-latg[j])) gt .49) then goto, jump
...
; Get the previous max scattering angle
  restore, outstr+'.sav'


  scatmxo = transpose(scatmxo)
  a = where(scatmxo lt 155)
  eqt[a] = !values.f_nan

  set_plot, 'ps'
  device, file=sinc+'.'+swath+'km.lat_x_time.155.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
  ...
  plotgrid, transpose(eqt), levels, colors, indgen(365)+1, latg, 1, 1
  loadct, 0
...
; histogram
  set_plot, 'ps'
  device, file=sinc+'.'+swath+'km.lat_x_time_hist.155.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
...
;   if(a[0] ne -1) then hist[0,ij] = n_elements(a)
  ...
  a = where(scatmxo lt 165)
  eqt[a] = !values.f_nan

  set_plot, 'ps'
  device, file=sinc+'.'+swath+'km.lat_x_time.165.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
...
; histogram
  set_plot, 'ps'
  device, file=sinc+'.'+swath+'km.lat_x_time_hist.165.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=14
  !p.font=0
