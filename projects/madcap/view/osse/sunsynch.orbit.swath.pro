...
; Reduce to swath edges and center
  lon_  = transpose(reform(lon_[[0,498,995],iy,*]))
  lat_  = transpose(reform(lat_[[0,498,995],iy,*]))
  vza_  = transpose(reform(vza_[[0,498,995],iy,*]))
  sza_  = transpose(reform(sza_[[0,498,995],iy,*]))
  saa_  = transpose(reform(saa_[[0,498,995],iy,*]))
  scat_ = transpose(reform(scat_[[0,498,995],iy,*]))
...
; parts
  set_plot, 'ps'
  device, file='sunsynch.orbit.swath.ps', /helvetica, font_size=14, $
  ...
  device, file='sunsynch.orbit2.swath.ps', /helvetica, font_size=14, $
  ...
  device, file='sunsynch.sza.swath.ps', /helvetica, font_size=14, $
  ...
  device, file='sunsynch.vza.swath.ps', /helvetica, font_size=14, $
   xoff=.5, yoff=.5, xsize=16, ysize=16, /color
  loadct, 39
  !p.font=0
  plot, indgen(100), /nodata, $
  ...
  device, file='sunsynch.scat.swath.ps', /helvetica, font_size=14, $
   xoff=.5, yoff=.5, xsize=16, ysize=16, /color
  loadct, 39
  !p.font=0
  plot, indgen(100), /nodata, $
