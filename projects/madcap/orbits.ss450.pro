...
; make histogram of equator crossing time
  ...
  ttl_hist = lonarr(48)
  for j = 0, 47 do begin
   b = where(ttl[a] ge j/2. and ttl[a] lt (j+1)/2.)
   if(b[0] ne -1) then ttl_hist[j] = n_elements(b)
  endfor

; Make a plot
  set_plot, 'ps'
  device, file='orbits.ss450.'+tle[i]+'.ps', /color, /helvetica, $
  ...
  oplot, findgen(48)/2., ttl_hist, thick=3
  plots, 1.5,[0,10000], lin=2
  plots, 13.5, [0,10000], lin=2
...
; Plot the first day orbit
  ...
  if(tle[i] eq 'sunsynch_500km.nodrag') then begin
   lon_ = lon[0:20*24*60]
   lat_ = lat[0:20*24*60]
   a = where(lat_ gt -1 and lat_ lt 1)
