...
from pyobs.sgp4     import getSwath as getTrackTLE
...
#   --------------------------
    ...
    parser.add_option("-d", "--dt_secs", dest="dt_secs", default=dt_secs,
              type='int',
              help="Timestep in seconds for TLE sampling (default=%s)"%dt_secs )

    parser.add_option("-w", "--width", dest="swathwidth", default=swathwidth,
              type='float',
              help="Swath width [km] centered on nadir point from TLE file (default=%s)"%swathwidth )
    ...
    if options.traj == 'TLE':
        ...
        lon, lat, tyme = getTrackTLE(trjFile, t1, t2, options.dt_secs, options.swathwidth)
