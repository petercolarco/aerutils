  pro get_stat, ddf, res, wantlon=wantlon, wantlat=wantlat, $
                nt0=nt0_, nt1=nt1_, aotm=aotm, aotn=aotn, cldn80=cldn80, lon=lon, lat=lat
  ...
  nc4readvar, filename, ['cldtot'], aot, wantlat=wantlat, wantlon=wantlon, lon=lon, lat=lat
...
; Get the number of samples
  ...
  for ix=0, nx-1 do begin
   for iy=0, ny-1 do begin
    ...
    n = where(finite(aot_[ix,iy,*]) eq 1 and aot_[ix,iy,*] ge .8)
    if(n[0] ne -1) then cldn80[ix,iy] = n_elements(n)
   endfor
  endfor
