...
; Make some plots
  set_plot, 'ps'
  device, file='swt_timeseries_swath.dual.day.c1440.ps', /helvetica, font_size=12, /color
  !p.font=0
  ...
  loadct, 57  ; blues
  for i = 0, 4 do begin
   oplot, indgen(12)+1, taug[*,i,2], thick=6, color=colors[i]
  endfor
  loadct, 0
...
; Difference in global mean
  device, file='swt_timeseries_swath.dual.day.global_diff.c1440.ps', /helvetica, font_size=12, /color
  !p.font=0
...
; Difference in global mean -- alternative
  device, file='swt_timeseries_swath.dual.day.global_diff2.c1440.ps', /helvetica, font_size=12, /color
  !p.font=0
...
; print a table of mean difference
  diff = fltarr(nswa,ntle)
  for j = 0, ntle-1 do begin
   for i = 0, nswa-1 do begin
    diff[i,j] = mean(taug[*,i,j]-taum)
   endfor
   print, diff[*,j], format="(5(f7.3,3x))"
  endfor
