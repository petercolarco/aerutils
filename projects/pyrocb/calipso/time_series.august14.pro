  ...
  cdfid = ncdf_open('calipso_august14.nc')
  ...
  cdfid = ncdf_open('calipso_ze_august14.nc')
...
; Make a plot
  set_plot, 'ps'
  device, file=fileout, /color, font_size=16, /helvetica, $
   xsize=36, ysize=18
  !p.font=0  
  loadct, 0
...
;  xyouts, -60, 250, 'Altitude [km]', orient=90, /data, chars=1.5, align=.5
  xyouts, 1900-75, 250, 'Altitude [km]', orient=90, /data, chars=1.5, align=.5
  loadct, 56
...
;   xrange=[0,1300], xstyle=1, xticks=5, xtickn=xtickn, $
  ...
  plots, [.15,.25], .7, thick=8, lin=2, /normal
  xyouts, .26, .69, 'Tropopause Height', /normal
...
;   xyouts, -45, 1.01*ple[1,k]/100., string(ze[0,k],format='(f6.1)'), /data, noclip=1, chars=1.1
   xyouts, 1900-68, 1.01*ple[1,k]/100., string(ze[0,k],format='(f6.1)'), /data, noclip=1, chars=1.1
  endfor

  loadct, 0
  makekey, .15, .87, .5, .05, 0., -0.04, $
   color=make_array(12,val=0), label=string(levels*1000,format='(i4)'), align=0
  xyouts, .325, .79, 'Extinction [Mm!E-1!N]', /normal
  loadct, 56
  makekey, .15, .87, .5, .05, 0., -0.04, color=indgen(12)*20+10, label=make_array(n_elements(levels),val=' ')
