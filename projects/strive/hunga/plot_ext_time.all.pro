  ...
  lev = 1
  for i = 1, 8 do begin
   filename = 'C90c_HTerupV02a.tavg24_3d_aer_Np.argos'+string(i,format='(i1)')+'.trj.nc'
   read_argos, filename, lon_, lat_, time_, sza_, $
               ext=ext_, lev=lev
; restrict 20 km
  a = where(lev eq 50.)
  ext_ = reform(ext_[a[0],*])


;  Retain only the first day
   if(i eq 1) then begin
    a = where(strmid(time_,5,2) lt '04')
    ext = ext_[a]
    lon = lon_[a]
    lat = lat_[a]
    sza = sza_[a]
...
; Make a plot
  set_plot, 'ps'
  device, file = 'plot_ext_time.all.ps', /color, /helvetica, font_size=14, $
   xsize=24, ysize=12
  !p.font=0
