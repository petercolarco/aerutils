...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_oc_all_diff.ps', /color, /helvetica, font_size=16, $
...
; Baseline
  ...
  nc4readvar, datafil, 'ocexttau', duem, lon=lon, lat=lat, lev=lev, /sum
...
; Get the b-resolution
  ...
  nc4readvar, datafil, 'ocexttau', duem_b, lon=lon, lat=lat, lev=lev, /sum
  duem_base_b = duem_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, 'ocexttau', duem_c, lon=lon, lat=lat, lev=lev, /sum, /tem
  duem_base_c = duem_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], 'ocexttau', duem, lon=lon, lat=lat, lev=lev, /sum
   area, lon, lat, nx, ny, dx, dy, area
