...
; If a title is provided write it and value at top of map
  if(keyword_set(title)) then begin
   ...
   if(not(keyword_set(format))) then format='(f6.3)'
   ...
   xyouts, position[0]+diff/2, position[3]-.06, title, color=textcolor, $
           /normal, align=.5, charsize=1
   xyouts, position[2], position[3]-.1, mval, color=textcolor, $
