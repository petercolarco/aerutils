...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_suexttau_all_diff.ps', /color, /helvetica, font_size=16, $
...
; Baseline
  ...
  nc4readvar, datafil, 'suexttau', duem, lon=lon, lat=lat, lev=lev, /sum
  ...
  makekey, .02, .6, .3, .03, 0, -.04, align=0, charsize=.6, $
           color=make_array(7,val=0), label=['0.01','0.02','0.04','0.07','0.1','0.2','0.4']
  tvlct, red, green, blue
...
; Get the b-resolution
  ...
  nc4readvar, datafil, 'suexttau', duem_b, lon=lon, lat=lat, lev=lev, /sum
  duem_base_b = duem_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, 'suexttau', duem_c, lon=lon, lat=lat, lev=lev, /sum, /tem
  duem_base_c = duem_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], 'suexttau', duem, lon=lon, lat=lat, lev=lev, /sum
   area, lon, lat, nx, ny, dx, dy, area
