...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_zonal_supso4g.ps', /color, /helvetica, font_size=16, $
...
;  seasons = ['M01','M02','M03','M04']
  ...
  for iseas = 0, 3 do begin
   ...
   plot, findgen(10), /nodata, thick=3, /noerase, $
         ...
         yrange=[0,.4], ytitle='Production [Tg mon!E-1!N]', yticks=4, ystyle=9, $
   ...
   for iexpid = 0, nexpid-1 do begin
    ...
    nc4readvar, datafil, ['supso4g'], pso4, lon=lon, lat=lat, lev=lev, /sum
...
print, total(pso4)
    oplot, lat, pso4, thick=6, color=color[iexpid], lin=lins[iexpid]
   endfor
   xyouts, -85, .18*.4/.2, seasons[iseas]
  endfor
