...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_u10_all_diff.ps', /color, /helvetica, font_size=16, $
...
; Baseline
  ...
  nc4readvar, datafil, ['u10m','v10m'], u10m, lon=lon, lat=lat, lev=lev
  u10m = sqrt(u10m[*,*,0]^2+u10m[*,*,1]^2)
  area, lon, lat, nx, ny, dx, dy, area
  title = titles[iplot]
  plot_map, u10m, lon, lat, dx, dy, levels, dcolors, $
...
; DIFFERENCE PLOT
  ...
  makekey, .25, .07, .5, .03, 0, -.04, align=.5, charsize=.6, $
           color=make_array(13,val=0), label=[' ','-3','-2','-1','-0.5','-0.3','-0.2','-0.1','0.1','0.2','0.3','0.5','1']
  ...
  tvlct, red, green, blue
  levels = [-2000,-3,-2,-1,-.5,-0.3,-0.2,-.1,.1,0.2,0.3,.5,1]
  dcolors=indgen(n_elements(levels))
  igrey  = n_elements(red)-2
  iblack = n_elements(red)-1
...
; Get the b-resolution
  ...
  nc4readvar, datafil, ['u10m','v10m'], u10m_b, lon=lon, lat=lat, lev=lev
  u10m_b = sqrt(u10m_b[*,*,0]^2+u10m_b[*,*,1]^2)
  u10m_base_b = u10m_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, ['u10m','v10m'], u10m_c, lon=lon, lat=lat, lev=lev
  u10m_c = sqrt(u10m_c[*,*,0]^2+u10m_c[*,*,1]^2)
  u10m_base_c = u10m_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], ['u10m','v10m'], u10m, lon=lon, lat=lat, lev=lev
   u10m = sqrt(u10m[*,*,0]^2+u10m[*,*,1]^2)
   area, lon, lat, nx, ny, dx, dy, area
   if(nx eq 576) then begin
    ...
    if(nx eq 288) then begin
     u10m = u10m-u10m_base_c
    endif else begin
     u10m = u10m-u10m_base_b
    endelse
   endelse
   title = titles[iplot]
   plot_map, u10m, lon, lat, dx, dy, levels, dcolors, $
