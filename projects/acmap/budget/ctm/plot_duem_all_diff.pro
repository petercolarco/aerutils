...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_duem_all_diff.ps', /color, /helvetica, font_size=16, $
...
; Baseline
  ...
  duem = duem*365.*86400.   ; kg m-2 yr-1
  ...
  makekey, .02, .6, .3, .03, 0, -.04, align=0, charsize=.6, $
           color=make_array(7,val=0), label=['0.01','0.02','0.05','0.1','0.2','0.3','0.5']
...
; DIFFERENCE PLOT
  ...
  tvlct, red, green, blue
  levels = [-2000,-.1,-.05,-0.02,-0.01,-.005,.005,0.01,0.02,.1,.2]
  dcolors=indgen(n_elements(levels))
  igrey  = n_elements(red)-2
  iblack = n_elements(red)-1
...
; Get the b-resolution
  ...
  nc4readvar, datafil, 'duem', duem_b, lon=lon, lat=lat, lev=lev, /sum, /tem
  duem_b = duem_b*365.*86400.
  duem_base_b = duem_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, 'duem', duem_c, lon=lon, lat=lat, lev=lev, /sum, /tem
  duem_c = duem_c*365.*86400.
  duem_base_c = duem_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], 'duem', duem, lon=lon, lat=lat, lev=lev, /sum, /tem
   duem = duem*365.*86400.   ; kg m-2 yr-1
   area, lon, lat, nx, ny, dx, dy, area
