...
; Setup the plot
  set_plot, 'ps'
  device, file='plot_ssem_all_diff.ps', /color, /helvetica, font_size=16, $
...
; Baseline
  ...
  nc4readvar, datafil, 'ssem', ssem, lon=lon, lat=lat, lev=lev, /sum, /tem
  ssem = ssem*365.*86400.   ; kg m-2 yr-1
  area, lon, lat, nx, ny, dx, dy, area
  title = titles[iplot]
  plot_map, ssem, lon, lat, dx, dy, levels, dcolors, $
            position[*,iplot], iblack, p0=p0, p1=p1, limits=geolimits, $
            textcolor=iblack, title=title, format='(i5)', $
...
; DIFFERENCE PLOT
  ...
  makekey, .25, .07, .5, .03, 0, -.04, align=.5, charsize=.6, $
           ...
           label=[' ','-0.01','-0.005','-0.003','-0.002','-0.001','0.001','0.002','0.003','0.005','0.01']
...
; Get the b-resolution
  ...
  nc4readvar, datafil, 'ssem', ssem_b, lon=lon, lat=lat, lev=lev, /sum, /tem
  ssem_b = ssem_b*365.*86400.
  ssem_base_b = ssem_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, 'ssem', ssem_c, lon=lon, lat=lat, lev=lev, /sum, /tem
  ssem_c = ssem_c*365.*86400.
  ssem_base_c = ssem_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], 'ssem', ssem, lon=lon, lat=lat, lev=lev, /sum, /tem
   ssem = ssem*365.*86400.   ; kg m-2 yr-1
   area, lon, lat, nx, ny, dx, dy, area
   if(nx eq 576) then begin
    ...
    if(nx eq 288) then begin
     ssem = ssem-ssem_base_c
    endif else begin
     ssem = ssem-ssem_base_b
    endelse
   endelse
   title = titles[iplot]
   plot_map, ssem, lon, lat, dx, dy, levels, dcolors, $
