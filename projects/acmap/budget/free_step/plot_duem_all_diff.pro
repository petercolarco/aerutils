...
; c180R_G40b11
  ...
  plotgrid, duem, levels, dcolors, lon, lat, dx, dy, /map
  ...
  title = 'c180R_G40b11'
  mval  = string(total(area*duem)*1.e-9,format='(f6.1)')+' Tg'
...
; c180F_G40b11
  ...
  nc4readvar, datafil, 'duem', duem, lon=lon, lat=lat, lev=lev, /tem, /sum
  duem = duem*365.*86400.   ; kg m-2 yr-1
  duem = duem-duem_base
  map_set, p0, p1, position=position[*,iplot], /noerase, /noborder, limit=geolimits, /robinson, /iso
  plotgrid, duem, levels, dcolors, lon, lat, dx, dy, /map
  ...
  area, lon, lat, nx, ny, dx, dy, area
  title = 'c180F_G40b11'
  mval  = string(total(area*duem)*1e-9,format='(f6.1)')+' Tg'
  xyouts, position[0,iplot], position[3,iplot]-.075, title, color=iblack, /normal, charsize=.6
  xyouts, position[2,iplot]-.16, position[3,iplot]-.075, mval, color=iblack, /normal, charsize=.6, align=.5 
...
; c48R_G40b11_step
  ...
  mval  = string(total(area*duem)*1e-9,format='(f6.1)')+' Tg'
  xyouts, position[0,iplot], position[3,iplot]-.075, title, color=iblack, /normal, charsize=.6
  xyouts, position[2,iplot]-.1, position[3,iplot]-.075, mval, color=iblack, /normal, charsize=.6, align=.5 
