...
; Baseline
  ...
  title = titles[iplot]
  plot_map, duwt, lon, lat, dx, dy, levels, dcolors, $
...
; Get the b-resolution
  ...
  nc4readvar, datafil, ['duwt','dusv'], duwt_b, lon=lon, lat=lat, lev=lev, /sum, /tem
  duwt_b = duwt_b*365.*86400.
  duwt_base_b = duwt_b
...
; Get the c-resolution
  ...
  nc4readvar, datafil, ['duwt','dusv'], duwt_c, lon=lon, lat=lat, lev=lev, /sum, /tem
  duwt_c = duwt_c*365.*86400.
  duwt_base_c = duwt_c

  for iplot = 1, 5 do begin

   nc4readvar, datafils[iplot], ['duwt','dusv'], duwt, lon=lon, lat=lat, lev=lev, /sum, /tem
   duwt = duwt*365.*86400.   ; kg m-2 yr-1
   area, lon, lat, nx, ny, dx, dy, area
   if(nx eq 576) then begin
    ...
    if(nx eq 288) then begin
     duwt = duwt-duwt_base_c
    endif else begin
     duwt = duwt-duwt_base_b
    endelse
   endelse
   title = titles[iplot]
   plot_map, duwt, lon, lat, dx, dy, levels, dcolors, $
