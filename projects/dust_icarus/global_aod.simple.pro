; Array for global AOD
  ...
  set_plot, 'ps'
  device, file='global_aod.simple.ps', /helvetica, font_size=12, /color, $
  ...
  for iexpid = 0, nexpid-1 do begin
   ...
   oplot, x, totexttau[*,iexpid], color=color[iexpid], thick=10, lin=lin[iexpid]
  endfor
  ...
  for iexpid = 0, nexpid-1 do begin
   if(iexpid lt 2) then loadct, 0
   if(iexpid ge 2) then loadct, 39
   oplot, x, duexttau[*,iexpid], color=color[iexpid], thick=10, lin=lin[iexpid]
  endfor
