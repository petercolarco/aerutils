...
; OPAC (full refractive indices)
  ...
  set_plot, 'ps'
  device, file='plot_ssa.zoom.bin01.ps', /helvetica, font_size=12, /color, $
   xoff=.5, yoff=.5, xsize=16, ysize=10
  !p.font=0

  loadct, 0
  plot, lambda*1d6, bsca[*,0,0]/bext[*,0,0], /nodata, $
  ...
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=16, color=100, lin=2
...
; MERRA-2 (truncated)
  ...
  loadct, 39
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8

; MERRA-2 (spheres, not truncated)
  filename = 'optics_DU.v15_6.spheres.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8, lin=2

; MERRA-2 (spheres, truncated)
  filename = 'optics_DU.v15_6.spheres.nc'
  readoptics, filedir+filename, reff, lambda, qext, qsca, bext, bsca, g, bbck, $
              rh, rmass, refreal, refimag
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8, lin=1
...
; Spher (new optics, truncated)
  ...
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8, color=254
...
; Mie (new optics, truncated)
  ...
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8, color=254, lin=1
...
; Mie (new optics)
  ...
  oplot, lambda*1e6, bsca[*,0,0]/bext[*,0,0], thick=8, color=254, lin=2
