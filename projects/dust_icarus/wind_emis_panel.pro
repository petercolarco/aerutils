...
; simulations
  ...
  set_plot, 'ps'
  device, file='wind_emis_panel.new_vs_old.eps', $
 ...
 !p.font=0
  ...
  if(iexp eq 0) then begin
   position = [ [.02,.68,.46,.93], $
                [.02,.37,.46,.62], $
                [.02,.06,.46,.31] ]
   title0      = '!4Kok - No Forcing!3'
  endif else begin
   position = [ [.54,.68,.98,.93], $
                [.54,.37,.98,.62], $
                [.54,.06,.98,.31] ]
   title0      = '!4Kok (Old) - No Forcing!3'
  endelse
...
;  var  = sqrt(v_u10*v_u10 + v_v10*v_v10)
  ...
  nc4readvar, filen, 'duem', emis, lon=lon, lat=lat, /tem, /sum
...
;  varb  = sqrt(v_u10b*v_u10b + v_v10b*v_v10b)
  filen = '/misc/prc18/colarco/'+expid1+'/tavg2d_aer_x/'+expid1+ $
          '.tavg2d_aer_x.monthly.200807.nc4'
  nc4readvar, filen, 'duem', emisb, lon=lon, lat=lat, /tem, /sum
