...
def plot(lev,varn='SUEXTCOEF',expid='C90c_HTerupV02a',ens=-1):
...
#    clevs = np.arange(0,10,.05)+.001
    clevs = 10**np.arange(-2,1,.02)
#    clevs = np.arange(0,5,.1)
#   Scale to 870 nm ad hoc
    print(np.min(np.transpose(exto)*1e7*0.32))
    cf = ax.contourf(daysx, latsx, np.transpose(exto)*1e7*0.32, clevs, cmap='Spectral_r', extend='both',norm=colors.LogNorm())
    #fig.colorbar(cf)
    ...
    print('plotting')
    if ens < 0:
        fig.savefig('%s_%02dkm_2022_2024.monthly.png'%(expid,lev))
    else:
        fig.savefig('C90c_HTerupV02ens_%02dkm_2022_2024.monthly.png'%(lev))
    print('done')
    plt.close(fig)
