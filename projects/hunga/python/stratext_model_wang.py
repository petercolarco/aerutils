...
def plot(lev,varn='SUEXTCOEF'):
    ...
    end   = datetime.datetime.strptime("31-10-2022", "%d-%m-%Y")
...
#   Scale to 870 nm ad hoc
    cf = ax.contourf(daysx, latsx, np.transpose(exto)*1e7*0.7, clevs, cmap='Spectral_r', extend='both')
    #fig.colorbar(cf)
    ...
    plt.tight_layout()
        #plt.show()
    print('plotting')
    fig.savefig('C90c_HTerupV03a_%02dkm_2012_2022.wang.png'%(lev))       
    print('done')
    plt.close(fig)
