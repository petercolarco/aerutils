...
; Write to a file
  openw, lun, 'so2_volcanic_emissions_TongaMIP_exp4.GMI_ht_only.20220115.rc', /get
  printf, lun, '###  LAT (-90,90), LON (-180,180), SULFUR [kg SO2/s], ELEVATION [m], CLOUD_COLUMN_HEIGHT [m]'
  printf, lun, 'SO2::'
...
;  sflux = so2flux/2.
  ...
  for iz = 0, nz-1 do begin
   for ix = 0, nx-1 do begin
    for iy = 0, ny-1 do begin
     printf, lun, lat[iy], lon[ix], so2flux[ix,iy,iz], zm[iz], zm[iz], $
                  time0, time1, $
