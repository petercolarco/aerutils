  ...
  set_plot, 'ps'
  device, file='compare_load.ps', /color
  !p.font=0
  ...
  plot, indgen(10)+1, /nodata, $
   /ylog, yrange=[0.01,100], ytitle='Burden [Tg]', $
  ...
  boxwhisker, burden[0:11,*], i+0.7, 0.15, color[i], $
  ...
  get_budget_table, expid1, aers[i], yyyy1, $
                    emis, sed, dep, wet, scav, burden, tau
  boxwhisker, burden[0:11,*], i+.9, 0.15, color[i], $
    medval, qtr_25th, qtr_75th, minval, maxval, meanval, stddv, $
    linethick=1, linecolor=linecolor
check, burden
  ...
  get_budget_table, expid2, aers[i], yyyy2, $
                    emis, sed, dep, wet, scav, burden, tau
  boxwhisker, burden[0:11,*], i+1.1, 0.15, color[i], $
    medval, qtr_25th, qtr_75th, minval, maxval, meanval, stddv, $
    linethick=1, linecolor=linecolor
check, burden
...
if(i eq 5) then break
  ...
  get_budget_table, expid3, aers[i], yyyy2, $
                    emis, sed, dep, wet, scav, burden, tau
  boxwhisker, burden[0:11,*], i+1.3, 0.15, color[i], $
    medval, qtr_25th, qtr_75th, minval, maxval, meanval, stddv, $
    linethick=1, linecolor=linecolor
check, burden
  endfor
