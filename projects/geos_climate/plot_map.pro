  pro plot_map, var, position, colort, title, lon, lat, $
                levelarray, colorarray, labelarray, noerase=noerase, align=align, aod=aod
  ...
  totstr = string(totem,format='(i5)')+' Tg yr!E-1!N'
  if(totem lt 10) then totstr = string(totem,format='(f5.1)')+' Tg yr!E-1!N'
  if(keyword_set(aod)) then begin
   totem = total(area*var)/total(area)
   totstr = string(totem,format='(f5.3)')
  endif
  loadct, 0
