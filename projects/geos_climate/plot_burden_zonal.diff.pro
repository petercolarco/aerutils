...
;  for it = 0, n_elements(nymd)-1 do begin
  ...
  case varo of
   ...
   'niextcoef'  : begin
                  levels = [-100,-5,-2,-1,-.5,-.2,-.1,.1,.2,.5,1,2,5]*100
                  varn = 'ni'
                  tag  = 'Nitrate'
                  ctab = 67
                  end
   'nh3'        : begin
                  levels = [-100,-5,-2,-1,-.5,-.2,-.1,.1,.2,.5,1,2,5]*10
                  varn = 'nh3'
                  tag  = 'Ammonia'
                  ctab = 67
                  end
...
;                            0.002,0.003,0.004,0.005,0.1,0.2]
   ...
   'ocextcoef'  : begin
                  levels = [-100,-1,-.5,-.2,-.1,-.05,-0.01,.01,.05,.1,.2,.5,1]*100
                  varn =  ['oc']
                  tag  = 'Organic Aerosol'
                  ctab = 67
                  sum = 1
...
;  endif
  ...
  ext = (mean(ext1,dim=1,/nan)-mean(ext2,dim=1,/nan))*1e12

; Now make a plot
  set_plot, 'ps'
  device, file='plot_burden_zonal.diff.'+expid1+'_'+expid2+'.'+varo+ $
   '.1941_1950.ps', $
 ...
 ; contour, h, lat, lev, /over, levels=indgen(22)+16, c_label=make_array(20,val=1)
  ...
  makekey, .1, .09, .8, .04, 0., -0.04, color=findgen(13), $
           labels=[' ',string(levels[1:12],format='(i4)')], align=.5
  xyouts, .525, .01, Tag+' Mixing Ratio [pptm], 1941-1950', align=.5, /normal
  loadct, ctab
