...
;  see the daynum stuff below.
  ...
  pro model_track, outfile, tracklon, tracklat, trackalt, trackdate, $
                   maxdate=maxdate
...
; Setup control for output
  ...
  ga_getvar, 'etaext.ddf', '', nodata, lon=lon, lat=lat, lev=lev, /noprint
  nz = n_elements(lev)
...
;  One-d variables
   ...
   varname1d = ['longitude', $
                ...
                'mid-layer pressure', $
                'aircraft altitude', $
                'Dust extinction AOD [550 nm]', $
                'Seasalt extinction AOD [550 nm]', $
                'Organic carbon extinction AOD [550 nm]', $
                'Black carbon extinction AOD [550 nm]', $
                'Sulfate extinction AOD [550 nm]' $
   ]
   varunit1d = ['degrees', $
                'degrees', $
...
;                'CO mixing ratio (Asia/Europe BB)', $
;                'CO mixing ratio (North America BB)', $
;                'CO mixing ratio (Central/South America BB)', $
;                'CO mixing ratio (Africa BB)', $
;                'CO mixing ratio (global non-BB)', $
;                'CO mixing ratio (North American non-BB)', $
;                'CO mixing ratio (Asian non-BB)', $
;                'CO mixing ratio (European non-BB)', $
                'CFC-12 mixing ratio', $
                'Extinction', $
                ...
                'Attenuated backscatter from TOA', $
                'Mid-layer height' $
]
   varunit2d = ['ppbv', $
                'ppmv', $
...
; and the 2d vars (all on 3 hr increment)
  fday = fix(fracday*24.)*0.0416667d
  jday1d_0 = long(jday)+fday
  jday1d_1 = long(jday)+fday + 0.0416667d
...
;    varout1[a] = interpolate(varval1[*,*],long(ix+.5),long(iy+.5))
    ...
    dy[a]      = varout1[a]-varout0[a]  ; difference per hour
    dx[a]      = jday[a]-jday1d_0[a]    ; fraction of day difference
    varout[a]  = varout0[a] + dy[a]*(dx[a]*24.)
...
; Two-d
; Search by uniq dates
  date2d_0 = strcompress(string(date2d_0),/rem)
  date2d_1 = strcompress(string(date2d_1),/rem)
...
;   some variables are composed of sums of others
    case varlist2d[ivar] of
     ...
     'bc'     : varlist = ['bcPhilic','bcPhobic']
     'oc'     : varlist = ['ocPhilic','ocPhobic']
    ...
    dy[*,a]      = varout1[*,a]-varout0[*,a]  ; difference per 3 hours
    for iz = 0, nz-1 do begin
     dx[iz,a]      = jday[a]-jday2d_0[a]      ; fraction of day difference
    endfor
    varout[*,a]  = varout0[*,a] + (dy[*,a]/3.)*(dx[*,a]*24.)
...
;     get the air density
      ...
      iy = interpol(indgen(n_elements(lat)),lat,lat_)
      for iz = 0, nz-1 do begin
       airdens0[iz,a] = interpolate(airdens0_[*,*,iz],ix,iy)
       airdens1[iz,a] = interpolate(airdens1_[*,*,iz],ix,iy)
      endfor
      dy[*,a]      = airdens1[*,a]-airdens0[*,a]
      airdens[*,a] = airdens0[*,a] + (dy[*,a]/3.)*(dx[*,a]*24.)
