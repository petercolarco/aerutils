...
; Make a plot
  set_plot, 'ps'
  fileps  = './ext_dc8.'+datewant+'.ps'
  filepng = './ext_dc8.'+datewant+'.png'
...
; Make a plot
  set_plot, 'ps'
  fileps  = './aot_dc8.'+datewant+'.ps'
  filepng = './aot_dc8.'+datewant+'.png'
  device, file=fileps, font_size=14, /helvetica, $
   xoff=.5, yoff=.5, xsize=18, ysize=10, /color
  !p.font=0
...
; Make a text file of the column tau versus time.
  openw, lun, 'aot_dc8.'+datewant+'.txt', /get_lun
  for ix = 0, nx-1 do begin
  ...
  openw, lun, 'ext_dc8.'+datewant+'.20070719_1500z.txt', /get_lun
  for iz = 0, nz-1 do begin
