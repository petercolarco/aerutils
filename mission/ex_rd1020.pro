; $Header: /cvsroot/esma/sandbox/colarco/aerutils/mission/ex_rd1020.pro,v 1.1.1.1 2008/04/30 19:04:12 colarco Exp $

pro ex_rd1020, filename, xname=xname, xvalues=xvals $
...
;   ex_rd1020
; PURPOSE:
;   reads a format-1020 exchange file
; CATEGORY:
;   exchange
; CALLING SEQUENCE:
;   ex_rd1020, filename, xname=xname, xvalues=xvals $
;             , vname=vname, data=dat, vmiss=vmiss $
...
;    xname = the name of the independent coordinate (usually time)
;    xvalues = M-element vector of values of the independent coordinate
;    vname = nv-elements array of names of the variables read from the file
;    data = M by nv array of data values read from the file
...
;    n_dimens = 1 (format 1020 deals only with 1D data)
...
if ftype ne 1020 then stop,"Wrong file type for 1020:",ftype
...
; ##############################################
     ...
     xvals = fltarr(nvpm,ndata)
     dat = fltarr(nvpm,ndata,nv)
     ...
     xvals = reform(xvals,nvpm*ndata)
     dat = reform(dat,nvpm*ndata,nv)
