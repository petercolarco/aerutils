...
;  see the daynum stuff below.
  ...
  pro model_track, outfile, tracklon, tracklat, trackdate
...
; Control files
  ctlfiles = ['inst3_3d_asd_Np.ddf', $
              'inst3_3d_aer_Nv.ddf', $
              'inst3d_chm_v.ddf', $
              'inst3_3d_ext-532nm_Nv.ddf', $
              'inst3_3d_ext-532nm_Nv.du.ddf', $
              'inst3_3d_ext-532nm_Nv.bc.ddf', $
              'inst3_3d_ext-532nm_Nv.oc.ddf', $
              'inst3_3d_ext-532nm_Nv.su.ddf', $
              'inst3_3d_ext-532nm_Nv.ss.ddf', $
              'tavg3d_cld_v.ddf', $
              'tavg3d_dyn_v.ddf' $
  ]
...
;  Two-d variables
   ...
   varname2d = ['dust mass concentration', $
                ...
                'total aerosol extinction [532 nm]', $
                'total aerosol single scatter albedo [532 nm]', $
                'dust aerosol extinction [532 nm]', $
                'dust aerosol single scatter albedo [532 nm]', $
                'black carbon aerosol extinction [532 nm]', $
                'black carbon aerosol single scatter albedo [532 nm]', $
                'organic carbon aerosol extinction [532 nm]', $
                'organic carbon aerosol single scatter albedo [532 nm]', $
                'sulfate aerosol extinction [532 nm]', $
                'sulfate aerosol single scatter albedo [532 nm]', $
                'sea salt aerosol extinction [532 nm]', $
                'sea salt aerosol single scatter albedo [532 nm]', $
                'cloud_area_fraction', $
                ...
                'eastward_wind', $
                'northward_wind', $
                'Relative_Humidity_after_moist_processes', $
...
;  date1d_1 = yyyy*1000000L + mon*10000L + dd*100L + hh
  ...
  dy1d = 0.125d
  fday = fix(fracday*8. + 0.5)*dy1d - dy1d/2.d
  jday1d_0 = long(jday)+fday
  jday1d_1 = long(jday)+fday+dy1d
  caldat, jday1d_0, mon, dd, yyyy, hh, mm, ss
  date1d_0 = strpad(yyyy,1000)+strpad(mon,10)+strpad(dd,10)+ $
             strpad(hh,10)+strpad(mm,10)
  caldat, jday1d_1, mon, dd, yyyy, hh, mm, ss
  date1d_1 = strpad(yyyy,1000)+strpad(mon,10)+strpad(dd,10)+ $
             strpad(hh,10)+strpad(mm,10)

; So for 4x day on the 03, 09 ,15, 21
  dy2d = 0.25d
  fday = fix(fracday*4. + 0.5)*dy2d - dy2d/2.d
  jday2d_0 = long(jday)+fday
  jday2d_1 = long(jday)+fday+dy2d
;IDL> plot, jday2d_1-long(jday2d_1[0])           
;IDL> oplot, jday2d_0-long(jday2d_0[0]), thick=3 
;IDL> oplot, jday-long(jday[0]), lin=2           
  caldat, jday2d_0, mon, dd, yyyy, hh, mm, ss
  date2d_0 = strpad(yyyy,1000)+strpad(mon,10)+strpad(dd,10)+ $
             strpad(hh,10)
  caldat, jday2d_1, mon, dd, yyyy, hh, mm, ss
  date2d_1 = strpad(yyyy,1000)+strpad(mon,10)+strpad(dd,10)+ $
             strpad(hh,10)
...
; One-d
  for ivar = 5, nvar1d-1 do begin
   ...
   for idate = 0, ndate-1 do begin
    ...
    print, varlist1d[ivar],' ', n_elements(a),' ', ctlfile, ' ', $
           gradsdate(dateu0[idate]), ' ', gradsdate(dateu1[idate])
    lon_ = tracklon[a]
    lat_ = tracklat[a]
...
; Search by uniq dates
  ...
  for ivar = 0,nvar2d-1 do begin
   ...
   for idate = 0, ndate-1 do begin
    ...
    print, varlist2d[ivar],' ', n_elements(a),' ', ctlfile, ' ', $
           gradsdate(dateu0[idate]), ' ', gradsdate(dateu1[idate])
    lon_ = tracklon[a]
    lat_ = tracklat[a]
...
;
    ...
    iv = 0
    nc4readvar, ctlfile, dateu0[idate], varlist[iv], varval0
    if(nv gt 1) then begin
     for iv = 1, nv-1 do begin
      nc4readvar, ctlfile, dateu0[idate], varlist[iv], varval_
      varval0 = varval0+varval_
     endfor
    endif
    iv = 0
    nc4readvar, ctlfile, dateu1[idate], varlist[iv], varval1
    if(nv gt 1) then begin
     for iv = 1, nv-1 do begin
      nc4readvar, ctlfile, dateu1[idate], varlist[iv], varval_
...
;     get the air density
      nc4readvar, ctlfiles[1], dateu0[idate], 'airdens', airdens0_
      nc4readvar, ctlfiles[1], dateu1[idate], 'airdens', airdens1_
      ix = interpol(indgen(n_elements(lon)),lon,lon_)
      iy = interpol(indgen(n_elements(lat)),lat,lat_)
      for iz = 0, nz-1 do begin
