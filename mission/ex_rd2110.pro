; $Header: /cvsroot/esma/sandbox/colarco/aerutils/mission/ex_rd2110.pro,v 1.1.1.1 2008/04/30 19:04:12 colarco Exp $

pro ex_rd2110, filename, xname=xname, xvalues=xvals $
...
;   ex_rd2110
; PURPOSE:
;   reads a format-2110 exchange file
; CATEGORY:
;   exchange
; CALLING SEQUENCE:
;   ex_rd2110, filename, xname=xname, xvalues=xvals $
...
if ftype ne 2110 then stop,"Wrong file type for 2110:",ftype
...
; ##############################################
     ...
     while ii lt ndata do begin
         ...
         for jj=0,ny-1 do begin
             readf,lun,y,indata
             yvals(y_start(ii)+jj) = y
             for kk=0,nv-1 do  dat(y_start(ii)+jj,kk) = indata(kk)
         endfor
         ii = ii + 1L
     endwhile
