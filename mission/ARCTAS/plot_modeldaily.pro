...
; Read in a and plot a model monthly mean AOT
  ...
  fileland = filehead+'/Y'+yyyy+'/M'+mm+'/' $
             +modid+'.inst2d_ext_x.total.'+yyyy+mm+dd+'18.nc4'

  geolimits=[45,-140,70,-90]

; Get the satellite
  ga_getvar, fileland, 'aodtau', aotlnd, lon=lon, lat=lat, time=time
  ...
  !p.font=0
   ...
   map_continents, color=0, thick=8
   map_continents, color=0, thick=5, /countries
   map_set, /noerase, limit=geolimits, position = [.05,.2,.95,.9]
   xyouts, .05, .96, 'ARCTAS_REPLAY 550 nm AOD ('+yyyy+mm+dd+'_18z)', /normal
