...
;  Header variables
   ...
   if(keyword_set(orbit)) then begin
    idOrbit = ncdf_vardef(cdfid,'orbit_track',idnT)
    ncdf_attput, cdfid, idOrbit, 'long_name', 'orbit track'
   endif
...
;  Write variables for header
   ...
   if(keyword_set(orbit)) then ncdf_varput, cdfid, idOrbit, fix(orbit)
...
;   get the variable
    ...
    nc4readvar, filename, varlist, varval0, lon=lon, lat=lat, lev=lev, /sum;, $
;                                            wantlon=wantlon, wantlat=wantlat, /sum
    filename = strtemplate(parsectl_dset(ctlfile),nymd1[idate],nhms1[idate])
    nc4readvar, filename, varlist, varval1, lon=lon, lat=lat, lev=lev, /sum;, $
;                                            wantlon=wantlon, wantlat=wantlat, /sum
