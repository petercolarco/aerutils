...
; Read
  vol = 1
  depol = 1
  read_curtain, filename, lon, lat, time, z, dz, vol=vol, depol=depol
...
;  a = where(lon lt -20 and lon gt -80 and $
  ...
  z  = z[*,a]
  dz = dz[*,a]
  vol = transpose(vol[*,a])*1e6  ; cm3 m-3

    datestr = strcompress(string(time[0],format='(i8)'),/rem)

    plotfile = lambda+'nm-'+version+'.vol.ps'
...
;   Put time into hour of day
    ...
    plot, indgen(n_elements(time)), /nodata, $
     ...
     position=position, $
     title = 'GEOS-5 particle volume [cm!E3!N m!E-3!N] !C'+ $
             'along CALIPSO track '+datestr, $
             charsize=.75
    loadct, 39
    levelarray = findgen(60)*20

    colorarray = findgen(60)*4+16
    plotgrid, vol, levelarray, colorarray, $
              time, z, time[1]-time[0], dz
    labelarray = strarr(60)
    labels = [0., 0.05,0.1,0.15,0.2,0.25]*4000
