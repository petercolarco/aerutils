...
; Read
  reff = 1
  depol = 1
  read_curtain, filename, lon, lat, time, z, dz, reff=reff, depol=depol
...
;  a = where(lon lt -20 and lon gt -80 and $
  ...
  z  = z[*,a]
  dz = dz[*,a]
  reff = transpose(reff[*,a])*1e6  ; um

    datestr = strcompress(string(time[0],format='(i8)'),/rem)

    plotfile = lambda+'nm-'+version+'.reff.ps'
...
;   Put time into hour of day
    ...
    plot, indgen(n_elements(time)), /nodata, $
     ...
     position=position, $
     title = 'GEOS-5 Effective Radius [um] !C'+ $
    ...
    plotgrid, reff, levelarray, colorarray, $
