; MODIS extract on offset tracks
  ...
  outdir = '/misc/prc10/colarco/tc4/d530_tc4_02/lidar/'

;  for it = 0, 6 do begin
  for it = 0,0 do begin

;  Read the track file to get sampling date, lon, lat
   trackfile = '/misc/prc08/calipso/MBL_STK_Orbit_Tracks_Jul07/'+track[it]
   read_calipso_track, trackfile, lonInp, latInp, dateInp, mbl = 2

;  Keep only the points >= 20070713 12Z
   a = where(dateInp ge 20070701.375d)
   dateInp = dateInp[a]
   lonInp  = lonInp[a]
   latInp  = latInp[a]
...
;  Model
   ...
   for isp = 0, 5 do begin
    ...
    printf, lun, 'dset /misc/prc10/colarco/tc4/d530_tc4_02/tau/Y%y4/M%m2/' + $
                       'd530_tc4_02.inst3d_ext-532nm_v.%y4%m2%d2_%h200z.hdf'+type[isp]
    printf, lun, 'options template'
    printf, lun, 'tdef time 80 linear 12:00z13Jul2007 6hr'
    free_lun, lun
    filetocreate = outdir+'d530_tc4_02.inst3d_ext-532nm_v.'+species[isp]+'.'+track[it]+'.hdf'
...
;  Do the met track
   ...
   printf, lun, 'dset /misc/prc10/colarco/tc4/d530_tc4_02/diag/Y%y4/M%m2/' + $
                      'd530_tc4_02.tavg3d_dyn_v.%y4%m2%d2_%h200z.hdf'
   printf, lun, 'options template'
   printf, lun, 'tdef time 80 linear 12:00z13Jul2007 6hr'
   free_lun, lun
   ...
   printf, lun, 'dset /misc/prc10/colarco/tc4/d530_tc4_02/diag/Y%y4/M%m2/' + $
                      'd530_tc4_02.tavg3d_met_e.%y4%m2%d2_%h200z.hdf'
   printf, lun, 'options template'
   printf, lun, 'tdef time 80 linear 12:00z13Jul2007 6hr'
   free_lun, lun
   ...
   printf, lun, 'dset /misc/prc10/colarco/tc4/d530_tc4_02/diag/Y%y4/M%m2/' + $
                      'd530_tc4_02.tavg2d_met_x.%y4%m2%d2_%h2%n2z.hdf'
   printf, lun, 'options template'
   ...
   printf, lun, 'dset /misc/prc10/colarco/tc4/d530_tc4_02/diag/Y%y4/M%m2/' + $
                      'd530_tc4_02.tavg3d_cld_v.%y4%m2%d2_%h200z.hdf'
   printf, lun, 'options template'
   printf, lun, 'tdef time 80 linear 12:00z13Jul2007 6hr'
   free_lun, lun

   filetocreate = outdir+'d530_tc4_02.met.'+track[it]+'.hdf'
