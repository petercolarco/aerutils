...
; Read the curtain files
  ...
  pro read_curtain, filename, lon, lat, time, z, dz, $
                    ...
                    depol = depol, reff=reff, vol=vol, area=area, $
                    refreal=refreal, refimag=refimag
...
; Effective radius
  if(keyword_set(reff)) then begin
...
; Real Refractive Index
  if(keyword_set(refreal)) then begin
   id = ncdf_varid(cdfid,'refreal')
   ncdf_varget, cdfid, id, refreal
  endif

; Imaginary Refractive Index
  if(keyword_set(refimag)) then begin
