...
; Read
  ...
  extinction = 1
  aback = 1
  read_curtain, filename, lon, lat, time, z, dz, aback_toa_tot = depol
...
; select
  ...
  plotfile = 'aback.'+lambda+'nm-'+version+'.ps'
...
;   xrange=[time[0],time[nt-1]], xtitle='Hour of Day', xstyle=1, $
   ...
   position=position, $
   title = 'Attenuated Backscatter ('+version+') [km-1 sr-1]', charsize=.75
  nlon = n_elements(lon)
  ...
  for il = 0, n_elements(labels)-1 do begin
   for ia = n_elements(levelarray)-1, 0, -1 do begin
    if(levelarray[ia]*1e3 ge labels[il]) then a = ia
   endfor
   labelarray[a] = string(labels[il],format='(i1)')
  endfor
  polyfill, [.13,.475,.475,.13,.13],[.75,.75,.85,.85,.75], color=255, /normal  
  makekey, .15, .8, .3, .035, 0, -.035, color=colorarray, $
   label=labelarray, $
   align=.5, /no, charsize=.75
  xyouts, .45, .765, 'x 1e-3', charsize=.75, /normal, align=.5
