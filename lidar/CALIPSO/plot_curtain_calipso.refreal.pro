...
; Read
  refreal = 1
  read_curtain, filename, lon, lat, time, z, dz, refreal=refreal
...
;  a = where(lon lt -20 and lon gt -80 and $
  ...
  z  = z[*,a]
  dz = dz[*,a]
  refreal = transpose(refreal[*,a])

    datestr = strcompress(string(time[0],format='(i8)'),/rem)

    plotfile = lambda+'nm-'+version+'.refreal.ps'
...
;   Put time into hour of day
    ...
    plot, indgen(n_elements(time)), /nodata, $
     ...
     position=position, $
     title = 'GEOS-5 Real Component of Refractive Index !C'+ $
             'along CALIPSO track '+datestr, $
             charsize=.75
    loadct, 39
    levelarray = 1.3+findgen(60)*.005

    colorarray = findgen(60)*4+16
    plotgrid, refreal, levelarray, colorarray, $
              time, z, time[1]-time[0], dz
    labelarray = strarr(60)
    labels = 1.3+[0., 0.05,0.1,0.15,0.2,0.25]
