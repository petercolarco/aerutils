...
; Set up the plot
  set_plot, 'ps'
  device, file='corr_tatl_modis.f.'+regstr+'.ps', /helvetica, font_size=14, /color, $
   xoff=.5, yoff=.5, xsize=18, ysize=26
  loadct, 39
  !p.font=0

; Read in the tracks
  read_mbltrack_modis, time, date, lon, lat, lev, tau, res='f'
...
;  Plot a map of tracks and AOT retrieval points
   map_set, limit=[latwant[0],lonwant[0],latwant[1],lonwant[1]], $
    position=[.075,.775,.5,.95]
   xyouts, .075, .975, $
      'MYD04: '+strcompress(string(dateday[uniqday[iday]]), /rem), $
      /normal

   loadct, 0
   if(qb[0] ne -1) then begin
    for iqb = 0, n_elements(qb)-1 do begin
     wanttime = date[iestart[qb[iqb]]]
;    Get the modis data for this time
     ga_getvar, 'taufile.modis.f.ctl', 'aodtau', aodtau, lon=lonm, lat=latm, $
      wantlev = ['550'], wantlon=lonwant, wantlat=latwant, wanttime = wanttime
     qa = where(aodtau gt 10.)
     if(qa[0] ne -1) then aodtau[qa] = !values.f_nan
     dx = lonm[1]-lonm[0]
     dy = latm[1]-latm[0]
     plotgrid, aodtau, [0], [180], lonm, latm, dx, dy
    endfor
   endif
...
;    Get the modis data for this time
   ...
   for i = 0, ntracks-1 do begin
    ...
    if(usetrack[i] eq 1) then begin
     ...
     hour_ = (time_ - dateday[uniqday[iday]])*24.
      ...
      for iy = 1, 7 do begin
       ...
       if(ix eq iy) then begin
        ...
        if(n_elements(b) lt 3) then begin
         ...
         if(r eq !values.f_nan) then begin
          xyouts, xp, yp, '-', /normal, align=.5
         endif else begin
