...
; Little snip of code to help figure out the distances in the data
;  for i = 0, 5 do begin
;   print, dist_pnt2pnt(lat[1,i],lon[1,i],lat[1,i+1],lon[1,i+1])
;  endfor
;  dist = fltarr(n_elements(time)-1)
;  for i = 0L, n_elements(time)-2 do begin
;   dist[i] = dist_pnt2pnt(latn[i],lonn[i],latn[i+1],lonn[i+1])
;  endfor
;stop
  nlag = 400
...
;map_set, /cont
  ...
  set_plot, 'ps'
  device, filename='autocor.ps', /color
  !p.font=0
  loadct, 39
...
; This is what I will lag against
  ...
  lagcorr = make_array(ntracks,7,nlag,value=!values.f_nan)
  clagcorr = make_array(ntracks,7,nlag,value=0)
  for itrack = 0, ntracks-1 do begin
   for imbl = 0, 6 do begin
    tau_ = tau[trackstart[itrack]:trackend[itrack],imbl]
...
;    Mean of points -k away from another data point
     for il = lag, n_elements(tau_)-1 do begin
      if(finite(tau_[il-lag]) eq 1 and finite(tau_[il]) eq 1) then begin
       m1 = m1 + tau_[il-lag]
       c1 = c1 + 1
      endif
     endfor
...
;    Standard deviation of points -k away from another data point
     for il = lag, n_elements(tau_)-1 do begin
      if(finite(tau_[il-lag]) eq 1 and finite(tau_[il]) eq 1) then begin
       s1 = s1 + (tau_[il-lag]-m1)^2.
      endif
     endfor
     s1 = sqrt(s1/(c1-1))

;    Mean of points +k away from another data point
     for il = 0, n_elements(tau_)-1-lag do begin
      if(finite(tau_[il]) eq 1 and finite(tau_[il+lag]) eq 1) then begin
       m2 = m2 + tau_[il+lag]
       c2 = c2 + 1
      endif
     endfor
...
;    Lag correlation
     for il = 0, n_elements(tau_)-1-lag do begin
      if(finite(tau_[il]) eq 1 and finite(tau_[il+lag]) eq 1) then begin
       corr = corr + (tau_[il]-m1)*(tau_[il+lag]-m2)
      endif
     endfor

     lagcorr[itrack,imbl,lag] = corr/(s1*s2)/sqrt((c1-1.)*(c2-1.))
     clagcorr[itrack,imbl,lag] = sqrt((c1-1.)*(c2-1.))
...
; Now compose a weighted average of the lagcorrelations
  ...
  for lag = 0, nlag-1 do begin
   lc = reform(lagcorr[*,*,lag],ntracks*7)
   cc = reform(clagcorr[*,*,lag],ntracks*7)
   a = where(finite(lc) eq 1 and cc gt 2)
   lagcorr_[lag] = total(lc[a]*cc[a])/total(cc[a])
  endfor

  hour = findgen(nlag)*68./20.
  hour = findgen(nlag)*13./20.
  hour[0] = .1
  oplot, hour, lagcorr_, thick=6, color=colorarray[ilon]
