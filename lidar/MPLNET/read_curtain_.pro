...
; Read the curtain files
  ...
  pro read_curtain_, filename, lon, lat, time, z0, z, dz, $
                     pblh = pblh, $
...
; Mass concentration
  ...
  if(keyword_set(oc)) then begin
   id = ncdf_varid(cdfid,'oc')
   ncdf_varget, cdfid, id, oc
  endif
...
; Aback_Sfc
  ...
  if(keyword_set(pblh)) then begin
