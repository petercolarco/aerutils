...
# Modify below to specify kind of averaging you want: time, resolution, etc.
  set years       = `echo 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010`
  set months      = `echo 01 02 03 04 05 06 07 08 09 10 11 12`
  set satellites  = `echo MOD04 MYD04`
  set resolution  = `echo c`
...
#       QA selection
        ...
        set ntime = 2920
        if($YYYY == 2000 || $YYYY == 2004 || $YYYY == 2008) set ntime = 2928
        set starttime  = 0z1jan$YYYY
        set middletime = 12z15jun$YYYY 
        set tdefstr = "tdef time "$ntime" linear "$starttime" 3hr"
        echo $tdefstr

cat > qawt.${rand}.ddf << EOF
dset $datadir/Y$YYYY/M%m2/$datatyp.$odsver.$qawt.%y4%m2%d2.nc4
options template
$tdefstr
EOF

cat > qafl.${rand}.ddf << EOF
dset $datadir/Y$YYYY/M%m2/$datatyp.$odsver.$qafl.%y4%m2%d2.nc4
options template
$tdefstr
EOF
...
#       Average the AOT
        lats4d.sh -v -i qawt.${rand}.ddf -j qafl.${rand}.ddf -o tmp_qawt.${rand} \
           -func "sum(@.1*num.2(z=1),t=1,t=$ntime)/sum(maskout(num.2(z=1),@.1(z=$z550)),t=1,t=$ntime)" \
           -levs 550 \
...
#       Average the QAFL
        ...
        \mv -f tmp_qawt.${rand}.nc4 $datadir/Y$YYYY/$datatyp.$odsver.$qawt.num_all.$YYYY.nc4
        \mv -f tmp_qafl.${rand}.nc4 $datadir/Y$YYYY/$datatyp.$odsver.$qafl.num_all.$YYYY.nc4
