...
dset $datadir/Y$YYYY/M%m2/$datatyp.$odsver.$qawt.%y4%m2.nc4
options template
$tdefstr
EOF
...
#       Average the AOT
        lats4d.sh -v -i qawt.${rand}.ddf -j qafl.${rand}.ddf -o tmp_qawt.${rand} \
           -func "sum(@.1*qasum.2(z=1),t=1,t=12)/sum(maskout(qasum.2(z=1),@.1),t=1,t=12)" \
...
#       Average the QAFL
        ...
        \mv -f tmp_qawt.${rand}.nc4 $datadir/Y$YYYY/$datatyp.$odsver.$qawt.$YYYY.nc4
        \mv -f tmp_qafl.${rand}.nc4 $datadir/Y$YYYY/$datatyp.$odsver.$qafl.$YYYY.nc4
