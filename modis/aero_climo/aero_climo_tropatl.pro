;** define input params
    lat0 =  10.0           ;** center latitude of Mercator grid
    lon0 = -20.		   ;** center longitude
    months = indgen(12)+1  ;** months to analyze
    dirpl='./'             ;** directory of plots
    years = findgen(8)+2003
...
for imon=0,n_elements(months)-1 do begin	;** month loop
    ...
    fn =  dirpl+'AOD_' + strmid(strlowcase(strtrim( monstr(month) ,2)),0,3) + '_'  $
		  + strtrim(long(p(ip)),2)+'_tropatl.ps'
...
;    !color=0
    ...
    lim=[lat0-50,lon0-50,lat0+50,lon0+50]
    map_set,lat0,lon0,/ortho,/noerase,lim=lim,tit=datl0+' '+datl1, charsize=1.25, $
     position=[.05,.1,.95,.9]
...
;    contour,dat,lonxx,lata,/fill,/follow,/over,lev=levs,/cell
    plotgrid, dat, levelarray, colorarray, lonxx, lata, $
              lonxx[1]-lonxx[0], lata[1]-lata[0], $
              undef=!values.f_nan, /map
    contour, dat, lonxx, lata, /over, level=0.2, thick=6
    makekey, .1,.05,.8,.03,0,-.025,color=colorarray, label=['0','1'], /noborder, charsize=.75
; PRC: take out the range rings
