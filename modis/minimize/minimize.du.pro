...
;  Read the observation
   ...
   nc4readvar, filename, 'aodtau', t_o, wantlev=550., wanttime=[nymd[it],nhms[it]]
;  Screen fill values
...
;  Read the model
   ...
   varlist = ['ss','SO4','BC','OC']
   nc4readvar, filename, varlist, tau, wantlev=5.5e-7, wanttime=[nymd[it],nhms[it]], /sum, /template
   nc4readvar, filename, 'du', tss, wantlev=5.5e-7, wanttime=[nymd[it],nhms[it]], /sum, /template
...
; normalize result
  ...
  save, file='aqua.du.2009.sav', /all

  set_plot, 'ps'
  device, file='du_minimize.aqua_2009.ps', /helvetica, font_size=12, /color
  !p.font=0
...
; Find min of each curve
  ...
  plot, f, r_[*,0], /nodata, $
   ...
   title = 'Dust minimization vs. Aqua, R_qfed21_2_3 (2009)!Ckernel = step function'
