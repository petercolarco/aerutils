...
; serving to Google Earth
  ...
  if(varname eq 'totexttau') then begin 
   ...
   ga_getvar, filename, 'duexttau', duexttau, lon=lon, lat=lat, wanttime=wanttime, /dods5, rc=rc
   ga_getvar, filename, vars, tau, lon=lon, lat=lat, wanttime=wanttime, /dods5
   varout = (duexttau+tau)*scalefac
  endif else begin
   ga_getvar, filename, varname, varout, lon=lon, lat=lat, $
              wanttime=wanttime, wantlev=wantlev, dods5=dods5
