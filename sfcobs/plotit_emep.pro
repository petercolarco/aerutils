...
; Create a plot file
  set_plot, 'ps'
  device, file='./output/plots/sfcobs_site.emep.ps', /color, /helvetica, font_size=14, $
...
; position the plots
  ...
  for iloc = 0, nloc-1 do begin
   ...
   if(n_elements(a) gt 3) then begin
    plot_panel, date, so4smass[*,iloc], so4smassstd[*,iloc], so4emep[*,iloc], so4emepstd[*,iloc], $
                location[iloc], 'Model', 'EMEP', 'SO4 Concentration [!Mm!3g m!E-3!N]', position[*,0:1], $
                locuse=locuse_
    locuse[iloc] = locuse_
   endif
...
; Make a map
  loadct, 0
  map_set, /cont, limit=[30,-20,70,50]
