  pro sample_month, satId, satPath, expStr, modPath, yyyy, mm, type, $
                    ...
                    hourly=hourly, diag=diag, qawt=qawt, geos4=geos4
...
; check on hourly
  ...
  printf, lun, $
   'dset '+satpath+'Y%y4/M%m2/'+satid+'_L2.aero_F12_0022.'+qawtstr+'.%y4%m2%d2.hdf'
  ...
  printf, lun, $
   'dset '+satpath+'Y%y4/M%m2/'+satid+'_L2.aero_F12_0022.'+qaflstr+'.%y4%m2%d2.hdf'
  printf, lun, 'options template'
  printf, lun, 'tdef time '+string(nd*ntd)+' linear 0z01'+mon[fix(mm)-1]+yyyy+incstr
  free_lun, lun

  if(diag) then begin
;  read from a model chem_diag file instead of tau2d
;  check on hourly
   if(hourly) then begin
    dsetstr = modpath+'Y%y4/M%m2/'+expstr+'.chem_diag.sfc.inst.%y4%m2%d2_%h200z.hdf'
   endif else begin
    dsetstr = modpath+'Y%y4/M%m2/'+expstr+'.chem_diag.sfc.%y4%m2%d2_%h200z.hdf'
   endelse
   openw, lun, modddf, /get_lun
   printf, lun, 'dset '+dsetstr
   printf, lun, 'options template'
   printf, lun, 'tdef time '+string(nd*ntd)+' linear 0z01'+mon[fix(mm)-1]+yyyy+incstr
   free_lun, lun
  endif else begin
;  read from tau2d
   ...
   if(geos4) then begin
    printf, lun, $
     'dset '+modpath+'Y%y4/M%m2/'+expStr+'.%y4%m2%d2.hdf'
   endif else begin
    printf, lun, $
     'dset '+modpath+'Y%y4/M%m2/'+expStr+'.%y4%m2%d2_%h200z.hdf'
   endelse
   printf, lun, 'options template'
   printf, lun, 'tdef time '+string(nd*ntd)+' linear 0z01'+mon[fix(mm)-1]+yyyy+incstr
   free_lun, lun
...
;   Get the satellite grid to check nx,ny
    ...
    nx = n_elements(lon)
    ny = n_elements(lat)
;   If the satellite longitudes go from -180 to 180, possibly want
;   to shift for consistency with GEOS-4 model longitudes 0 to 360
    xshift = 0
    if(geos4) then begin
     a = where(lon lt 0)
...
; Create the template for the output file
  idvar = lonarr(nv)
  filetag = expstr
  if(diag) then filetag = '.chem_diag.sfc'
  if(pixelweight) then filetag = filetag+'_pw'
  if(hourly) then filetag = filetag+'.hourly'
  filetag = filetag+'.'+qawtstr+'.'
  
  fileout = modpath+'/Y'+yyyy+'/M'+mm+'/'+filetag+satid+'_F12_0022.'+yyyy+mm+'.hdf'
...
;     possibly shift arrays in longitude if wanting GEOS-4 like orientation
      if(geos4) then begin
       alon = where(lon lt 0)
       if(alon[0] ne -1) then begin
        lon[alon] = lon[alon]+360.
        xshft = n_elements(alon)
        lon = shift(lon,xshft)
        vartmp = shift(vartmp,xshft,0,0,0)
       endif
      endif
...
; lats4d it so it's grads readable
  ...
  if(geos4) then begin
   ...
   printf, lun, 'xdef lon '+string(nx)+' linear -180 '+string(360./nx)
  endelse
