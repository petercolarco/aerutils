...
dset $expdir/Y$YYYY/M%m2/$expstr.%y4%m2%d2_%h200z.nc4
options template
$tdefstr
$lonstr
$latstr
...
# First subset possibly large input files
  lats4d.sh -v -i ${expstr}.${rand}.ddf -o tmp_aer.${rand} \
     -mxtimes $ntime -vars $varstr $levstr
...
# clean-up
  ...
  mv -f tmp_qawt.${rand}.nc4 $expdir/Y$YYYY/$expstr.$datatyp.$algorithm.$qawt.${YYYY}ASO.nc4
