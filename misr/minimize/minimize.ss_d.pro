...
; Read in a masking map (e.g., to get only over ocean)
  filename = '../../data/d/ARCTAS.region_mask.x576_y361.2008.nc'
  nc4readvar, filename, 'region_mask', mask, lon=lon, lat=lat
...
;  Read the observation
   ...
   nc4readvar, filename, 'aodtau', t_o, wantlev=558., wanttime=[nymd[it],nhms[it]]
;  Screen fill values
   a = where(t_o gt 1.e14)
   if(a[0] ne -1) then t_o[a] = !values.f_nan
   y_o = alog10(t_o + .01)

;  Read the model
   filename = strtemplate(dset_m,nymd[it],nhms[it])
   varlist = ['duexttau','suexttau','bcexttau','ocexttau']
   nc4readvar, filename, varlist, tau, wantlev=5.5e-7, wanttime=[nymd[it],nhms[it]], /sum
   nc4readvar, filename, 'ssexttau', tss, wantlev=5.5e-7, wanttime=[nymd[it],nhms[it]], /sum
...
;contour, k,title=f[iff]
  ...
  set_plot, 'ps'
  device, file='ss_minimize.200906.d.ps', /helvetica, font_size=12, /color
  !p.font=0
...
; Find min of each curve
  ...
  plot, f, r_[*,0], /nodata, $
   ...
   title = 'Sea salt minimization vs. MISR, dR_qfed21_2_3 (200906)!Ckernel = step function'
