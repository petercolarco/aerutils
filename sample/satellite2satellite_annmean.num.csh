...
dset $datadir/Y$YYYY/M%m2/$datatyp.$sample$odsver.${qawt}_qawt.num.%y4%m2.nc4
options template
$tdefstr
EOF
...
#       Average the AOT
        lats4d.sh -v -i qawt.${rand}.ddf -j qafl.${rand}.ddf -o tmp_qawt.${rand} \
           -func "sum(@.1*num.2,t=1,t=12)/sum(num.2,t=1,t=12)" \
           -vars aot \
        ...
        \mv -f tmp_qawt.${rand}.nc4 $datadir/Y$YYYY/$datatyp.$sample$odsver.${qawt}_qawt.num.$YYYY.nc4
