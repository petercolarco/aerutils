  ...
  for isample = 0, nsamples-1 do begin
   ...
   for ioro = 0, 1 do begin
    ...
    for it = 0, nt-1 do begin
     ...
     globaot[it,ioro,isample] = aave(numtmp,area,nan=nan)/aave(dentmp,area,nan=nan)-corrfact[isample,ioro]
    endfor
...
print, isample, globaot[*,1,isample]
  ...
  !p.multi = [0,1,2]
  set_plot, 'ps'
  device, file='modis_aot_monthly_climatology_corrected.ocn.ps', /color, /helvetica, font_size=12, $
  ...
  plot, x, globaot[*,0,0], /nodata, yrange=[0.1,.18], $
  ...
  xtickv = [0,indgen(nt)+1]
  xticklabels = [' ','J','F','M','A','M','J','J','A','S','O','N','D',' ']
  plot, x, globnum[*,0,0], /nodata, yrange=[1e5,1e9], /ylog, $
   xtitle = 'Month', ytitle='Number', title='MYD04 Ocean Number', $
   xticks = nt+1, xtickn = xticklabels, xminor=1, yminor=2, $
   xstyle=9, ystyle=9, thick=3, xrange=[0,nt+1], xtickv=xtickv
  y = reform([globaot[*,0,0]-.01,globaot[*,0,0]+.01],nt,2)
  polymaxmin, x, y, thick=6, fill=200
  ...
  !P.multi=[0,1,2]
  set_plot, 'ps'
  device, file='modis_aot_monthly_climatology_corrected.lnd.ps', /color, /helvetica, font_size=12, $
   xsize=16, ysize=20, xoff=.5, yoff=.5
  !p.font=0
  loadct, 0
  x = indgen(nt)+1
  xtickv = [0,indgen(nt)+1]
  xticklabels = [' ','J','F','M','A','M','J','J','A','S','O','N','D',' ']
  plot, x, globaot[*,1,0], /nodata, yrange=[0.1,.30], $
  ...
  loadct, 0
  plot, x, globaot[*,1,0], /nodata, yrange=[-.06,.06], $
   xtitle = 'Month', ytitle='AOT', title='Difference from Full Swath', $
   xticks = nt+1, xtickn = xticklabels, xminor=1, yminor=2, $
   xstyle=9, ystyle=9, thick=3, xrange=[0,nt+1], xtickv=xtickv
  polyfill, [0,max(x)+1,max(x)+1,0,0], [-.01,-.01,.01,.01,-.01], color=200
  ...
  xtickv = [0,indgen(nt)+1]
  xticklabels = [' ','J','F','M','A','M','J','J','A','S','O','N','D',' ']
  plot, x, globnum[*,1,0], /nodata, yrange=[1e4,1e8], /ylog, $
   xtitle = 'Month', ytitle='Number', title='MYD04 Land Number', $
   xticks = nt+1, xtickn = xticklabels, xminor=1, yminor=2, $
   xstyle=9, ystyle=9, thick=3, xrange=[0,nt+1], xtickv=xtickv
  y = reform([globaot[*,0,0]-.01,globaot[*,0,0]+.01],nt,2)
  polymaxmin, x, y, thick=6, fill=200
