  ...
  sample = ['','shift.misr.','shift.subpoint.']
...
; Make plot
  set_plot, 'ps'
  device, /color, file='./output/plots/frequency.MOD04.'+seasons[im]+'_climatology.ps', $
...
; Read in the files
  ...
  for ictl = 0, nctl-1 do begin
   title_array = 'Fraction of AOT > '+hstr+' ('+seasons[im]+', 2000 - 2006) ('+sample_name[ictl]+')'
   map_set, position=[0.05,0.2,0.95,.9], title=title_array
   for ix = 0, nxr-1 do begin
    for iy = 0, nyr-1 do begin

;    Get the data and turn into something you can plot
     filename = './output/tables/'+outfilehead+plottitle[ix,iy]+sample[ictl]+'txt'
     read_frequency_histogram, filename, $
       date, histnorm, num, histmin, histmax, nbin
;    months
     mm = fix( (date-fix(date/10000)*10000.)/100)
     a = where(mm eq im+1)
     histnorm = histnorm[*,a]
     date = date[a]
     num = num[a]
