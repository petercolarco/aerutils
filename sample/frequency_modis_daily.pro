...
; Procedure call
  pro frequency_modis_daily, resolution
...
; Models
  outfilehead  = 'MOD04_regions.'+resolution+'.freq.daily.'
  outfilehead2 = 'MOD04_regions.'+resolution+'.daily.'
  sample = ['modis','shift_misr','shift_subpoint.obs_22']
  typefile = ['MODIS','MODIS','MODIS','MODIS']
...
;  Loop over months
   for im = 0, 11 do begin
    ...
    cmd = 'mkdir -p ./output/tables/Y'+yyyy[iy]+'/M'+mm[im]
    spawn, cmd, /sh
...
;   Create the output files
    for ictl = 0, nctl-1 do begin
     for ireg = 0, nreg-1 do begin
      filename  = $
       './output/tables/Y'+yyyy[iy]+'/M'+mm[im]+'/'+outfilehead+plottitle[ireg]+sample[ictl]+'.'+yyyy[iy]+mm[im]+'.txt'
      filename2 = $
       './output/tables/Y'+yyyy[iy]+'/M'+mm[im]+'/'+outfilehead2+plottitle[ireg]+sample[ictl]+'.'+yyyy[iy]+mm[im]+'.txt'
...
;     Control file names
      qawtfile = 'MOD04_L2_ocn.aero_tc8_005.qawt.'+resolution+'.'+sample[ictl]+'.ctl'
      qaflfile = 'MOD04_L2_ocn.aero_tc8_005.qafl.'+resolution+'.'+sample[ictl]+'.ctl'
