...
;  This returns all possible points, even those with no retrieval
   ...
   obsq  = obs[a]
   ksq   = ks[a]
;  Filter for those that meet minimum QA
   if(not(keyword_set(qa_thresh))) then qa_thresh=1
   a = where(obsq ge qa_thresh and levq eq 550.)
   latq  = latq[a]
   lonq  = lonq[a]
   levq  = levq[a]
   timeq = timeq[a]
...
; Filter any remaining undef
  ...
  if(a[0] ne -1) then begin
   ...
   timea = timea[a]
   obsa  = obsa[a]
   if(keyword_set(ktqa)) then obsq  = obsq[a]
   ksa   = ksa[a]
  endif
...
; Check that finally all the variables are the same length
  ...
  if(keyword_set(ktaqa)) then begin
   if(n_elements(lata) ne (n_elements(obsq))) then rc = 8
  endif
