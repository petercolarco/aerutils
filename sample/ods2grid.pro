...
; Create the filename to read
  ...
  odsfile = odsdir + 'Y'+yyyy+'/M'+mm+'/'+satellite+'_L2_051.'+odsver+'.'+method+'.'+yyyy+mm+dd+'.ods'
...
; What is the next date?
  ...
  odsfile_ = odsdir + 'Y'+yyyy_+'/M'+mm_+'/'+satellite+'_L2_051.'+odsver+'.'+method+'.'+yyyy_+mm_+dd_+'.ods'
...
; ordered for the aotOcn as for the aotfOcn, aotqOcn.
  ...
  modpdf = make_array(nx,ny,nbins,ntday,val=0)        ; Number of obs in each PDF bin
...
;   Aggregate the time and level specific variables
    ...
    if(a[0] ne -1) then begin
     ...
     for ipts = 0L, npts_-1L do begin
      ...
      ibin   = max([0,min([nbins-1,fix(obstmp*(1./dbin))])])
      modpdf_[ix[ipts],iy[ipts],ibin] = modpdf_[ix[ipts],iy[ipts],ibin] + 1
...
;    Find the std deviation of the 550 nm channel
    ...
    modpdf_ = reform(modpdf_,1L*nx*ny,nbins)
    if(b[0] ne -1) then modpdf_[b,*] = 1.e15
    modAot[*,*,it] = modAot_
    modnum[*,*,it] = modnum_
    modqa[*,*,it] = modqa_
    modpdf[*,*,*,it] = modpdf_
...
;  Write to output files
   ...
   filehead = satellite+'_L2_'+tag+'.'+sampling+odsver+'_051.'+qaststr
...
;  if not(noundef) and first date then write undef files
   if(not(noundef) and date eq datestart) then begin
    filehead = 'undef.'+satellite+'_L2_'+tag+'.'+sampling+odsver+'_051.'+qaststr
    undef = modAot
    undef[*] = 1.e15
    undefpdf = modpdf
    undefpdf[*] = 1.e15
