...
;  a = where(aotsat gt 100. or aotsat lt 0.)
  ...
  set_plot, 'ps'
  device, file='./output/plots/'+satId+samplestr+'.qast.'+res+numstr+'.aodtau550_annual.'+excstr+yyyy+geoname+'.ps', $
...
; Resstr
  ...
  title = 'MODIS Aqua'+ sample_title + '('+resstr+', '+yyyy+')'
  if(keyword_set(exclude)) then $
   title = 'MODIS Aqua full swath where'+sample_title+'does not observe ('+resstr+', '+yyyy+')'
  if(keyword_set(exclude) and keyword_set(inverse)) then $
   title = 'MODIS Aqua full swath only where'+sample_title+'observes ('+resstr+', '+yyyy+')'
