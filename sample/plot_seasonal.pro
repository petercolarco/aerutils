...
;   print, regtitle[ireg], reg_aot[ireg], reg_std[ireg], format='(a20,2(2x,f6.4))'
  ...
  set_plot, 'ps'
  device, file='./output/plots/'+satId+samplestr+'.qast.'+res+numstr+'.aodtau550_seasonal.'+excstr+yyyy+season+geoname+'.ps', $
...
; Resstr
  ...
  title = 'MODIS Aqua'+ sample_title + '('+yyyy+' '+season+')'
  if(keyword_set(exclude)) then $
   title = 'MODIS Aqua full swath where'+sample_title+'does not observe ('+yyyy+' '+season+')'
  if(keyword_set(exclude) and keyword_set(inverse)) then $
   title = 'MODIS Aqua full swath only where'+sample_title+'observes ('+yyyy+' '+season+')'
