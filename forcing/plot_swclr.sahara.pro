...
; Get the lifetimes
  expid = ['bF_F25b9-base-v1.',$
           'bF_F25b9-base-v7.', $
           'bF_F25b9-base-v6.', $
           'bF_F25b9-base-v5.' ] + 'geosgcm_surf.sahara'
...
;  draft = 1 ; show experiment information on plot
  ...
  set_plot, 'ps'
  device, file = './output/plots/plot_swclr.'+expid[0]+'.ps', /color, /helvetica, $
...
; SWTOACLR
  ...
  if(rc eq 0) then begin
   plot, [0,13], [-2,6], /nodata, $
    thick=4, xstyle=9, ystyle=9, color=0, $
    xticks=13, xtickname=xtickname, $
    xtitle = 'Month', ytitle='SWTOACLR [W m!E-2!N]', title='SWTOACLR'
   polymaxmin, indgen(12)+1, swtoaclr[0:11,*], color=0, fillcolor=208, lin=linarray[0], thick=12
  endif
  if(nexpid gt 1) then begin
   ...
   for iexpid = 1, nexpid-1 do begin
    read_forcing_table, expid[iexpid], years, $
                        ...
                        swtoaall, swsfcall, swatmall, rc=rc
    if(rc eq 0) then $
     polymaxmin, indgen(12)+1, swtoaclr[0:11,*], fillcolor=-1, color=colorarray[iexpid], $
...
; SWSFCCLR
  read_forcing_table, expid[0], years, $
                      ...
                      swtoaall, swsfcall, swatmall, rc=rc
  if(rc eq 0) then begin
   plot, [0,13], [0,25], /nodata, $
    thick=4, xstyle=9, ystyle=9, color=0, $
    xticks=13, xtickname=xtickname, $
    xtitle = 'Month', ytitle='SWSFCCLR [W m!E-2!N]', title='SWSFCCLR'
   polymaxmin, indgen(12)+1, swsfcclr[0:11,*], color=0, fillcolor=208, lin=linarray[0], thick=12
  endif
  if(nexpid gt 1) then begin
   !quiet = 1L
   loadct, 39
   for iexpid = 1, nexpid-1 do begin
    read_forcing_table, expid[iexpid], years, $
                        ...
                        swtoaall, swsfcall, swatmall, rc=rc
    if(rc eq 0) then $
     polymaxmin, indgen(12)+1, swsfcclr[0:11,*], fillcolor=-1, color=colorarray[iexpid], $
                 lin=linarray[iexpid], thick=12
   endfor
   loadct, 0
  endif
...
; SWATMCLR
  ...
  if(rc eq 0) then begin
   plot, [0,13], [0,25], /nodata, $
    thick=4, xstyle=9, ystyle=9, color=0, $
    xticks=13, xtickname=xtickname, $
    xtitle = 'Month', ytitle='SWATMCLR [W m!E-2!N]', title='SWATMCLR'
   polymaxmin, indgen(12)+1, swatmclr[0:11,*], color=0, fillcolor=208, lin=linarray[0], thick=12
  endif
  if(nexpid gt 1) then begin
   !quiet = 1L
   loadct, 39
   for iexpid = 1, nexpid-1 do begin
    read_forcing_table, expid[iexpid], years, $
                        ...
                        swtoaall, swsfcall, swatmall, rc=rc
    if(rc eq 0) then $
     polymaxmin, indgen(12)+1, swatmclr[0:11,*], fillcolor=-1, color=colorarray[iexpid], $
                 lin=linarray[iexpid], thick=12
   endfor
   loadct, 0
  endif
