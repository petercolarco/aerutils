  ga_getvar,'modis.ctl', 'aodtau', aodmodis, lon=lon, lat=lat, lev=lev, $
   time=time, wantlev=550., wanttime=['1','12']
  aodmodis = reform(aodmodis)

  ga_getvar,'finerat.ctl', 'finerat', finemodis, lon=lon, lat=lat, lev=lev, $
   time=time, wantlev=550., wanttime=['1','12']
  finemodis = reform(finemodis)

  coarsemode = 1
  if(coarsemode) then begin
   aodmodis = aodmodis * (1.- finemodis)
   modelvar = ['du001','du002','du003','du004','du005', $
               'ss001','ss002','ss003','ss004','ss005']
   plotend = '.sample_coarse.ps'
  endif else begin
   modelvar = ['du001','du002','du003','du004','du005', $
               'ss001','ss002','ss003','ss004','ss005', $
               'so4','bcphilic','bcphobic','ocphilic','ocphobic']
   plotend = '.sample_all.ps'
  endelse
  nbin = n_elements(modelvar)
...
;  aodmodis = shift(aodmodis,72,0,0)
  ...
  for ibin = 0, nbin-1 do begin
   ...
   ga_getvar,'model.sample.ctl', modelvar[ibin], aodin, lon=lon, lat=lat, lev=lev, $
...
; Seasonalize it all
  ...
  a = where(aodModis gt 1e14 or aodmodis lt -1.e14)
