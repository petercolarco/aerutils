...
; Make a histogram plot
  set_plot, 'ps'
  device, file='ai_v_aot.daily.ps', /color, /helvetica, font_size=10, $
  ...
  plot, indgen(10), /nodata, $
   xrange=[0,4], yrange=[0,2], $
   xtitle='AI', ytitle='AOT [354 nm]', $
...
; Select -- region downwind of Africa
  ...
  plots, aia[a,*], aota[a,*], psym=3, color=0, noclip=0
  plots, aib[a,*], aotb[a,*], psym=3, color=48, noclip=0
  plots, aic[a,*], aotc[a,*], psym=3, color=80, noclip=0
  plots, aid[a,*], aotd[a,*], psym=3, color=120, noclip=0
  plots, aie[a,*], aote[a,*], psym=3, color=176, noclip=0
  plots, aif[a,*], aotf[a,*], psym=3, color=208, noclip=0
  plots, aig[a,*], aotg[a,*], psym=3, color=254, noclip=0

  tmp0 = aia[a,*] & tmp1 = aota[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=0, noclip=0
  print, res[1]

  tmp0 = aib[a,*] & tmp1 = aotb[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=48, noclip=0
  print, res[1]

  tmp0 = aic[a,*] & tmp1 = aotc[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=80, noclip=0
  print, res[1]

  tmp0 = aid[a,*] & tmp1 = aotd[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=120, noclip=0
  print, res[1]

  tmp0 = aie[a,*] & tmp1 = aote[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=176, noclip=0
  print, res[1]

  tmp0 = aif[a,*] & tmp1 = aotf[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=208, noclip=0
  print, res[1]

  tmp0 = aig[a,*] & tmp1 = aotg[a,*]
  tmp0 = tmp0[where(finite(tmp0) eq 1)] & tmp1 = tmp1[where(finite(tmp1) eq 1)]
  res = linfit(tmp0,tmp1)
  plots, findgen(5), res[0]+res[1]*findgen(5), thick=6, color=254, noclip=0
  print, res[1]
