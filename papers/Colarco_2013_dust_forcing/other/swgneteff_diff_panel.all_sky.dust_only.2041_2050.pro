...
; Get the forcing for first experiment
  ...
  force0[*] = !values.f_nan
  a = where(aot gt 0.01)
  force0[a] = ( (swgnetc[a]-swgnetc_ss[a]) - (swgnetcna[a]-swgnetcna_ss[a]) )/aot[a]
...
; Get the forcing for baseline experiment
  ...
  force3[*] = !values.f_nan
  a = where(aot gt 0.01)
  force3[a] = ( (swgnetc[a]-swgnetc_ss[a]) - (swgnetcna[a]-swgnetcna_ss[a]) )/aot[a]

; Make a plot
  set_plot, 'ps'
  device, file='swgneteff_panel_du_all_sky_dust_only.'+expid0+'.2041_2050.eps', $
