...
; Get the lifetimes
  expid = ['b_F25b9-base-v1',$      ; no forcing
           ...
           'bF_F25b9-kok-v1' ]      ; OPAC-Spheres - Kok PSD

  set_plot, 'ps'
  device, file = './plot_totexttau.'+expid[0]+'.aeronet.eps', /color, /helvetica, $
   font_size=12, xsize=32, xoff=.5, ysize=44, yoff=.5, /encap
  !P.font=0
  !p.multi=[0,2,4]
  loadct, 0
...
;  for iex = 2, 2 do begin
  if(iex eq n_elements(site)-1) then !p.multi=[1,2,4]
...
; Dust
  ...
  filename = '/Volumes/bender/prc14/colarco/'+expid[0]+'/tavg2d_carma_x/'+expid[0]+'.tavg2d_carma_x.monthly.clim.M%m2.nc4'
...
; Compute the standard deviation
  ...
  if(rc eq 0) then begin
   plot, [0,13], [0,ymax[iex]], /nodata, $
    ...
    xtitle = 'Month', ytitle='AOT', title=site_title[iex], charsize=1.5
...
;goto, jump
  if(nexpid gt 1) then begin
   ...
   for iexpid = 1, nexpid-1 do begin
    ...
    tdef = 'tdef time 12 linear 12z15jan2011 1mo'
    filename = '/Volumes/bender/prc14/colarco/'+expid[iexpid]+'/tavg2d_carma_x/'+expid[iexpid]+'.tavg2d_carma_x.monthly.clim.M%m2.nc4'
    nc4readvar, filename, 'totexttau', exttau, nymd=nymd, rc=rc, tdef=tdef, $
                wantlon=lon[iex], wantlat=lat[iex]
    nc4readvar, filename, 'var_totexttau', var_exttau, nymd=nymd, rc=rc, tdef=tdef, $
                wantlon=lon[iex], wantlat=lat[iex]
...
; Legend hard wired
  if(aersite[iex] eq 'Bermuda') then begin
  loadct, 39
