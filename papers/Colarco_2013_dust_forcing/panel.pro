...
;  find levels lt 0
  ...
  map_grid, /box
  if(keyword_set(title1)) then xyouts, .05, .95, title1, /normal

  if(keyword_set(title0)) then xyouts, 0.05, 0.98, title0, /normal, charsize=1.2
  ...
  if(x[0] ne -1) then velovect, x, y, lon, lat, /over, length=1.5, thick=3
...
;  find levels lt 0
  ...
  map_grid, /box
  if(keyword_set(title2)) then xyouts, .05, .64, title2, /normal
...
;  find levels lt 0
  ...
  map_grid, /box
  if(keyword_set(title3)) then xyouts, .05, .33, title3, /normal
...
nokey = 0
  ...
  if(not(nokey)) then begin
   makekey, .05, .025, .9, .015, 0, -.02, color=dcolors, label=dlabelarray, $
    align=.5
  endif
