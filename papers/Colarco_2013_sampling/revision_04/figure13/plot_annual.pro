...
; Apply LAR derived correction factors (valid only for MYD04) for sub-sampling
  ...
  pro plot_annual, satid, sample, yyyy, res=res, exclude=exclude, geolimits=geolimits, geoname=geoname, inverse=inverse, $
                   iplot=iplot, tplot=tplot, model=model
...
print, iplot
  case sample of
   ...
   'modis_aqua'        : sample_title = ' Full Swath '
   'misr_aqua'         : sample_title = ' Narrow Swath '
   'calipso'           : sample_title = ' Curtain '
   'modis_terra'       : sample_title = ' Terra Full Swath '
   else        : sample_title = ' Full Swath '
  endcase
  samplestr = '.'+sample
  if(sample_title eq ' Full Swath ' and satid ne 'MERRAero') then samplestr = ''

  satstr = ''
  if(satid eq 'MOD04') then satstr = 'MODIS Terra'
  if(satid eq 'MYD04') then satstr = 'MODIS Aqua'
  if(satid eq 'MERRAero') then satstr = 'MERRAero'
...
; Sample swath
  ...
  if(keyword_set(model)) then begin
    dset = '/Users/pcolarco/data/dR_MERRA-AA-r2/inst2d_hwl_x/dR_MERRA-AA-r2.inst2d_hwl_x'+samplestr+'.%y4.nc4'
    filename = strtemplate(dset, nymd, nhms)
print, filename
    nc4readvar, filename, 'totexttau', aoto, lon=lon, lat=lat
    aotl = aoto
  endif else begin
...
; Resstr
  ...
  title = satstr+ sample_title + 'AOT ('+yyyy+')'
  if(keyword_set(exclude)) then $
   title = 'MODIS Aqua Full Swath AOT where'+sample_title+'does not observe'
  if(keyword_set(exclude) and keyword_set(inverse)) then $
   title = 'MODIS Aqua Full Swath AOT only where'+sample_title+'observes'

  xyouts, position[0,iplot], position[3,iplot]+yoff, lstr+title, /normal, charsize=1.2
