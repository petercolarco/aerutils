...
; Get a mask value
  nc4readvar, '/Users/pcolarco/data/dR_MERRA-AA-r2/tavg2d_aer_x/dR_MERRA-AA-r2.tavg2d_aer_x.monthly.200301.nc4', $
...
;  polymaxmin, x, y, thick=8, fill=200
  for isample = 0, 9 do begin
   oplot, x, globaot[*,0,isample], thick=8, color=120
  endfor
...
;  polymaxmin, x, y, thick=8, fill=200
  ...
  for isample = 0, 9 do begin
   oplot, x, globaot[*,1,isample], thick=8, color=120
  endfor
...
;  polyfill, [0,max(x)+1,max(x)+1,0,0], [-.01,-.01,.01,.01,-.01], color=200
  ...
  for isample = 1, 9 do begin
   oplot, x, globaot[*,0,isample]-globaot[*,0,0], thick=8, color=120
  endfor
...
;  polyfill, [0,max(x)+1,max(x)+1,0,0], [-.01,-.01,.01,.01,-.01], color=200
  ...
  for isample = 1, 9 do begin
   oplot, x, globaot[*,1,isample]-globaot[*,1,0], thick=8, color=120
  endfor
