...
; regional maximums
  ymax = [0.16, 0.05, 0.08, 0.25, 0.18, 0.16, 0.16, 0.08]
...
;  oplot, x, delta, thick=8
  ...
  if(ireg eq 1) then begin
   plots, [2,5], .0925*ymax[ireg]*10., thick=8, color=254
   plots, [6,8], .086*ymax[ireg]*10., thick=10, color=254, lin=1
   plots, [6,8], .080*ymax[ireg]*10., thick=10, color=254, lin=2
   xyouts, 6, .091*ymax[ireg]*10., '!9D!3AOT (along-track)', charsize=.8, color=254
   xyouts, 9, .085*ymax[ireg]*10., 'FS, C1, C2, C4 only', charsize=.6, color=254
   xyouts, 9, .079*ymax[ireg]*10., 'FS, N1, N2, N4 only', charsize=.6, color=254
   plots, [20,23], .0925*ymax[ireg]*10., thick=8, color=75
   plots, [2,5], .075*ymax[ireg]*10., thick=8
   xyouts, 24, .091*ymax[ireg]*10., '!9D!3AOT (across-track)', charsize=.8, color=75
   plots, [24,26], .086*ymax[ireg]*10., thick=10, color=75, lin=0
   plots, [24,26], .080*ymax[ireg]*10., thick=10, color=75, lin=2
   xyouts, 27, .085*ymax[ireg]*10., 'sample-then-average', charsize=.6, color=75
   xyouts, 27, .079*ymax[ireg]*10., 'average-then-mask', charsize=.6, color=75
   xyouts, 6, .0735*ymax[ireg]*10., '!9D!3AOT (MERRAero)', charsize=.8
   loadct, 0
   polyfill, [20,23,23,20,20], ymax[ireg]*10.*[.073,.073,.077,.077,.073], color=200, /fill
   xyouts, 24, .0735*ymax[ireg]*10., '!9D!3Forcing (MERRAero)', charsize=.8, color=100
   loadct, 39
  endif
