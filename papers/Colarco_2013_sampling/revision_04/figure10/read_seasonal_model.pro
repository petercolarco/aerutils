...
; If keyword "num" is set then just withdraw the number of retrievals
  ...
  pro read_seasonal_model, model, sample, yyyy, season, aotsat, res=res, exclude=exclude, inverse=inverse, $
  ...
  if(model eq 'dR_F25b18') then modeldir = '/Users/pcolarco/data/'+model+'/inst2d_hwl_x/'
  if(model eq 'dR_MERRA-AA-r2') then modeldir = '/Users/pcolarco/data/'+model+'/inst2d_hwl_x/'
...
; Read the land and ocean AOT for the requested sample
  ...
  if(model eq 'dR_F25b18') then filetemplate = modeldir +'/'+ model + '.inst2d_hwl_x'+samplestr+'.%y4'+season+'.nc4'
  if(model eq 'dR_MERRA-AA-r2') then filetemplate = modeldir +'/'+ model + '.inst2d_hwl_x'+samplestr+'.%y4'+season+'.nc4'
