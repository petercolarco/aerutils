...
; If keyword "num" is set then just withdraw the number of retrievals
  ...
  pro read_seasonal_model_surf, model, sample, yyyy, season, aotsat, res=res, exclude=exclude, inverse=inverse, $
                     lon0=lon0, lon1=lon1, lat0=lat0, lat1=lat1, reg_aot=reg_aot, reg_std=reg_std, num=num, pdf=pdf, $
                     clear=clear ; clearsky

  if(not(keyword_set(res))) then res = 'd'
  area, x, y, nx, ny, dx, dy, area, grid = res

  case sample of
   'misr_aqua' : sample_title = ' N '
   'calipso'   : sample_title = ' C '
  ...
  if(model eq 'dR_F25b18') then modeldir = '/Users/pcolarco/data/'+model+'/geosgcm_surf/'
  if(model eq 'dR_MERRA-AA-r2') then modeldir = '/Users/pcolarco/data/'+model+'/geosgcm_surf/'
...
; Read the land and ocean AOT for the requested sample
  if(model eq 'c1000d') then filetemplate = modeldir +'/'+ model + '.tinst_2d_wms_chem_Lx.totexttau'+samplestr+'.%y4'+season+'.nc4'
  if(model eq 'dR_F25b18') then filetemplate = modeldir +'/'+ model + '.geosgcm_surf'+samplestr+'.%y4'+season+'.nc4'
  if(model eq 'dR_MERRA-AA-r2') then filetemplate = modeldir +'/'+ model + '.geosgcm_surf'+samplestr+'.%y4'+season+'.nc4'
  filename = strtemplate(filetemplate,nymd,nhms)
print, filename
  if(keyword_set(clear)) then begin
   nc4readvar, filename, 'swtnetc', swtnet, /sum, lon=lon, lat=lat, lev=lev
   nc4readvar, filename, 'swtnetcna', swtnetna, /sum, lon=lon, lat=lat, lev=lev
  endif else begin
   nc4readvar, filename, 'swtnet', swtnet, /sum, lon=lon, lat=lat, lev=lev
   nc4readvar, filename, 'swtnetna', swtnetna, /sum, lon=lon, lat=lat, lev=lev
  endelse
  aoto=swtnet-swtnetna
  aotl = aoto
