...
; Resstr
  ...
  if(keyword_set(exclude)) then $
   title = 'Full AOT swath where'+sample_title+'does not observe ('+yyyy+' '+season+')'
  if(keyword_set(exclude) and keyword_set(inverse)) then $
   title = 'Full AOT swath only where'+sample_title+'observes ('+yyyy+' '+season+')'

  xyouts, position[0,iplot], position[3,iplot]+.05, lstr+title, /normal, charsize=1.2
