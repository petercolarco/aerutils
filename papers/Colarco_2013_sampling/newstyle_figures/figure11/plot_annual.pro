...
; plot missing data as light shade
  ...
  dlevels = [0.,0.05,0.1,0.15,0.2,0.25,0.3,0.4,0.5]
  xyouts, .5, .13, 'AOT', /normal, charsize=1.2, align=.5
  levstr = string(dlevels,format='(f4.2)')
  makekey, .1, .065, .8, .05, 0, -.035, color=make_array(n_elements(dlevels),val=255), $
   align=0, label=levstr



  red   = [255,255,254,254,254,236,204,153,102]
  green = [255,247,227,196,153,112,76,52,37]
  blue  = [229,188,145,79,41,20,2,4,6]
  ncolor = n_elements(dlevels)
  tvlct, red, green, blue
  dcolors=indgen(ncolor)

  plotgrid, aotsat, dlevels, dcolors, lon, lat, dx, dy, /map

  levstr = string(dlevels,format='(f4.2)')
  makekey, .1, .065, .8, .05, 0, -.035, color=dcolors, $
   align=0, label=make_array(n_elements(dlevels),val=' ')
