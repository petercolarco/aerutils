...
; plot missing data as light shade
  ...
  dlevels = findgen(11)*.4-2.1
  dlevels[0] = -2000.
  dlevels = [-2000.,-0.2,-0.15,-0.1,-.05,-.02,.02,.05,0.1,0.15,0.2]
  ncolor = n_elements(dlevels)
  red   = [49,69,116,171,224,255,254,253,244,215,165]
  green = [54,117,173,217,243,255,224,174,109,48,0]
  blue  = [149,180,209,233,248,191,144,97,67,39,38]
  tvlct, red, green, blue
  dcolors=indgen(ncolor)

; mask missing data
  slope[where(finite(abs(slope/tstd)) eq 0)] = !values.f_nan
  plotgrid, slope*10., dlevels, dcolors, lon, lat, dx, dy, /map
...
; Resstr
  ...
  levstr[0:10] = ' '
  levstr = [' ','-0.2','-0.15','-0.1','-0.05','-0.02','0.02','0.05','0.1','0.15','0.2']
  if(tplot eq 1) then begin
   title = 'MODIS Aqua'+ sample_title + '('+resstr+', AOT trend !9D!3AOT decade!E-1!N [550 nm])'
  ...
  if(tplot eq 4) then begin
   ...
   xyouts, position[0,iplot], position[3,iplot]+.025, lstr+title, /normal, charsize=1.2
   if(iplot eq 0) then begin
    title = 'AOT trend !9D!3AOT decade!E-1!N'
