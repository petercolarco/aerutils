...
print, isample, globaot[*,1,isample]
  ...
  !p.multi = [0,1,2]
  set_plot, 'ps'
  device, file='modis_aot_annual.'+satid+'.'+res+'.ocn.eps', /color, /helvetica, font_size=12, $
...
;  polyfill, [0,max(x)+1,max(x)+1,0,0], [-.01,-.01,.01,.01,-.01], color=200
  ...
  !p.multi = [0,1,2]
  set_plot, 'ps'
  device, file='modis_aot_annual.'+satid+'.'+res+'.lnd.eps', /color, /helvetica, font_size=12, $
   xsize=16, ysize=20, xoff=.5, yoff=.5, /encap
  !p.font=0
  loadct, 0
