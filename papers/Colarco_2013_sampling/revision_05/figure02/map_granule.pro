...
; Get the MODIS full swath
  ...
  nc4readvar, fdir+'/dR_MERRA-AA-r2.inst2d_hwl_x.modis_aqua.20100605.times.nc4', $
              'totexttau', aot, lon=lon_, lat=lat_
  a = where(aot ge 1e14)
  aot[a] = !values.f_nan
  dx = 0.625
  dy = 0.5
...
; MODIS along-track sampling
  odsfile = '/Volumes/bender/science/modis/data/Level3/MYD04/ODS/Y2010/M06/MYD04_L2_051.aero_tc8.ocean.20100605.ods'
  kx = 311
  kt = 45
  ktqa = 74
  qathresh = 1
  ...
  odsfile = '/Volumes/bender/science/modis/data/Level3/MYD04/ODS/Y2010/M06/MYD04_L2_051.aero_tc8.land.20100605.ods'
  kx = 312
  kt = 45
  ktqa = 74
  qathresh = 3
  readods, odsfile, kx, kt, lat__, lon__, lev__, time__, obs, rc=fail, ks=ks__, $
           ktqa=ktqa, qathresh=qathresh, obsq=obsq
...
;  polyfill, [-131,-105,-118,-140,-131],[-40,-40,20,20,-40], color=255
  polyfill, [-170,-95,-95,-170,-170], [-40,-40,60,60,-40], color=220
;  polyfill, [-131,-105,-125,-157,-131], [-40,-40,60,60,-40], color=255
  plotgrid, aot[*,*,22], [0.], [254], lon_, lat_, dx, dy
  plotgrid, aot[*,*,23], [0.], [254], lon_, lat_, dx, dy
  xyouts, -135, 20, 'glint', /data, charsize=2, align=.5, orient=-80
...
; and set the filled flag:
  ...
  map_continents
  map_grid, /box, londel=10, latdel=10, $
            lons=findgen(10)*10-170, $
            lats=findgen(10)*10-40.
...
; Get the MODIS full swath
  ...
  a = where(aot ge 1e14)
  aot[a] = !values.f_nan
  a = where(lon lt -142 or lon gt -110)
  aot[a,*] = !values.f_nan
...
; Fill in extraneous points from other orbits
  loadct, 0
  polyfill, [-45,-140,-140,-145,-145], [30,30,36,36,30], color=100
  polyfill, [-142.5,-135,-135,-142.5,-142.5], [18.5,18.5,30,30,18.5], color=100
  polyfill, [-118,-105,-105,-118,-118], [25,25,60,60,25], color=220
...
; Legend
  ...
  loadct, 39
  plots, [.465,.505], .03, color=254, thick=8, /normal
