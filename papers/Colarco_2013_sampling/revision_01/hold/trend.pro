...
; Plot the trend
  ...
  device, file='./MYD04'+samplestr+'.qast.'+res+'.num'+'.trend.eps', $
          ...
          xoff=.5, yoff=.5, xsize=24, ysize=10, /encap
 !p.font=0
  p0 = 0
  p1 = 0
...
; Resstr
  ...
  title = '!4(a)!3 MODIS Aqua'+ sample_title + '(AOT trend 100*AOT yr!E-1!N [550 nm])'
  xyouts, .025, .95, title, /normal

  levstr = string(dlevels,format='(f4.1)')
  levstr[0:20:2] = ' '
  levstr = [' ','-1.9',' ','-1.5',' ','-1.1',' ','-0.7',' ','-0.3',' ',$
            ' ','-0.3',' ','0.7',' ','1.1',' ','1.5',' ','1.9',' ']
  makekey, .025, .075, .45, .05, 0, -.035, color=dcolors, $
...
;  endcase
  ...
  title = '!4(b)!3 MODIS Aqua'+ sample_title + '(trend significance)'
  xyouts, .525, .95, title, /normal

  makekey, .675, .075, .3, .05, 0, -.035, color=[255,254], $
   label=[' ',' ',' ']
  xyouts, .6, .03, 'No Data', /normal, align=.5
  xyouts, .75, .03, 'Trend Not Significant', /normal, align=.5
  xyouts, .9, .03, 'Trend Significant!Cat 95% Confidence', /normal, align=.5
 device, /close
